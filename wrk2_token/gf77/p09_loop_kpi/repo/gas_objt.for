...
!
            IF(.NOT. SAVE_GAS_DEMAND_FILE_EXISTS) THEN
               allocate(TEMP_TRANS_CLASS_POINTER(0:1023),stat=stv_er)
               call check_alloc("gas_objt:0001",
     +                              "TEMP_TRANS_CLASS_POINTER",stv_er)
               allocate(TEMP_ASSET_CLASS_POINTER(0:1023),stat=stv_er)
               call check_alloc("gas_objt:0002",
     +                              "TEMP_ASSET_CLASS_POINTER",stv_er)
               allocate(TEMP_CUST_CLASS_POINTER(0:1023),stat=stv_er)
               call check_alloc("gas_objt:0003",
     +                               "TEMP_CUST_CLASS_POINTER",stv_er)
...
!                  IF(GAS_DEMAND_YEAR /= YEAR - STUDY_BASE_YEAR) THEN
!                     WRITE(4,*) 'The Base TSYtem Forecast File in'
!                     WRITE(4,*) 'Record',GAS_DEMAND_YEAR,
!                     WRITE(4,*) '  and Year',YEAR
...
!                  CALL CALCULATE_TSYTEM_PEAKS(TSY_FC_DATA,GAS_DEMAND_YEAR)
               ENDDO ! LOAD GROUP
...
!      WRITE(6,1010) FILE_TYPE
      ...
      ENDDO
      DO FILE_ID = 0, MAX_GAS_DEMAND_FILES-1
         IF(.NOT. ACTIVE_BASE_GAS_DEMAND_FILES(FILE_ID)) THEN 
           CYCLE
         ENDIF
...
!         
            allocate(TEMP_TRANS_CLASS_POINTER(0:1023),stat=stv_er)
            call check_alloc("gas_objt:0004",
     +                              "TEMP_TRANS_CLASS_POINTER",stv_er)
            allocate(TEMP_ASSET_CLASS_POINTER(0:1023),stat=stv_er)
            call check_alloc("gas_objt:0005",
     +                              "TEMP_ASSET_CLASS_POINTER",stv_er)
            allocate(TEMP_CUST_CLASS_POINTER(0:1023),stat=stv_er)
            call check_alloc("gas_objt:0006",
     +                               "TEMP_CUST_CLASS_POINTER",stv_er)
 ...
 !    +                     COMMENT, ! CHAR*50
     ...
     +                     INTERRUPTIBLE_PERCENT
         IF(IOS /= 0) THEN 
           EXIT
         ENDIF
!         CALL CALCULATE_TSYTEM_PEAKS(TSY_FC_DATA,YEAR)
      ENDDO
      CLOSE(10)
      RETURN
!
...
!
      LOGICAL*1   WRITE_MTHLY_GAS_DEM_CLASS_SUMM,
...
!         
     ...
     +                         0:MAX_TRANS_LOAD_TABLES),stat=stv_er) 
         call check_alloc("gas_objt:0007",
     +                           "POWER_DAILY_VOLUME_BY_MONTH",stv_er)
...
!
         IF( .NOT. ALLOCATED(CUSTOMER_GROUP)) THEN
            ALLOCATE(CUSTOMER_GROUP(MAX_TRANS_LOAD_TABLES),stat=stv_er) ! INT2
            call check_alloc("gas_objt:0008","CUSTOMER_GROUP",stv_er)
     
            allocate(iPort(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0009","iPort",stv_er)
            allocate(SNamePos(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0010","SNamePos",stv_er)
      
            ALLOCATE(CUSTOMER_GROUP_2(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0011","CUSTOMER_GROUP_2",stv_er)
     
            allocate(SPECIAL_CUSTOMER_TYPE(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0012",
     +                                 "SPECIAL_CUSTOMER_TYPE",stv_er)
            allocate(INTERRUPTIBLE_MARKUP_PERENT(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0013",
     +                           "INTERRUPTIBLE_MARKUP_PERENT",stv_er)
            allocate(INTERRUPTIBLE_MARKUP_CAP(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0014",
     +                              "INTERRUPTIBLE_MARKUP_CAP",stv_er)
            allocate(INTERRUPTIBLE_MAX_CAPACITY(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0015",
     +                            "INTERRUPTIBLE_MAX_CAPACITY",stv_er)
            allocate(INTERRUPTIBLE_MIN_CAPACITY(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0016",
     +                            "INTERRUPTIBLE_MIN_CAPACITY",stv_er)
            allocate(INTERRUPTIBLE_MARKUP_ADDER(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0017",
     +                            "INTERRUPTIBLE_MARKUP_ADDER",stv_er)
            allocate(INTERRUPTIBLE_PERCENT(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0018",
     +                                 "INTERRUPTIBLE_PERCENT",stv_er)
            allocate(
     +        HOURLY_INTERRUPIBLE_REVENUE(MAX_TRANS_LOAD_TABLES,24,4),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0019",
     +                           "HOURLY_INTERRUPIBLE_REVENUE",stv_er)
      
            ALLOCATE(GAS_GROUP_ID(MAX_TRANS_LOAD_TABLES),stat=stv_er) ! INT2
            call check_alloc("gas_objt:0020","GAS_GROUP_ID",stv_er)
     
            ALLOCATE(REFERENCE_LOAD_NUMBER(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0021",
     +                                 "REFERENCE_LOAD_NUMBER",stv_er)
     
            ALLOCATE(MARKET_ENERGY_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0022",
     +                                   "MARKET_ENERGY_PRICE",stv_er)
     
            ALLOCATE(ANNUAL_TG_PEAK(0:12,MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0023","ANNUAL_TG_PEAK",stv_er)
     
            ALLOCATE(ANNUAL_TG_VOLUME(0:MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0024","ANNUAL_TG_VOLUME",stv_er)
     
            IF(.NOT. ALLOCATED(ANNUAL_TG_POWER_VOLUME)) THEN
               ALLOCATE(ANNUAL_TG_POWER_VOLUME(0:MAX_TRANS_GROUPS),
     +                                                    stat=stv_er)
               call check_alloc("gas_objt:0025",
     +                                "ANNUAL_TG_POWER_VOLUME",stv_er)
     
            ENDIF
            ALLOCATE(MONTHLY_ENERGY_PRICE_PATTERN(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0026",
     +                          "MONTHLY_ENERGY_PRICE_PATTERN",stv_er)
     
            ALLOCATE(MARKET_DEMAND_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0027",
     +                                   "MARKET_DEMAND_PRICE",stv_er)
     
            ALLOCATE(MONTHLY_DEMAND_PRICE_PATTERN(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0028",
     +                          "MONTHLY_DEMAND_PRICE_PATTERN",stv_er)
     
            ALLOCATE(MARKET_CUSTOMER_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0029",
     +                                 "MARKET_CUSTOMER_PRICE",stv_er)
     
            ALLOCATE(ASSET_CLASS_ID(MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0030","ASSET_CLASS_ID",stv_er)
     
            ALLOCATE(ASSET_CLASS_REV_ALLOC_VECTOR(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0031",
     +                          "ASSET_CLASS_REV_ALLOC_VECTOR",stv_er)
     
            ALLOCATE(ANNUAL_ENERGY(MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0032","ANNUAL_ENERGY",stv_er)
     
            ALLOCATE(ANNUAL_PEAK(MAX_TRANS_LOAD_TABLES),stat=stv_er) ! REAL4
            call check_alloc("gas_objt:0033","ANNUAL_PEAK",stv_er)
     
            ALLOCATE(ANNUAL_CUSTOMERS(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0034","ANNUAL_CUSTOMERS",stv_er)
     
            ALLOCATE(ANNUAL_MULTIPLIER(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0035","ANNUAL_MULTIPLIER",stv_er)
     
            ALLOCATE(MONTHLY_ENERGY(12,MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0036","MONTHLY_ENERGY",stv_er)
     
            ALLOCATE(MONTHLY_PEAK(12,MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0037","MONTHLY_PEAK",stv_er)
     
            ALLOCATE(MONTHLY_CUSTOMERS(12,MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0038","MONTHLY_CUSTOMERS",stv_er)
     
            ALLOCATE(DIST_ENERGY_LOSS_FACTOR(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0039",
     +                               "DIST_ENERGY_LOSS_FACTOR",stv_er)
     
            ALLOCATE(TRANS_ENERGY_LOSS_FACTOR(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0040",
     +                              "TRANS_ENERGY_LOSS_FACTOR",stv_er)
     
            ALLOCATE(PEAK_LOSS_FACTOR(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0041","PEAK_LOSS_FACTOR",stv_er)
     
            ALLOCATE(PEAK_COIN_FACTOR(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0042","PEAK_COIN_FACTOR",stv_er)
     
            ALLOCATE(DISTRIBUTION_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0043",
     +                                    "DISTRIBUTION_PRICE",stv_er)
     
            ALLOCATE(TRANSMISSION_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0044",
     +                                    "TRANSMISSION_PRICE",stv_er)
     
            ALLOCATE(CUSTOMER_CLASS_NAME(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0045",
     +                                   "CUSTOMER_CLASS_NAME",stv_er)
     
            ALLOCATE(CALCULATION_MODE(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0046","CALCULATION_MODE",stv_er)
     
            ALLOCATE(REFERENCE_LOAD_NAME(MAX_TRANS_LOAD_TABLES),
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0047",
     +                                   "REFERENCE_LOAD_NAME",stv_er)
     
            ALLOCATE(TRANS_MONTHLY_ENERGY(12,MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0048",
     +                                  "TRANS_MONTHLY_ENERGY",stv_er)
     
            ALLOCATE(DEMAND_GROUP_AVAILABLE(MAX_TRANS_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0049",
     +                                "DEMAND_GROUP_AVAILABLE",stv_er)
     
            ALLOCATE(WH_TRANS_MONTHLY_ENERGY(12,
     +                        MAX_TRANS_GROUPS_FROM_TG),stat=stv_er) 
            call check_alloc("gas_objt:0050",
     +                               "WH_TRANS_MONTHLY_ENERGY",stv_er)
     
            ALLOCATE(WH_TRANS_MONTHLY_CAPACITY(12,
     +                        MAX_TRANS_GROUPS_FROM_TG),stat=stv_er) 
            call check_alloc("gas_objt:0051",
     +                             "WH_TRANS_MONTHLY_CAPACITY",stv_er)
     
            ALLOCATE(TRANS_MONTHLY_PEAK(12,MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0052",
     +                                    "TRANS_MONTHLY_PEAK",stv_er)
     
            ALLOCATE(TRANS_MONTHLY_CUSTOMERS(12,MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0053",
     +                               "TRANS_MONTHLY_CUSTOMERS",stv_er)
     
            ALLOCATE(TABLE_ACTIVE(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0054","TABLE_ACTIVE",stv_er)
     
            ALLOCATE(BASECASE_MARKET_AREA_ID(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0055",
     +                               "BASECASE_MARKET_AREA_ID",stv_er)
     
            ALLOCATE(WD_INDEX(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0056","WD_INDEX",stv_er)
     
            ALLOCATE(GW_INDEX(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0057","GW_INDEX",stv_er)
     
            ALLOCATE(BASECASE_TRANS_AREA_ID(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0058",
     +                                "BASECASE_TRANS_AREA_ID",stv_er)
     
            ALLOCATE(BASECASE_NERC_SUB_ID(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0059",
     +                                  "BASECASE_NERC_SUB_ID",stv_er)
     
            ALLOCATE(MONTHLY_UNITS(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0060","MONTHLY_UNITS",stv_er)
     
            ALLOCATE(PRICE_INDEX_ACTIVE(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0061",
     +                                    "PRICE_INDEX_ACTIVE",stv_er)
     
            ALLOCATE(THREE_FACTOR_TRANSFORM(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0062",
     +                                "THREE_FACTOR_TRANSFORM",stv_er)
     
            ALLOCATE(JURISDICTIONAL_CUSTOMER(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0063",
     +                               "JURISDICTIONAL_CUSTOMER",stv_er)
     
            ALLOCATE(FUEL_COST_RECOVERY_THROUGH_FAC(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) 
            call check_alloc("gas_objt:0064",
     +                        "FUEL_COST_RECOVERY_THROUGH_FAC",stv_er)
     
            ALLOCATE(BASE_COST_OF_FAC_FUEL(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0065",
     +                                 "BASE_COST_OF_FAC_FUEL",stv_er)
     
            ALLOCATE(MINIMUM_MARKET_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0066",
     +                                  "MINIMUM_MARKET_PRICE",stv_er)
     
            ALLOCATE(MAXIMUM_MARKET_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0067",
     +                                  "MAXIMUM_MARKET_PRICE",stv_er)
     
            ALLOCATE(INDEXED_ENERGY_PRICE(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0068",
     +                                  "INDEXED_ENERGY_PRICE",stv_er)
     
            ALLOCATE(TG_COUNTER(MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0069","TG_COUNTER",stv_er)
     
            ALLOCATE(LOAD_DISPATCH_POSITION(MAX_TRANS_LOAD_TABLES,
     +                                MAX_TRANS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0070",
     +                                "LOAD_DISPATCH_POSITION",stv_er)
     
            ALLOCATE(LOAD_DISPATCH_INDEX(MAX_TRANS_LOAD_TABLES,
     +                                MAX_TRANS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0071",
     +                                   "LOAD_DISPATCH_INDEX",stv_er)
     
!
            ALLOCATE(GAS_LOAD_GROUPS_INDEX(MAX_TRANS_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0072",
     +                                 "GAS_LOAD_GROUPS_INDEX",stv_er)
     
            ALLOCATE(CUST_CLASS_GROUPS_INDEX(0:MAX_CLASS_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0073",
     +                               "CUST_CLASS_GROUPS_INDEX",stv_er)
     
            ALLOCATE(CUST2_CLASS_GROUPS_INDEX(0:MAX_CLASS2_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0074",
     +                              "CUST2_CLASS_GROUPS_INDEX",stv_er)
     
            ALLOCATE(ASSET_CLASS_GROUPS_INDEX(0:MAX_ASSET_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0075",
     +                              "ASSET_CLASS_GROUPS_INDEX",stv_er)
     
            ALLOCATE(ASSET_2_TRANS_INDEX(0:MAX_ASSET_GROUPS,
     +                                MAX_TRANS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0076",
     +                                   "ASSET_2_TRANS_INDEX",stv_er)
     
            ALLOCATE(NUMBER_ASSET_2_TRANS(MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0077",
     +                                  "NUMBER_ASSET_2_TRANS",stv_er)
     
            ALLOCATE(ASSET_TRANSACTION_CROSS_INDEX(0:MAX_ASSET_GROUPS,
     +                                MAX_TRANS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0078",
     +                         "ASSET_TRANSACTION_CROSS_INDEX",stv_er)
     
            ALLOCATE(NUMBER_TRANS_PER_AC_TG(0:MAX_ASSET_GROUPS,
     +                                MAX_TRANS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0079",
     +                                "NUMBER_TRANS_PER_AC_TG",stv_er)
     
            ALLOCATE(TRANS_WITHIN_AC_TG(0:MAX_ASSET_GROUPS,
     +          MAX_TRANS_GROUPS,MAX_TRANS_LOAD_TABLES),stat=stv_er) 
            call check_alloc("gas_objt:0080",
     +                                    "TRANS_WITHIN_AC_TG",stv_er)
     
            ALLOCATE(FIRST_AC_TG(0:MAX_ASSET_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0081","FIRST_AC_TG",stv_er)
     
            ALLOCATE(FIRST_TABLE_TG(0:MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0082","FIRST_TABLE_TG",stv_er)
     
            ALLOCATE(GAS_LOAD_2_TRANS_GROUPS(MAX_TRANS_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0083",
     +                               "GAS_LOAD_2_TRANS_GROUPS",stv_er)
     
            ALLOCATE(GAS_LOAD_GROUP_2_TG(MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0084",
     +                                   "GAS_LOAD_GROUP_2_TG",stv_er)
     
            ALLOCATE(CUST_CLASS_GROUP_2_CG(MAX_CLASS_GROUPS+1),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0085",
     +                                 "CUST_CLASS_GROUP_2_CG",stv_er)
     
            ALLOCATE(CUST2_CLASS_GROUP_2_CG(MAX_CLASS_GROUPS+1),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0086",
     +                                "CUST2_CLASS_GROUP_2_CG",stv_er)
     
            ALLOCATE(ASSET_CLASS_GROUP_2_AC(0:MAX_ASSET_GROUPS+1),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0087",
     +                                "ASSET_CLASS_GROUP_2_AC",stv_er)
     
            ALLOCATE(LAST_TABLE_FOR_TG(MAX_TRANS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0088","LAST_TABLE_FOR_TG",stv_er)
     
            ALLOCATE(LAST_TABLE_FOR_CG(MAX_CLASS_GROUPS+1),stat=stv_er)
            call check_alloc("gas_objt:0089","LAST_TABLE_FOR_CG",stv_er)
     
            ALLOCATE(LAST_TABLE_FOR_CG2(MAX_CLASS_GROUPS+1),stat=stv_er)
            call check_alloc("gas_objt:0090",
     +                                    "LAST_TABLE_FOR_CG2",stv_er)
     
            ALLOCATE(TG_CG_DATABASE(0:12,0:MAX_TRANS_GROUPS,
     +                            0:MAX_CLASS_GROUPS,3),stat=stv_er) 
            call check_alloc("gas_objt:0091","TG_CG_DATABASE",stv_er)
     
            ALLOCATE(TG_CG2_DATABASE(0:12,0:MAX_TRANS_GROUPS,
     +                            0:MAX_CLASS_GROUPS,3),stat=stv_er) 
            call check_alloc("gas_objt:0092","TG_CG2_DATABASE",stv_er)
     
            ALLOCATE(TABLE_DAY_SHIFT(MAX_TRANS_LOAD_TABLES),stat=stv_er)
            call check_alloc("gas_objt:0093","TABLE_DAY_SHIFT",stv_er)
     
            ALLOCATE(GAS_DEMAND_PLANNING_PEAK(0:MAX_TRANS_GROUPS,12),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0094",
     +                              "GAS_DEMAND_PLANNING_PEAK",stv_er)
     
            ALLOCATE(PA_PLANNING_PEAK(0:NUMBER_OF_ACTIVE_GROUPS,12),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0095","PA_PLANNING_PEAK",stv_er)
     
            ALLOCATE(GLOBAL_PA_PEAK(0:NUMBER_OF_ACTIVE_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0096","GLOBAL_PA_PEAK",stv_er)
     
            ALLOCATE(GLOBAL_PA_PEAK_MONTH(0:NUMBER_OF_ACTIVE_GROUPS),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0097",
     +                                  "GLOBAL_PA_PEAK_MONTH",stv_er)
     
            ALLOCATE(TG_2_PLANNING_AREA(MAX_TRANS_GROUPS_FROM_TG),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0098",
     +                                    "TG_2_PLANNING_AREA",stv_er)
     
            ALLOCATE(REV_CLASS_INDEX(0:MAX_TRANS_LOAD_TABLES,4),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0099","REV_CLASS_INDEX",stv_er)
     
            ALLOCATE(ENERGY_LOSS_MULT(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0100","ENERGY_LOSS_MULT",stv_er)
     
            ALLOCATE(REF_LEAP_YEAR_DAY_SHIFT(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0101",
     +                               "REF_LEAP_YEAR_DAY_SHIFT",stv_er)
     
            ALLOCATE(DEMAND_PRICING_METHOD(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0102",
     +                                 "DEMAND_PRICING_METHOD",stv_er)
     
            ALLOCATE(MONTHLY_TABLE_PEAK_SALES(MAX_TRANS_LOAD_TABLES,12),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0103",
     +                              "MONTHLY_TABLE_PEAK_SALES",stv_er)
     
            ALLOCATE(INTRA_COMPANY_TRANSACTION(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0104",
     +                             "INTRA_COMPANY_TRANSACTION",stv_er)
     
            ALLOCATE(INTRA_ASSET_CLASS_ID(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0105",
     +                                  "INTRA_ASSET_CLASS_ID",stv_er)
     
            ALLOCATE(INTRA_ASSET_CLASS_ALLOCATION(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) 
            call check_alloc("gas_objt:0106",
     +                          "INTRA_ASSET_CLASS_ALLOCATION",stv_er)
     
            ALLOCATE(INTRA_ACCOUNT_CLASSIFICATION(
     +                           MAX_TRANS_LOAD_TABLES),stat=stv_er) 
            call check_alloc("gas_objt:0107",
     +                          "INTRA_ACCOUNT_CLASSIFICATION",stv_er)
     
            ALLOCATE(INTRA_EXPENSE_COLLECTION(MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0108",
     +                              "INTRA_EXPENSE_COLLECTION",stv_er)
...
!
     ...
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0109","TARGET_DAILY_STORAGE",stv_er)
         allocate(AVE_DAILY_VOLUME(0:12,0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0110","AVE_DAILY_VOLUME",stv_er)
...
!     
     ...
     +                       0:MAX_ASSET_GROUPS+1,0:12),stat=stv_er) 
         call check_alloc("gas_objt:0111",
     +                         "MONTHLY_INTERRUPTIBLE_REVENUE",stv_er)
...
!
     ...
     +                                                   stat=stv_er) 
            call check_alloc("gas_objt:0112",
     +                                   "LAST_THIS_YR_ENERGY",stv_er)
     
            ALLOCATE(LAST_THIS_YR_PEAK(2,12,0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
            call check_alloc("gas_objt:0113","LAST_THIS_YR_PEAK",stv_er)
...
!
            DO TRANS = 1, MAX_TRANS_LOAD_TABLES
               ...
               IF(TEMP_I2 == 0 .OR. 
     +                        GAS_LOAD_GROUPS_INDEX(TEMP_I2) == 0) THEN 
                 CYCLE
               ENDIF
!               IF(TG == 60) CYCLE ! TEMP
...
!
     ...
     +                   0:MAX_ASSET_CLASS_GROUPS,0:12),stat=stv_er) 
         call check_alloc("gas_objt:0114",
     +                             "MONTHLY_AC_COST_AT_MARKET",stv_er)
     
         ALLOCATE(MONTHLY_AC_CONTRACT_REVENUE(
     +                      -1:MAX_ASSET_CLASS_GROUPS,4,0:12,
     +                            0:LAST_INCOME_LINE,3),stat=stv_er) !msg thinks the order is 3=BUSBAR and 2=meter NOT 1=$,2=BUSBAR,3=METER
         call check_alloc("gas_objt:0115",
     +                           "MONTHLY_AC_CONTRACT_REVENUE",stv_er)
     
         ALLOCATE(MONTHLY_AC_CONTRACT_EXPENSE(
     +                      -1:MAX_ASSET_CLASS_GROUPS,0:12,
     +                             0:LAST_EXPENSE_ITEM),stat=stv_er) !msg thinks the order is 3=BUSBAR and 2=meter NOT 1=$,2=BUSBAR,3=METER
         call check_alloc("gas_objt:0116",
     +                           "MONTHLY_AC_CONTRACT_EXPENSE",stv_er)
...
!
     ...
     +                                  MAX_GAS_LOAD_GROUPS == 0) THEN
         ...
         ALLOCATE(DAY_OF_WEEK(31,0:MAX_TRANS_LOAD_TABLES),stat=stv_er)
         call check_alloc("gas_objt:0117","DAY_OF_WEEK",stv_er)
...
!
     ...
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0118",
     +                           "ANNUAL_CLASS_ENERGY_REVENUE",stv_er)
     
            ALLOCATE(
     +           ANNUAL_CLASS_PEAK_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0119",
     +                             "ANNUAL_CLASS_PEAK_REVENUE",stv_er)
     
            ALLOCATE(
     +       ANNUAL_CLASS_CUSTOMER_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0120",
     +                         "ANNUAL_CLASS_CUSTOMER_REVENUE",stv_er)
     
            ALLOCATE(
     +     ANNUAL_CLASS_ENERGY(0:MAX_CUST_CLASS_GROUPS),stat=stv_er) 
            call check_alloc("gas_objt:0121",
     +                                   "ANNUAL_CLASS_ENERGY",stv_er)
     
            ALLOCATE(
     +           ANNUAL_CLASS_PEAK(0:MAX_CUST_CLASS_GROUPS),stat=stv_er)
            call check_alloc("gas_objt:0122","ANNUAL_CLASS_PEAK",stv_er)
     
            ALLOCATE(
     +              ANNUAL_CLASS_CUSTOMERS(0:MAX_CUST_CLASS_GROUPS),
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0123",
     +                                "ANNUAL_CLASS_CUSTOMERS",stv_er)
     
            ALLOCATE(
     +        ANNUAL_TRANS_INDEXED_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0124",
     +                          "ANNUAL_TRANS_INDEXED_REVENUE",stv_er)
...
!
     ...
     +        MAX_HOURS_IN_MONTH,0:MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0125","TRANS_HOURLY_LOAD",stv_er)
     
         ALLOCATE(TRANS_DAILY_LOAD(
     +       0:MAX_DAYS_IN_MONTH,0:MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0126","TRANS_DAILY_LOAD",stv_er)
     
         ALLOCATE(INTER_DAILY_AVAIL(
     +         MAX_DAYS_IN_MONTH,0:MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0127","INTER_DAILY_AVAIL",stv_er)
     
         ALLOCATE(CUSTOMER_DAILY_LOAD(
     +       MAX_DAYS_IN_MONTH,0:MAX_CUST_CLASS_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0128","CUSTOMER_DAILY_LOAD",stv_er)
     
         ALLOCATE(WH_LOADS_PER_HOUR(
     +        MAX_HOURS_IN_MONTH,0:MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0129","WH_LOADS_PER_HOUR",stv_er)
     
         ALLOCATE(HYDRO_HOURLY_LOAD( ! NEED TO COUNT HYDRO GROUPS
     +     MAX_HOURS_IN_MONTH,0:NUMBER_OF_HYDRO_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0130","HYDRO_HOURLY_LOAD",stv_er)
     
         ALLOCATE(TABLE_HOURLY_LOAD(
     +      MAX_HOURS_IN_MONTH,0:MAX_TRANS_LOAD_TABLES),stat=stv_er) 
         call check_alloc("gas_objt:0131","TABLE_HOURLY_LOAD",stv_er)
     
         ALLOCATE(MONTHLY_TRANS_ENERGY(0:MAX_GAS_LOAD_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0132","MONTHLY_TRANS_ENERGY",stv_er)
     
         ALLOCATE(MONTHLY_HYDRO_ENERGY(0:NUMBER_OF_HYDRO_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0133","MONTHLY_HYDRO_ENERGY",stv_er)
     
         ALLOCATE(MONTHLY_TRANS_PEAK(0:MAX_GAS_LOAD_GROUPS),stat=stv_er)
         call check_alloc("gas_objt:0134","MONTHLY_TRANS_PEAK",stv_er)
     
         ALLOCATE(MONTHLY_TRANS_BASE(0:MAX_GAS_LOAD_GROUPS),stat=stv_er)
         call check_alloc("gas_objt:0135","MONTHLY_TRANS_BASE",stv_er)
     
!
         ALLOCATE(MONTHLY_HYDRO_PEAK(0:NUMBER_OF_HYDRO_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0136","MONTHLY_HYDRO_PEAK",stv_er)
     
         ALLOCATE(MONTHLY_HYDRO_BASE(0:NUMBER_OF_HYDRO_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0137","MONTHLY_HYDRO_BASE",stv_er)
     
!
         ALLOCATE(MONTHLY_TABLE_ENERGY(0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0138","MONTHLY_TABLE_ENERGY",stv_er)
     
         ALLOCATE(MONTHLY_TABLE_SALES_ENERGY(
     +                         0:MAX_TRANS_LOAD_TABLES),stat=stv_er) 
         call check_alloc("gas_objt:0139",
     +                            "MONTHLY_TABLE_SALES_ENERGY",stv_er)
     
         ALLOCATE(MONTHLY_TABLE_PEAK(0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0140","MONTHLY_TABLE_PEAK",stv_er)
     
         ALLOCATE(TABLE_ENERGY_PRICE(MAX_TRANS_LOAD_TABLES),stat=stv_er)
         call check_alloc("gas_objt:0141","TABLE_ENERGY_PRICE",stv_er)
     
         ALLOCATE(TABLE_ENERGY_REVENUE(0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0142","TABLE_ENERGY_REVENUE",stv_er)
     
         ALLOCATE(TABLE_DEMAND_REVENUE(0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0143","TABLE_DEMAND_REVENUE",stv_er)
     
         ALLOCATE(TABLE_CUSTOMER_REVENUE(0:MAX_TRANS_LOAD_TABLES),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0144",
     +                                "TABLE_CUSTOMER_REVENUE",stv_er)
     
!
         ALLOCATE(TRANS_ENERGY_REVENUE(0:MAX_GAS_LOAD_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0145","TRANS_ENERGY_REVENUE",stv_er)
     
         ALLOCATE(TRANS_DEMAND_REVENUE(0:MAX_GAS_LOAD_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0146","TRANS_DEMAND_REVENUE",stv_er)
     
         ALLOCATE(TRANS_CUSTOMER_REVENUE(0:MAX_GAS_LOAD_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0147",
     +                                "TRANS_CUSTOMER_REVENUE",stv_er)
     
!
         ALLOCATE(CLASS_ENERGY_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0148","CLASS_ENERGY_REVENUE",stv_er)
     
         ALLOCATE(TRANS_INDEXED_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0149",
     +                                 "TRANS_INDEXED_REVENUE",stv_er)
     

         ALLOCATE(CLASS_PEAK_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0150","CLASS_PEAK_REVENUE",stv_er)
     
         ALLOCATE(CLASS_CUSTOMER_REVENUE(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0151",
     +                                "CLASS_CUSTOMER_REVENUE",stv_er)
     
         ALLOCATE(MONTHLY_CLASS_ENERGY(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0152","MONTHLY_CLASS_ENERGY",stv_er)
     
         ALLOCATE(MONTHLY_CLASS_PEAK(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0153","MONTHLY_CLASS_PEAK",stv_er)
     
         ALLOCATE(MONTHLY_CLASS_CUSTOMERS(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0154",
     +                               "MONTHLY_CLASS_CUSTOMERS",stv_er)
     
         ALLOCATE(LOCAL_CUSTOMER_NAME(0:MAX_CUST_CLASS_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0155","LOCAL_CUSTOMER_NAME",stv_er)
     
         ALLOCATE(ASSET_CLASS_HOURLY_LOAD(MAX_HOURS_IN_MONTH,
     +                                       0:MAX_ASSET_CLASS_GROUPS,
     +                           0:MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0156",
     +                               "ASSET_CLASS_HOURLY_LOAD",stv_er)
...
!
     ...
     +                             MAX_GAS_LOAD_GROUPS),stat=stv_er) 
         call check_alloc("gas_objt:0157","WH_TRANS_ALLOC",stv_er)
         ...
         IF(WH_FILE_EXISTS) THEN
            DO TG = 1, MAX_GAS_LOAD_GROUPS 
               TEMP_R = 0.
               DO TRANS = 1, MAX_TRANS_LOAD_TABLES
                  IF(TABLE_ACTIVE(TRANS) == 'F') THEN 
                    CYCLE
                  ENDIF
!
                  IF( .NOT. IN_ACTIVE_MARKET_AREA(
     +                            BASECASE_MARKET_AREA_ID(TRANS))) THEN 
                    CYCLE
                  ENDIF
!
...
!
                  IF(I == 0 .OR. 
     +                           GAS_LOAD_GROUPS_INDEX(I) == 0) THEN 
                    CYCLE
                  ENDIF
!
                  IF(.NOT. YES_ASSET_ANALYST_ONLY) THEN            
...
!
     ...
     +                                WH_LOADS_PER_HOUR(HR_IN_MONTH,TG))
                     IF(HR_IN_MONTH >= DAYS_IN_MONTH*24) THEN 
                       EXIT
                     ENDIF
                  ENDDO
!     
...
! 5/20/02. GAT. NEW CONDITION TO AVOID WARNING IN GET_GAS_GROUP_POSITION           
!
            IF(TG == 0 .OR. GAS_LOAD_GROUPS_INDEX(TG) == 0) THEN 
              CYCLE
            ENDIF
!
            IF(.NOT. YES_ASSET_ANALYST_ONLY) THEN            
...
!
     ...
     +                                            MAX_CUST_CLASS_GROUPS)
               ...
               DO J = 1, MAX_TRANS_LOAD_TABLES
                  I = GAS_GROUP_ID(J)
                  IF(I == 0 .OR. GAS_LOAD_GROUPS_INDEX(I) == 0) THEN 
                    CYCLE
                  ENDIF
                  WRITE(TEMP_STR,'(I6)') K
...
!***********************************************************************
     ...
     +                               DEMAND_GROUP_AVAILABLE(R_GAS_GROUP)
      ...
      RETURN
!***********************************************************************
      ENTRY WRITE_MTHLY_GAS_DEM_CLASS_SUMM(R_YEAR,R_ISEAS)
!***********************************************************************
...
!     +                                    BASECASE_MARKET_AREA_ID(TRANS)
!                  ELSE
                     IF(SPECIAL_CUSTOMER_TYPE(TRANS) /= 'P') THEN 
                       CYCLE
                     ENDIF

                     GAS_ID = GAS_GROUP_ID(TRANS)
                     IF(.NOT. GAS_GROUP_ACTIVE_SWITCH(GAS_ID)) THEN 
                       CYCLE
                     ENDIF
...
!
     ...
     +            INDEXED_ENRG_AVERAGE_REVENUE
            ...
            ENDIF ! R_ISEAS == 12
         ENDIF
         WRITE_MTHLY_GAS_DEM_CLASS_SUMM = .TRUE.
      RETURN
!***********************************************************************
...
!***********************************************************************
!
!        PROGRAM TO READ MULTI-TAB INFORMATION ON GAS SUPPLY NODES
!                 AND CONVERT TO BINARY FORMAT
!                       COPYRIGHT (C) 2006
...
!***********************************************************************
!
! CONVERT THE GAS_SUPPLY GROUPS FILE
!
!
!***********************************************************************
...
!     +                     PLANNING_AREA_2_GS,
     ...
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0158",
     +                                "SUPPLY_GROUP_AVAILABLE",stv_er)
     
         ALLOCATE(GS_GROUP_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0159","GS_GROUP_INDEX",stv_er)
     
         ALLOCATE(GS_SCENARIO_VARIABLE_INDEX(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0160",
     +                            "GS_SCENARIO_VARIABLE_INDEX",stv_er)
     
         ALLOCATE(HYDRO_GROUP_2_GS(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0161","HYDRO_GROUP_2_GS",stv_er)
     
!         ALLOCATE(PLANNING_AREA_2_GS(MAX_GAS_GROUP_INDEX))
         ALLOCATE(GAS_GROUP_POSITION(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0162","GAS_GROUP_POSITION",stv_er)
     
         ALLOCATE(REPORT_THIS_GROUP(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0163","REPORT_THIS_GROUP",stv_er)
     
         ALLOCATE(GS_2_HYDRO_GROUP(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0164","GS_2_HYDRO_GROUP",stv_er)
     
         ALLOCATE(GS_2_PLANNING_AREA(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0165","GS_2_PLANNING_AREA",stv_er)
     
         ALLOCATE(HYDRO_AGGREGATION_POSITION(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0166",
     +                            "HYDRO_AGGREGATION_POSITION",stv_er)
     
         ALLOCATE(HYDRO_AGGREGATION_INDEX(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0167",
     +                               "HYDRO_AGGREGATION_INDEX",stv_er)
     
         ALLOCATE(PLANNING_AREA_POSITION(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0168",
     +                                "PLANNING_AREA_POSITION",stv_er)
     
         ALLOCATE(PLANNING_AREA_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0169","PLANNING_AREA_INDEX",stv_er)
     
         ALLOCATE(ASSET_CLASS_GROUPS_INDEX(0:MAX_ASSET_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0170",
     +                              "ASSET_CLASS_GROUPS_INDEX",stv_er)
     
         ALLOCATE(ASSET_CLASS_2_GS(0:MAX_ASSET_GROUPS),stat=stv_er)
         call check_alloc("gas_objt:0171","ASSET_CLASS_2_GS",stv_er)
     
         ALLOCATE(ASSET_CLASS_GROUP_2_AC(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0172",
     +                                "ASSET_CLASS_GROUP_2_AC",stv_er)
...
!
     ...
     +                                 ANNUAL_SUPPLY_QUANTITY,
     +                                 ANNUAL_SUPPLY_PRICE)
         ALLOCATE(GROUP_NAME(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0173","GROUP_NAME",stv_er)
     
         ALLOCATE(GROUP_ACTIVE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0174","GROUP_ACTIVE",stv_er)
     
         ALLOCATE(GAS_GROUP_ID(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0175","GAS_GROUP_ID",stv_er)
     
         ALLOCATE(GAS_GROUP_ID_POSITION(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0176",
     +                                 "GAS_GROUP_ID_POSITION",stv_er)
     
         ALLOCATE(BASECASE_MARKET_AREA_ID(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0177",
     +                               "BASECASE_MARKET_AREA_ID",stv_er)
     
         ALLOCATE(BASE_CASE_GAS_AREA_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0178",
     +                                 "BASE_CASE_GAS_AREA_ID",stv_er)
     
         ALLOCATE(BASECASE_SUBREGION_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0179",
     +                                 "BASECASE_SUBREGION_ID",stv_er)
     
         ALLOCATE(SPINNING_UNITS(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0180","SPINNING_UNITS",stv_er)
     
         ALLOCATE(OFF_PEAK_SPINNING_UNITS(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0181",
     +                               "OFF_PEAK_SPINNING_UNITS",stv_er)
     
         ALLOCATE(SPINNING_RESERVE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0182","SPINNING_RESERVE",stv_er)
     
         ALLOCATE(OFF_PEAK_SPINNING_RESERVE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0183",
     +                             "OFF_PEAK_SPINNING_RESERVE",stv_er)
     
         ALLOCATE(MAX_HOURLY_RAMP_UP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0184","MAX_HOURLY_RAMP_UP",stv_er)
     
         ALLOCATE(MAX_HOURLY_RAMP_DOWN(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0185","MAX_HOURLY_RAMP_DOWN",stv_er)
     
         ALLOCATE(FIRST_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0186","FIRST_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(FIRST_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0187",
     +                                "FIRST_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(SECOND_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0188",
     +                                 "SECOND_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(SECOND_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0189",
     +                               "SECOND_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(THIRD_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0190","THIRD_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(THIRD_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0191",
     +                                "THIRD_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(ADDITIONAL_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS,7),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0192",
     +                             "ADDITIONAL_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(ADDITIONAL_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS,7),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0193",
     +                           "ADDITIONAL_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(RTO_GROUP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0194","RTO_GROUP",stv_er)
     
         ALLOCATE(NOX_YEAR(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0195","NOX_YEAR",stv_er)
     
         ALLOCATE(END_NOX_YEAR(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0196","END_NOX_YEAR",stv_er)
     
         ALLOCATE(BASIN_START_YEAR(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0197","BASIN_START_YEAR",stv_er)
     
         ALLOCATE(ST_LHS_FOR_PRICES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0198","ST_LHS_FOR_PRICES",stv_er)
     
         ALLOCATE(MRX_VOLATILITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0199","MRX_VOLATILITY_MULT",stv_er)
     
         ALLOCATE(NIGHT_SCARCITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0200","NIGHT_SCARCITY_MULT",stv_er)
     
         ALLOCATE(WEEKEND_SCARCITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0201",
     +                                 "WEEKEND_SCARCITY_MULT",stv_er)
     
         ALLOCATE(PRICE_CAP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0202","PRICE_CAP",stv_er)
     
         ALLOCATE(MAX_HOURLY_GS_IMPORT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0203","MAX_HOURLY_GS_IMPORT",stv_er)
     
         ALLOCATE(MAX_HOURLY_GS_EXPORT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0204","MAX_HOURLY_GS_EXPORT",stv_er)
     
         ALLOCATE(HYDRO_LOAD_AGGREGATION(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0205",
     +                                "HYDRO_LOAD_AGGREGATION",stv_er)
     
         ALLOCATE(GS_REGIONAL_PLANNING_AREA(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0206",
     +                             "GS_REGIONAL_PLANNING_AREA",stv_er)
     
         ALLOCATE(REPORT_CL_CAPACITY(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0207","REPORT_CL_CAPACITY",stv_er)
     
         ALLOCATE(ASSET_CLASS_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0208","ASSET_CLASS_ID",stv_er)
     
         ALLOCATE(ASSET_CLASS_REV_ALLOC_VECTOR(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0209",
     +                          "ASSET_CLASS_REV_ALLOC_VECTOR",stv_er)
     
         ALLOCATE(TIME_ZONE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0210","TIME_ZONE",stv_er)
     
         ALLOCATE(CAPACITY_ADDER(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0211","CAPACITY_ADDER",stv_er)
     
         ALLOCATE(NOX_SEASON(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0212","NOX_SEASON",stv_er)
     
         ALLOCATE(PURCHASE_POWER_ASSIGN(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0213",
     +                                 "PURCHASE_POWER_ASSIGN",stv_er)
     
         ALLOCATE(CREATE_HOURLY_PRICE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0214","CREATE_HOURLY_PRICE",stv_er)
     
         ALLOCATE(SUPPLY_TYPE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0215","SUPPLY_TYPE",stv_er)
     
         ALLOCATE(HOURLY_PRICE_NAME(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0216","HOURLY_PRICE_NAME",stv_er)
     
         ALLOCATE(PURCHASE_ASSET_CLASS_ID(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0217",
     +                               "PURCHASE_ASSET_CLASS_ID",stv_er)
     
         ALLOCATE(PURCHASE_ASSET_ALLOC_VECTOR(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0218",
     +                           "PURCHASE_ASSET_ALLOC_VECTOR",stv_er)
     
         ALLOCATE(OPERATING_COSTS(30,GAS_GROUPS_RECORDS),stat=stv_er) ! (25,GAS_GROUPS_RECORDS))
         call check_alloc("gas_objt:0219","OPERATING_COSTS",stv_er)
     
         ALLOCATE(CAPACITY_VALUES(30,GAS_GROUPS_RECORDS),stat=stv_er) ! (25,GAS_GROUPS_RECORDS))
         call check_alloc("gas_objt:0220","CAPACITY_VALUES",stv_er)
     
         ALLOCATE(INT_RES_REMAIN(30,GAS_GROUPS_RECORDS),stat=stv_er) ! (25,GAS_GROUPS_RECORDS))
         call check_alloc("gas_objt:0221","INT_RES_REMAIN",stv_er)
     
         ALLOCATE(POTENTIAL_RESERVES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0222","POTENTIAL_RESERVES",stv_er)
     
         ALLOCATE(PROVEN_RESERVES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0223","PROVEN_RESERVES",stv_er)
     
         ALLOCATE(MAX_DAILY_EXTRACTION_RATE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0224",
     +                             "MAX_DAILY_EXTRACTION_RATE",stv_er)
     
         ALLOCATE(MAX_DAILY_EXTRACTION_IN_MONTH(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0225",
     +                         "MAX_DAILY_EXTRACTION_IN_MONTH",stv_er)
     
         ALLOCATE(MAX_MONTHLY_EXTRACTION_RATE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0226",
     +                           "MAX_MONTHLY_EXTRACTION_RATE",stv_er)
     
         ALLOCATE(SHRINKAGE_PERCENT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0227","SHRINKAGE_PERCENT",stv_er)
     
         ALLOCATE(SHRINKAGE_COST(GAS_GROUPS_RECORDS),stat=stv_er) 
         call check_alloc("gas_objt:0228","SHRINKAGE_COST",stv_er)
     
         ALLOCATE(EXTRACTION_COST_ESC_RATE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0229",
     +                              "EXTRACTION_COST_ESC_RATE",stv_er)
     
         ALLOCATE(RESERVE_APPRECIATION_PERCENT(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0230",
     +                          "RESERVE_APPRECIATION_PERCENT",stv_er)
     
         ALLOCATE(CUMULATIVE_RESERVE_IMPACT(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0231",
     +                             "CUMULATIVE_RESERVE_IMPACT",stv_er)
     
         ALLOCATE(MAX_DAILY_HARD_LIMIT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0232","MAX_DAILY_HARD_LIMIT",stv_er)
     
         ALLOCATE(MIN_DAILY_LIMIT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0233","MIN_DAILY_LIMIT",stv_er)
     
         ALLOCATE(DYNAMIC_EXTRACT_RATE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0234","DYNAMIC_EXTRACT_RATE",stv_er)
     
         ALLOCATE(MIN_BASIN_COST(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0235","MIN_BASIN_COST",stv_er)
     
         ALLOCATE(X_SUPPLY_CURVE_COEFFICIENT(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0236",
     +                            "X_SUPPLY_CURVE_COEFFICIENT",stv_er)
     
         ALLOCATE(X2_SUPPLY_CURVE_COEFFICIENT(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0237",
     +                           "X2_SUPPLY_CURVE_COEFFICIENT",stv_er)
     
         ALLOCATE(X2_SUPPLY_CURVE_INTERCEPT(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0238",
     +                             "X2_SUPPLY_CURVE_INTERCEPT",stv_er)
     
         ALLOCATE(SUPPLY_CURVE_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0239","SUPPLY_CURVE_MULT",stv_er)
     
         ALLOCATE(SUPPLY_BASIS_DIFFERENTIAL(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0240",
     +                             "SUPPLY_BASIS_DIFFERENTIAL",stv_er)
     
         ALLOCATE(DYN_SUPPLY_BASIS_DIFF(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0241",
     +                                 "DYN_SUPPLY_BASIS_DIFF",stv_er)
     

         ALLOCATE(ANNUAL_SUPPLY_QUANTITY(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0242",
     +                                "ANNUAL_SUPPLY_QUANTITY",stv_er)
     
         ALLOCATE(ANNUAL_SUPPLY_PRICE(0:GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0243","ANNUAL_SUPPLY_PRICE",stv_er)
...
!
     ...
     +                      .NOT. GAS_GROUP_ACTIVE_SWITCH(GAS_ID)) THEN 
              CYCLE
            ENDIF
!
...
!            
            DO I = 25, 2,-1
               IF(ABS(INT_RES_REMAIN(I,GAS_GROUP)) < 0.0001) THEN 
                 CYCLE
               ENDIF
...
!
     ...
     +                                HYDRO_AGGREGATION_INDEX(HG)) THEN 
                       CYCLE
                     ENDIF
...
!         
     ...
     +                        POINTS_PER_GS,
     +                        LAST_K)
            allocate(FIRST_GS(GS),stat=stv_er)
            call check_alloc("gas_objt:0244","FIRST_GS",stv_er)
            allocate(GROUPS_PER_GS(GS),stat=stv_er)
            call check_alloc("gas_objt:0245","GROUPS_PER_GS",stv_er)
            allocate(POINTS_PER_GS(GS),stat=stv_er)
            call check_alloc("gas_objt:0246","POINTS_PER_GS",stv_er)
            allocate(LAST_K(GS),stat=stv_er)
            call check_alloc("gas_objt:0247","LAST_K",stv_er)
...
!
                     DO M = 1, 30 ! 25
                        IF(INT_RES_REMAIN(M,J) < 0.000001) THEN 
                          EXIT
                        ENDIF
...
!         
     ...
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0248",
     +                         "TEMP_GS_SUPPLY_CAPACITY_CURVE",stv_er)
         allocate(TEMP_SYSTEM_SUPPLY_CAPACITY_CURVE(MAX_POINTS_IN_GS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0249",
     +                     "TEMP_SYSTEM_SUPPLY_CAPACITY_CURVE",stv_er)
         allocate(TEMP_GS_SUPPLY_COST_CURVE(MAX_POINTS_IN_GS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0250",
     +                             "TEMP_GS_SUPPLY_COST_CURVE",stv_er)
         allocate(TEMP_GS_SUPPLY_POSITION(MAX_POINTS_IN_GS),stat=stv_er)
         call check_alloc("gas_objt:0251",
     +                               "TEMP_GS_SUPPLY_POSITION",stv_er)
         allocate(INTERVAL_INDEX(MAX_POINTS_IN_GS),stat=stv_er)
         call check_alloc("gas_objt:0252","INTERVAL_INDEX",stv_er)
...
!     +                   R_EXTRACT_CURVE)
     ...
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0253",
     +                              "GS_SUPPLY_CAPACITY_CURVE",stv_er)
         allocate(GS_SUPPLY_EXTRACT_CURVE(MAX_POINTS_IN_GS,GS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0254",
     +                               "GS_SUPPLY_EXTRACT_CURVE",stv_er)
         allocate(GS_SUPPLY_POSITION(MAX_POINTS_IN_GS,GS),stat=stv_er)
         call check_alloc("gas_objt:0255","GS_SUPPLY_POSITION",stv_er)
         allocate(GS_SUPPLY_COST_CURVE(MAX_POINTS_IN_GS,GS),stat=stv_er)
         call check_alloc("gas_objt:0256","GS_SUPPLY_COST_CURVE",stv_er)
...
!
                  IF(ABS(INT_RES_REMAIN(K,M))< 0.00001) THEN 
                    EXIT
                  ENDIF
!
...
!***********************************************************************
!
!                  ROUTINE TO CONVERT GAS_STORAGE FILE
!
!                          COPYRIGHT (C) 2007
...
! DECLARATION FOR DBREAD COMMON BLOCK
!      CHARACTER*1024 RECLN
! DECLARATION FOR TRANSACT GAS_STORAGE DETERMINANTS
...
!
     ...
     +            Storage_Start_Year,
     +            STORAGE_ID_INDEX)
         allocate(Storage_Name(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0257","Storage_Name",stv_er)
         allocate(Gas_Storage_Active(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0258","Gas_Storage_Active",stv_er)
         allocate(Storage_ID_Number(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0259","Storage_ID_Number",stv_er)
         allocate(Num_Storage_Demand_Markets(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0260",
     +                            "Num_Storage_Demand_Markets",stv_er)
         allocate(Num_Storage_Alloc_Markets(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0261",
     +                             "Num_Storage_Alloc_Markets",stv_er)
         allocate(Storage_Demand_Index(MAX_OUT_GAS_STORAGE,30),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0262","Storage_Demand_Index",stv_er)
         allocate(Storage_Alloc_Index(MAX_OUT_GAS_STORAGE,30),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0263","Storage_Alloc_Index",stv_er)
         allocate(DAILY_STORAGE_BY_GS(0:MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0264","DAILY_STORAGE_BY_GS",stv_er)
         allocate(Storage_Initial_Capacity(0:MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0265",
     +                              "Storage_Initial_Capacity",stv_er)
         allocate(Storage_Current_Capacity(0:MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0266",
     +                              "Storage_Current_Capacity",stv_er)
         allocate(DAILY_INJECT_WITHDRAWAL(0:MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0267",
     +                               "DAILY_INJECT_WITHDRAWAL",stv_er)
         allocate(Storage_Demand_Markets(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0268",
     +                                "Storage_Demand_Markets",stv_er)
         allocate(Storage_Demand_Allocation(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0269",
     +                             "Storage_Demand_Allocation",stv_er)
         allocate(Storage_Maximum_Capacity(0:MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0270",
     +                              "Storage_Maximum_Capacity",stv_er)
         allocate(Storage_Demand_Slope(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0271","Storage_Demand_Slope",stv_er)
         allocate(Storage_Demand_Intercept(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0272",
     +                              "Storage_Demand_Intercept",stv_er)
         allocate(Maximum_Daily_Injection(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0273",
     +                               "Maximum_Daily_Injection",stv_er)
         allocate(Maximum_Daily_Withdrawal(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0274",
     +                              "Maximum_Daily_Withdrawal",stv_er)
         allocate(Storage_Weekly_Min_Capacity(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0275",
     +                           "Storage_Weekly_Min_Capacity",stv_er)
         allocate(Storage_Weekly_Max_Capacity(MAX_OUT_GAS_STORAGE),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0276",
     +                           "Storage_Weekly_Max_Capacity",stv_er)
         allocate(Storage_Scalar(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0277","Storage_Scalar",stv_er)
         allocate(Storage_Start_Year(MAX_OUT_GAS_STORAGE),stat=stv_er)
         call check_alloc("gas_objt:0278","Storage_Start_Year",stv_er)
         allocate(STORAGE_ID_INDEX(100),stat=stv_er)
         call check_alloc("gas_objt:0279","STORAGE_ID_INDEX",stv_er)
...
!
     ...
     +                                                       ACTIVE_LINK
            IF(INDEX(Gas_Storage_Active(ACTIVE_LINK),'N') /= 0) THEN 
              CYCLE
            ENDIF
!
...
!         
         DO I = 1, NUM_STORAGE_NODES
            IF(LOCAL_YEAR < Storage_Start_Year(I)) THEN 
              CYCLE
            ENDIF
...
!***********************************************************************
!
!                  ROUTINE TO CONVERT GAS_LINKS FILE
!
!                          COPYRIGHT (C) 2006
!                        GLOBAL ENERGY DECISIONS
!                          ALL RIGHTS RESERVED
...
! DECLARATION FOR DBREAD COMMON BLOCK
!      CHARACTER*1024 RECLN
! DECLARATION FOR TRANSACT GAS_LINK DETERMINANTS
...
! 070906.
      INTEGER (KIND=2) :: DELETE,YR,ZONE,POINTR,BASE_YEAR,
     +                    PRC_MULTI_ZONE_TIE_LIMIT_FILES,I,
     +                    TieID
      INTEGER (KIND=4) :: IOS
      CHARACTER (LEN=4096) RECLN
...
!
     ...
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0280","TRANS_GROUP_POSITION",stv_er)
         allocate(TieID(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0281","TieID",stv_er)
         allocate(
     +         TieID_INDEX(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0282","TieID_INDEX",stv_er)
         allocate(ExpansionCost(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0283","ExpansionCost",stv_er)
         allocate(ExpansionCostEscalation(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0284",
     +                               "ExpansionCostEscalation",stv_er)
         allocate(MAX_C_FOR_(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0285","MAX_C_FOR_",stv_er)
         allocate(MIN_C_FOR_(0:MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0286","MIN_C_FOR_",stv_er)
         allocate(LINE_LOSSES_A_TO_B(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0287","LINE_LOSSES_A_TO_B",stv_er)
         allocate(LINE_LOSSES_B_TO_A(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0288","LINE_LOSSES_B_TO_A",stv_er)
         allocate(CONGESTION_PERCENT(MAX_OUT_GAS_LINKS,10),stat=stv_er)
         call check_alloc("gas_objt:0289","CONGESTION_PERCENT",stv_er)
         allocate(CONGESTION_COST(MAX_OUT_GAS_LINKS,10),stat=stv_er)
         call check_alloc("gas_objt:0290","CONGESTION_COST",stv_er)
         allocate(MaximumExpansion(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0291","MaximumExpansion",stv_er)
         allocate(PipeExpansionCapacity(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0292",
     +                                 "PipeExpansionCapacity",stv_er)
         allocate(TempPipeExpanCapacity(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0293",
     +                                 "TempPipeExpanCapacity",stv_er)
         allocate(PipelineDistance(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0294","PipelineDistance",stv_er)
         allocate(ExpansionCCRF(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0295","ExpansionCCRF",stv_er)
         allocate(CONGESTION_COST_ESC(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0296","CONGESTION_COST_ESC",stv_er)
         allocate(CONGESTION_COST_NOMINAL(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0297",
     +                               "CONGESTION_COST_NOMINAL",stv_er)
         allocate(MONTHLY_CONGEST_PERCENT(MAX_OUT_GAS_LINKS,10),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0298",
     +                               "MONTHLY_CONGEST_PERCENT",stv_er)
         allocate(MONTHLY_CONGEST_COST(MAX_OUT_GAS_LINKS,10),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0299","MONTHLY_CONGEST_COST",stv_er)
         allocate(ZONE_A_TO_B_PEAK_TIES(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0300",
     +                                 "ZONE_A_TO_B_PEAK_TIES",stv_er)
         allocate(ZONE__A_TO_B_OFF_PEAK_TIES(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0301",
     +                            "ZONE__A_TO_B_OFF_PEAK_TIES",stv_er)
         allocate(ZONE__A_TO_B_WHEELING_RATES(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0302",
     +                           "ZONE__A_TO_B_WHEELING_RATES",stv_er)
         allocate(ZONE_B_TO_A_PEAK_TIES(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0303",
     +                                 "ZONE_B_TO_A_PEAK_TIES",stv_er)
         allocate(ZONE__B_TO_A_OFF_PEAK_TIES(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0304",
     +                            "ZONE__B_TO_A_OFF_PEAK_TIES",stv_er)
         allocate(ZONE__B_TO_A_WHEELING_RATES(MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0305",
     +                           "ZONE__B_TO_A_WHEELING_RATES",stv_er)
         allocate(ESCALATION_RATE(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0306","ESCALATION_RATE",stv_er)
         allocate(ZONE_A_GROUP_ID(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0307","ZONE_A_GROUP_ID",stv_er)
         allocate(ZONE_B_GROUP_ID(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0308","ZONE_B_GROUP_ID",stv_er)
         allocate(ZONE_A(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0309","ZONE_A",stv_er)
         allocate(ZONE_B(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0310","ZONE_B",stv_er)
         allocate(TieDescription(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0311","TieDescription",stv_er)
         allocate(iPort(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0312","iPort",stv_er)
         allocate(SNamePos(MAX_OUT_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0313","SNamePos",stv_er)
         allocate(LINK_PAIR(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0314","LINK_PAIR",stv_er)
         allocate(LINK_INDEX(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0315","LINK_INDEX",stv_er)
         allocate(
     +        I_TO_K_PATHS(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0316","I_TO_K_PATHS",stv_er)
         allocate(
     +        J_TO_K_PATHS(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0317","J_TO_K_PATHS",stv_er)
         allocate(PATH_FOR_J_TO_K(MAX_J_TO_K_PATHS,MAX_OUT_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0318","PATH_FOR_J_TO_K",stv_er)
         allocate(VLI(MAX_GAS_GROUP_NUMBER,MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0319","VLI",stv_er)
         allocate(ACTIVE_DEMAND_NODE(MAX_GAS_GROUP_NUMBER),stat=stv_er)
         call check_alloc("gas_objt:0320","ACTIVE_DEMAND_NODE",stv_er)
         allocate(ACTIVE_SUPPLY_NODE(MAX_GAS_GROUP_NUMBER),stat=stv_er)
         call check_alloc("gas_objt:0321","ACTIVE_SUPPLY_NODE",stv_er)
...
!
     ...
     +            .NOT. GAS_GROUP_ACTIVE_SWITCH(B_ID)) THEN 
              CYCLE
            ENDIF
!
            DO I = 1, 10
               IF(ABS(CONGESTION_COST(ACTIVE_LINK,I)) < .0001) THEN 
                 EXIT
               ENDIF
...
!
               IF(ABS(MaximumExpansion(J)) < .001) THEN 
                 CYCLE
               ENDIF
...
!         
     ...
     +            NUMBER_OF_PATHS_FOR_I) ! VERY BIG
         allocate(J_PATH(0:100000),stat=stv_er)
         call check_alloc("gas_objt:0322","J_PATH",stv_er)
         allocate(K_PATH(0:100000),stat=stv_er)
         call check_alloc("gas_objt:0323","K_PATH",stv_er)
         allocate(SET_OF_J(100000),stat=stv_er)
         call check_alloc("gas_objt:0324","SET_OF_J",stv_er)
         allocate(SET_OF_K(100000),stat=stv_er)
         call check_alloc("gas_objt:0325","SET_OF_K",stv_er)
         allocate(PATH(0:512000),stat=stv_er)
         call check_alloc("gas_objt:0326","PATH",stv_er)
         allocate(PARENT(0:512000),stat=stv_er)
         call check_alloc("gas_objt:0327","PARENT",stv_er)
         allocate(NODE(0:512000),stat=stv_er)
         call check_alloc("gas_objt:0328","NODE",stv_er)
         allocate(I_PATH(0:512000),stat=stv_er)
         call check_alloc("gas_objt:0329","I_PATH",stv_er)
         allocate(I_PARENT(0:512000),stat=stv_er)
         call check_alloc("gas_objt:0330","I_PARENT",stv_er)
         allocate(NUMBER_OF_PATHS_FOR_I(MAX_GAS_GROUP_NUMBER),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0331",
     +                                 "NUMBER_OF_PATHS_FOR_I",stv_er)
...
!         DO I = 1, MAX_GAS_GROUP_NUMBER ! ACTIVE_NODES ! INITIAL (SUPPLY) PATH
         ...
         DO I = 1,  MAX_GAS_GROUP_NUMBER ! ACTIVE_NODES ! INITIAL (SUPPLY) PATH
            IF(.NOT. ACTIVE_SUPPLY_NODE(I)) THEN 
              CYCLE
            ENDIF
!
...
!                     
                     ...
                     IF(J == K) THEN 
                       CYCLE
                     ENDIF
                     IF(I_TO_K_PATHS(I,K) > MAX_I_TO_K_PATHS) THEN 
                       CYCLE
                     ENDIF
                     IF(J_TO_K_PATHS(I,K) >= MAX_J_TO_K_PATHS) THEN 
                       CYCLE
                     ENDIF
!
!                     K = N ! ?
!
                     IF(.NOT. LINK_PAIR(J,K)) THEN 
                       CYCLE
                     ENDIF
!
...
! 021307. LIMIT SIZE TO REASONABLE LEVEL (?)                           
                           IF(Q == I .OR. Z > MAX_GAS_GROUP_NUMBER) THEN 
                             EXIT ! 32000) EXIT
                           ENDIF
!                     
                        ENDDO
...
!***********************************************************************
         ...
         DO J = 1, 10 
            IF(MONTHLY_CONGEST_PERCENT(I,J) > R_UTIL .AND. J <10 ) THEN 
              CYCLE
            ENDIF
            IF(J == 1) THEN
...
!***********************************************************************
!
!        PROGRAM TO READ MULTI-TAB INFORMATION ON GAS NODE NODES
!                 AND CONVERT TO BINARY FORMAT
!                       COPYRIGHT (C) 2006
!      ALL RIGHTS RESERVED GLOBAL ENERGY DECISIONS
!
...
!***********************************************************************
!
! CONVERT THE GAS_NODE GROUPS FILE
!
!
!***********************************************************************
...
!     +                     PLANNING_AREA_2_GN,
     ...
     +                     ASSET_CLASS_GROUP_2_AC,
     +                     REPORT_THIS_GROUP)
         ALLOCATE(GAS_GROUP_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0332","GAS_GROUP_INDEX",stv_er)
     
         ALLOCATE(GN_SCENARIO_VARIABLE_INDEX(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0333",
     +                            "GN_SCENARIO_VARIABLE_INDEX",stv_er)
     
         ALLOCATE(HYDRO_GROUP_2_GN(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0334","HYDRO_GROUP_2_GN",stv_er)
     
!         ALLOCATE(PLANNING_AREA_2_GN(MAX_GAS_GROUP_INDEX))
         ALLOCATE(GAS_GROUP_POSITION(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0335","GAS_GROUP_POSITION",stv_er)
     
         ALLOCATE(GAS_GROUP_STATE_POSITION(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0336",
     +                              "GAS_GROUP_STATE_POSITION",stv_er)
     
         ALLOCATE(NODE_FROM_STATE_POSITION(MAX_GAS_REGION_NO),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0337",
     +                              "NODE_FROM_STATE_POSITION",stv_er)
     
         ALLOCATE(REPORT_THIS_GROUP(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0338","REPORT_THIS_GROUP",stv_er)
     
         ALLOCATE(GN_2_HYDRO_GROUP(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0339","GN_2_HYDRO_GROUP",stv_er)
     
         ALLOCATE(GN_2_PLANNING_AREA(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0340","GN_2_PLANNING_AREA",stv_er)
     
         ALLOCATE(HYDRO_AGGREGATION_POSITION(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0341",
     +                            "HYDRO_AGGREGATION_POSITION",stv_er)
     
         ALLOCATE(HYDRO_AGGREGATION_INDEX(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0342",
     +                               "HYDRO_AGGREGATION_INDEX",stv_er)
     
         ALLOCATE(PLANNING_AREA_POSITION(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0343",
     +                                "PLANNING_AREA_POSITION",stv_er)
     
         ALLOCATE(PLANNING_AREA_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0344","PLANNING_AREA_INDEX",stv_er)
     
         ALLOCATE(ASSET_CLASS_GROUPS_INDEX(0:MAX_ASSET_GROUPS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0345",
     +                              "ASSET_CLASS_GROUPS_INDEX",stv_er)
     
         ALLOCATE(ASSET_CLASS_2_GN(0:MAX_ASSET_GROUPS),stat=stv_er)
         call check_alloc("gas_objt:0346","ASSET_CLASS_2_GN",stv_er)
     
         ALLOCATE(ASSET_CLASS_GROUP_2_AC(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0347",
     +                                "ASSET_CLASS_GROUP_2_AC",stv_er)
...
!     +                                 REPORT_THIS_GROUP,
     ...
     +                                 SHRINKAGE_COST )
         ALLOCATE(GROUP_NAME(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0348","GROUP_NAME",stv_er)
     
         allocate(iPort(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0349","iPort",stv_er)
         allocate(SNamePos(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0350","SNamePos",stv_er)
      
         ALLOCATE(WORLD_DATABASE_INDEX(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0351","WORLD_DATABASE_INDEX",stv_er)
     
         ALLOCATE(GROUP_ACTIVE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0352","GROUP_ACTIVE",stv_er)
     
         ALLOCATE(GAS_NODE_GROUP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0353","GAS_NODE_GROUP",stv_er)
     
         ALLOCATE(GAS_ID_2_GSP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0354","GAS_ID_2_GSP",stv_er)
     
         ALLOCATE(COUNTRY_NODE_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0355","COUNTRY_NODE_INDEX",stv_er)
     
         ALLOCATE(BASECASE_MARKET_AREA_ID(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0356",
     +                               "BASECASE_MARKET_AREA_ID",stv_er)
     
         ALLOCATE(BASE_CASE_GAS_AREA_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0357",
     +                                 "BASE_CASE_GAS_AREA_ID",stv_er)
     
         ALLOCATE(BASECASE_SUBREGION_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0358",
     +                                 "BASECASE_SUBREGION_ID",stv_er)
     
         ALLOCATE(SPINNING_UNITS(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0359","SPINNING_UNITS",stv_er)
     
         ALLOCATE(OFF_PEAK_SPINNING_UNITS(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0360",
     +                               "OFF_PEAK_SPINNING_UNITS",stv_er)
     
         ALLOCATE(SPINNING_RESERVE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0361","SPINNING_RESERVE",stv_er)
     
         ALLOCATE(OFF_PEAK_SPINNING_RESERVE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0362",
     +                             "OFF_PEAK_SPINNING_RESERVE",stv_er)
     
         ALLOCATE(MAX_HOURLY_RAMP_UP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0363","MAX_HOURLY_RAMP_UP",stv_er)
     
         ALLOCATE(MAX_HOURLY_RAMP_DOWN(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0364","MAX_HOURLY_RAMP_DOWN",stv_er)
     
         ALLOCATE(FIRST_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0365","FIRST_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(FIRST_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0366",
     +                                "FIRST_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(SECOND_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0367",
     +                                 "SECOND_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(SECOND_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0368",
     +                               "SECOND_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(THIRD_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0369","THIRD_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(THIRD_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0370",
     +                                "THIRD_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(ADDITIONAL_CAPACITY_VALUE(0:GAS_GROUPS_RECORDS,7),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0371",
     +                             "ADDITIONAL_CAPACITY_VALUE",stv_er)
     
         ALLOCATE(ADDITIONAL_CAPACITY_PERCENT(0:GAS_GROUPS_RECORDS,7),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0372",
     +                           "ADDITIONAL_CAPACITY_PERCENT",stv_er)
     
         ALLOCATE(RTO_GROUP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0373","RTO_GROUP",stv_er)
     
         ALLOCATE(NOX_YEAR(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0374","NOX_YEAR",stv_er)
     
         ALLOCATE(END_NOX_YEAR(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0375","END_NOX_YEAR",stv_er)
     
         ALLOCATE(ST_LHS_FOR_PRICES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0376","ST_LHS_FOR_PRICES",stv_er)
     
         ALLOCATE(GAS_NODE_LATITUDE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0377","GAS_NODE_LATITUDE",stv_er)
     
         ALLOCATE(GAS_NODE_LONGITUDE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0378","GAS_NODE_LONGITUDE",stv_er)
     
!         
         ALLOCATE(GAS_NODE_STATE_PROVINCE(MAX_GAS_GROUP_INDEX),
     +                                                   stat=stv_er) 
         call check_alloc("gas_objt:0379",
     +                               "GAS_NODE_STATE_PROVINCE",stv_er)
     
         ALLOCATE(GAS_NODE_COUNTRY(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0380","GAS_NODE_COUNTRY",stv_er)
     
         ALLOCATE(GAS_STORAGE_ALLOC_FACTOR(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0381",
     +                              "GAS_STORAGE_ALLOC_FACTOR",stv_er)
     
         ALLOCATE(GAS_NODE_TYPE_INDEX(MAX_GAS_GROUP_INDEX),stat=stv_er)
         call check_alloc("gas_objt:0382","GAS_NODE_TYPE_INDEX",stv_er)
     
         ALLOCATE(GAS_STORAGE_INDEX(MAX_GAS_GROUP_INDEX,2),stat=stv_er)
         call check_alloc("gas_objt:0383","GAS_STORAGE_INDEX",stv_er)
     
         ALLOCATE(GAS_STORAGE_POSITION(700,2),stat=stv_er)
         call check_alloc("gas_objt:0384","GAS_STORAGE_POSITION",stv_er)
     
         ALLOCATE(GAS_HUB_REFERENCE_INDEX(MAX_GAS_GROUP_INDEX),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0385",
     +                               "GAS_HUB_REFERENCE_INDEX",stv_er)
     
!         
         ALLOCATE(MRX_VOLATILITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0386","MRX_VOLATILITY_MULT",stv_er)
     
         ALLOCATE(NIGHT_SCARCITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0387","NIGHT_SCARCITY_MULT",stv_er)
     
         ALLOCATE(WEEKEND_SCARCITY_MULT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0388",
     +                                 "WEEKEND_SCARCITY_MULT",stv_er)
     
         ALLOCATE(PRICE_CAP(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0389","PRICE_CAP",stv_er)
     
         ALLOCATE(MAX_HOURLY_GN_IMPORT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0390","MAX_HOURLY_GN_IMPORT",stv_er)
     
         ALLOCATE(MAX_HOURLY_GN_EXPORT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0391","MAX_HOURLY_GN_EXPORT",stv_er)
     
         ALLOCATE(HYDRO_LOAD_AGGREGATION(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0392",
     +                                "HYDRO_LOAD_AGGREGATION",stv_er)
     
         ALLOCATE(GN_REGIONAL_PLANNING_AREA(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0393",
     +                             "GN_REGIONAL_PLANNING_AREA",stv_er)
     
         ALLOCATE(REPORT_CL_CAPACITY(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0394","REPORT_CL_CAPACITY",stv_er)
     
         ALLOCATE(ASSET_CLASS_ID(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0395","ASSET_CLASS_ID",stv_er)
     
         ALLOCATE(ASSET_CLASS_REV_ALLOC_VECTOR(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0396",
     +                          "ASSET_CLASS_REV_ALLOC_VECTOR",stv_er)
     
         ALLOCATE(TIME_ZONE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0397","TIME_ZONE",stv_er)
     
         ALLOCATE(CAPACITY_ADDER(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0398","CAPACITY_ADDER",stv_er)
     
         ALLOCATE(NOX_SEASON(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0399","NOX_SEASON",stv_er)
     
         ALLOCATE(PURCHASE_POWER_ASSIGN(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0400",
     +                                 "PURCHASE_POWER_ASSIGN",stv_er)
     
         ALLOCATE(CREATE_HOURLY_PRICE(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0401","CREATE_HOURLY_PRICE",stv_er)
     
         ALLOCATE(HOURLY_PRICE_NAME(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0402","HOURLY_PRICE_NAME",stv_er)
     
         ALLOCATE(PURCHASE_ASSET_CLASS_ID(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0403",
     +                               "PURCHASE_ASSET_CLASS_ID",stv_er)
     
         ALLOCATE(PURCHASE_ASSET_ALLOC_VECTOR(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0404",
     +                           "PURCHASE_ASSET_ALLOC_VECTOR",stv_er)
     
         ALLOCATE(OPERATING_COSTS(25,GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0405","OPERATING_COSTS",stv_er)
     
         ALLOCATE(CAPACITY_VALUES(25,GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0406","CAPACITY_VALUES",stv_er)
     
         ALLOCATE(CUM_RES_REMAIN(25,GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0407","CUM_RES_REMAIN",stv_er)
     
         ALLOCATE(POTENTIAL_RESERVES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0408","POTENTIAL_RESERVES",stv_er)
     
         ALLOCATE(PROVEN_RESERVES(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0409","PROVEN_RESERVES",stv_er)
     
         ALLOCATE(MAX_DAILY_EXTRACTION_RATE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0410",
     +                             "MAX_DAILY_EXTRACTION_RATE",stv_er)
     
         ALLOCATE(MAX_MONTHLY_EXTRACTION_RATE(GAS_GROUPS_RECORDS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0411",
     +                           "MAX_MONTHLY_EXTRACTION_RATE",stv_er)
     
         ALLOCATE(SHRINKAGE_PERCENT(GAS_GROUPS_RECORDS),stat=stv_er)
         call check_alloc("gas_objt:0412","SHRINKAGE_PERCENT",stv_er)
     
         ALLOCATE(SHRINKAGE_COST(GAS_GROUPS_RECORDS),stat=stv_er) 
         call check_alloc("gas_objt:0413","SHRINKAGE_COST",stv_er)
...
!
     ...
     +                          LOCAL_COUNTRY_NAME,
     +                          TEMP_STORAGE_ALLOC
            IF(GROUP_ACTIVE(GAS_GROUP) == 'F') THEN 
              CYCLE
            ENDIF
!
! 10/03/02. REGIONAL CONSOLIDATION FOR BURESH.
...
!
         ...
         allocate(GS_POS(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0414","GS_POS",stv_er)
         allocate(GT_POS(MAX_LINK_ID),stat=stv_er)
         call check_alloc("gas_objt:0415","GT_POS",stv_er)
...
!         
     ...
     +                      NODE_COUNTRY,
     +                      MON_BASIS_PRICE_DIFF)
         allocate(LNG_SCENARIO_PRICE(NUM_LNG,12,30),stat=stv_er)
         call check_alloc("gas_objt:0416","LNG_SCENARIO_PRICE",stv_er)
         allocate(LNG_INDEX(NUM_LNG),stat=stv_er)
         call check_alloc("gas_objt:0417","LNG_INDEX",stv_er)
         allocate(LNG_HUB_INDEX(NUM_LNG),stat=stv_er)
         call check_alloc("gas_objt:0418","LNG_HUB_INDEX",stv_er)
         allocate(STORAGE_INJ_INDEX(NUM_STORAGE_INJ),stat=stv_er)
         call check_alloc("gas_objt:0419","STORAGE_INJ_INDEX",stv_er)
         allocate(STORAGE_INJ_HUB_INDEX(NUM_STORAGE_INJ),stat=stv_er)
         call check_alloc("gas_objt:0420",
     +                                 "STORAGE_INJ_HUB_INDEX",stv_er)
         allocate(STORAGE_WD_INDEX(NUM_STORAGE_WD),stat=stv_er)
         call check_alloc("gas_objt:0421","STORAGE_WD_INDEX",stv_er)
         allocate(STORAGE_WD_HUB_INDEX(NUM_STORAGE_WD),stat=stv_er)
         call check_alloc("gas_objt:0422","STORAGE_WD_HUB_INDEX",stv_er)
         allocate(NODE_TYPE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0423","NODE_TYPE",stv_er)
         allocate(NODE_COUNTRY(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0424","NODE_COUNTRY",stv_er)
         allocate(MONTHLY_12_NODE_PRICE(MAX_NODES,12),stat=stv_er)
         call check_alloc("gas_objt:0425",
     +                                 "MONTHLY_12_NODE_PRICE",stv_er)
         allocate(MON_BASIS_PRICE_DIFF(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0426","MON_BASIS_PRICE_DIFF",stv_er)
...
!         
         IF(TEMP_I2 > 0) THEN
            DO J = 1, TEMP_I2 
               ...
               DO I = 1,  MAX_NODES
                  IF(GET_GAS_GROUP_INDEX(I) /= HUB_GS) THEN 
                    CYCLE
                  ENDIF
! RESETS THE INDEX                  
                  LNG_HUB_INDEX(J) = I
...
!***********************************************************************
     ...
     +                             ANNUAL_LINK_VOLUME,
     +                             AnnualGasCumSumPriceDiff)
         allocate(ANNUAL_NODE_PRICE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0427","ANNUAL_NODE_PRICE",stv_er)
         allocate(ANNUAL_NODE_VOLUME(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0428","ANNUAL_NODE_VOLUME",stv_er)
         allocate(ANNUAL_LINK_PRICE(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0429","ANNUAL_LINK_PRICE",stv_er)
         allocate(ANNUAL_LINK_VOLUME(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0430","ANNUAL_LINK_VOLUME",stv_er)
         allocate(AnnualGasCumSumPriceDiff(MAX_ACTIVE_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0431",
     +                              "AnnualGasCumSumPriceDiff",stv_er)
...
!         MAX_COUNTRIES = 1
     ...
     +             BALANCE_VARS,0:12,0:MAX_COUNTRY_NUM),stat=stv_er) 
         call check_alloc("gas_objt:0432","MONTHLY_GAS_BALANCE",stv_er)
...
!
     ...
     +                    STORAGE_INJECTION,
     +                    SUPPLY_ACTIVE)
         allocate(DAILY_DEMAND(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0433","DAILY_DEMAND",stv_er)
         allocate(DAILY_INPUT_DEMAND(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0434","DAILY_INPUT_DEMAND",stv_er)
         allocate(DAILY_DEMAND_INDEX(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0435","DAILY_DEMAND_INDEX",stv_er)
         allocate(DAILY_INTER_AVAIL(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0436","DAILY_INTER_AVAIL",stv_er)
         allocate(DAILY_MARG_NODE_PRICE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0437",
     +                                 "DAILY_MARG_NODE_PRICE",stv_er)
         allocate(DAILY_AVE_NODE_PRICE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0438","DAILY_AVE_NODE_PRICE",stv_er)
         allocate(MONTHLY_NODE_PRICE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0439","MONTHLY_NODE_PRICE",stv_er)
         allocate(MONTHLY_NODE_RESERVE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0440","MONTHLY_NODE_RESERVE",stv_er)
         allocate(LP_DAILY_DEMAND(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0441","LP_DAILY_DEMAND",stv_er)
         allocate(LP_NODE_COST(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0442","LP_NODE_COST",stv_er)
         allocate(LOCAL_COST_CURVE(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0443","LOCAL_COST_CURVE",stv_er)
         allocate(LOCAL_CAPACITY_CURVE(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0444","LOCAL_CAPACITY_CURVE",stv_er)
         allocate(LOCAL_ANNUAL_PRODUCTION_TARGET(MAX_NODE_ID),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0445",
     +                        "LOCAL_ANNUAL_PRODUCTION_TARGET",stv_er)
         allocate(PRODUCTION_SCARCITY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0446","PRODUCTION_SCARCITY",stv_er)
         allocate(LOCAL_EXTRACT_IN_DAY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0447","LOCAL_EXTRACT_IN_DAY",stv_er)
         allocate(LOCAL_HARD_EXTRACT_IN_DAY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0448",
     +                             "LOCAL_HARD_EXTRACT_IN_DAY",stv_er)
         allocate(LOCAL_MIN_EXTRACT_IN_DAY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0449",
     +                              "LOCAL_MIN_EXTRACT_IN_DAY",stv_er)
         allocate(CONSTANT_EXTRACT_IN_DAY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0450",
     +                               "CONSTANT_EXTRACT_IN_DAY",stv_er)
         allocate(CONSTANT_HARD_EXTRACT_IN_DAY(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0451",
     +                          "CONSTANT_HARD_EXTRACT_IN_DAY",stv_er)
         allocate(DAILY_LINK_VOLUME(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0452","DAILY_LINK_VOLUME",stv_er)
         allocate(DAILY_LINK_PRICE(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0453","DAILY_LINK_PRICE",stv_er)
         allocate(DAILY_LINK_CF(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0454","DAILY_LINK_CF",stv_er)
         allocate(DAILY_LINK_LIMIT(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0455","DAILY_LINK_LIMIT",stv_er)
         allocate(DAILY_LINK_COMMOD_COST(MAX_ACTIVE_GAS_LINKS),
     +                                                    stat=stv_er)
         call check_alloc("gas_objt:0456",
     +                                "DAILY_LINK_COMMOD_COST",stv_er)
         allocate(DAILY_NODE_VOLUME(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0457","DAILY_NODE_VOLUME",stv_er)
         allocate(DAILY_GAS_SUR_DEF_VOLUME(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0458",
     +                              "DAILY_GAS_SUR_DEF_VOLUME",stv_er)
         allocate(MONTHLY_NODE_VOLUME(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0459","MONTHLY_NODE_VOLUME",stv_er)
         allocate(MONTHLY_NODE_VOL_CURRENT_BASIN(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0460",
     +                        "MONTHLY_NODE_VOL_CURRENT_BASIN",stv_er)
         allocate(MONTHLY_LINK_VOLUME(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0461","MONTHLY_LINK_VOLUME",stv_er)
         allocate(MONTHLY_LINK_PRICE(MAX_ACTIVE_GAS_LINKS),stat=stv_er)
         call check_alloc("gas_objt:0462","MONTHLY_LINK_PRICE",stv_er)
         allocate(A_COEFFICIENT_STORAGE(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0463",
     +                                 "A_COEFFICIENT_STORAGE",stv_er)
         allocate(B_COEFFICIENT_STORAGE(MAX_NODE_ID),stat=stv_er)
         call check_alloc("gas_objt:0464",
     +                                 "B_COEFFICIENT_STORAGE",stv_er)
         allocate(STORAGE_WITHDRAWAL(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0465","STORAGE_WITHDRAWAL",stv_er)
         allocate(STORAGE_INJECTION(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0466","STORAGE_INJECTION",stv_er)
         allocate(SUPPLY_ACTIVE(MAX_NODES),stat=stv_er)
         call check_alloc("gas_objt:0467","SUPPLY_ACTIVE",stv_er)
...
!               I = J ! TEMP. TEST. GAS_LOAD_GROUP_2_TG(J)
               ALPHA_J = GET_NODE_ALPHA_ORDER(J)
               GS = GET_GAS_GROUP_INDEX(ALPHA_J)
               IF(.NOT. GAS_SUPPLY_GROUP_ACTIVE(GS)) THEN 
                 CYCLE
               ENDIF
               WRITE(TEMP_STR,'(I6)') ALPHA_J-1
               TEMP_STR = '"'//GET_GAS_GROUP_NAME(GS)//'"'//
...
! DEMAND NODES
            ...
            DO J = 1,  MAX_NODES
               ALPHA_J = GET_NODE_ALPHA_ORDER(J)
               GS = GET_GAS_GROUP_INDEX(ALPHA_J)
               IF(.NOT. GAS_DEMAND_GROUP_ACTIVE(GS)) THEN 
                 CYCLE
               ENDIF
               WRITE(TEMP_STR,'(I6)') ALPHA_J-1
               TEMP_STR = '"'//GET_GAS_GROUP_NAME(GS)//'"'//
...
! PRICE NODES
            ...
            DO J = 1,  MAX_NODES
               ALPHA_J = GET_NODE_ALPHA_ORDER(J)
               GS = GET_GAS_GROUP_INDEX(ALPHA_J)
               IF(GET_GAS_NODE_TYPE_INDEX(GS) /= 5) THEN 
                 CYCLE
               ENDIF
               WRITE(TEMP_STR,'(I6)') ALPHA_J-1
               TEMP_STR = '"'//GET_GAS_GROUP_NAME(GS)//'"'//
...
!
               DO L_J = 1, MAX_ACTIVE_GAS_LINKS
                  ...
                  IF(B_ID > 0) THEN
                     B_ID = GET_GAS_GROUP_POSITION(B_ID)
                  ENDIF
                  IF(.NOT. GAS_DEMAND_GROUP_ACTIVE(B_ID)) THEN 
                    CYCLE
                  ENDIF
...
!               ENDIF
     ...
     +                              DAILY_LINK_LIMIT(L_K) < 0.001) THEN 
                 CYCLE
               ENDIF
...
!***********************************************************************
!
!        PROGRAM TO READ MULTI-TAB INFORMATION ON GAS_WEATHER DEMAND
!                 AND CONVERT TO BINARY FORMAT
!                       COPYRIGHT (C) 2001
!      ALL RIGHTS RESERVED M.S. GERBER & ASSOCIATES, INC.
!
...
!         
         allocate(TEMP_TRANS_CLASS_POINTER(0:1023),stat=stv_er)
         call check_alloc("gas_objt:0468",
     +                              "TEMP_TRANS_CLASS_POINTER",stv_er)
         allocate(TEMP_ASSET_CLASS_POINTER(0:1023),stat=stv_er)
         call check_alloc("gas_objt:0469",
     +                              "TEMP_ASSET_CLASS_POINTER",stv_er)
         allocate(TEMP_CUST_CLASS_POINTER(0:1023),stat=stv_er)
         call check_alloc("gas_objt:0470",
     +                               "TEMP_CUST_CLASS_POINTER",stv_er)
...
!         
      allocate(TEMP_TRANS_CLASS_POINTER(0:1023),stat=stv_er)
      call check_alloc("gas_objt:0471",
     +                              "TEMP_TRANS_CLASS_POINTER",stv_er)
      allocate(TEMP_ASSET_CLASS_POINTER(0:1023),stat=stv_er)
      call check_alloc("gas_objt:0472",
     +                              "TEMP_ASSET_CLASS_POINTER",stv_er)
      allocate(TEMP_CUST_CLASS_POINTER(0:1023),stat=stv_er)
      call check_alloc("gas_objt:0473","TEMP_CUST_CLASS_POINTER",stv_er)
...
!***********************************************************************
!
      FUNCTION MANAGE_GAS_WEATHER_DEMAND_FCSTS()
!
!***********************************************************************
!
!
...
!
     ...
     +               MANAGE_GAS_WEATHER_DEMAND_FCSTS,
     +               GET_GW_LOAD
...
!
     ...
     +                                                  stat=stv_er) 
            call check_alloc("gas_objt:0474","GW_LOAD",stv_er)
...
!
     ...
     +               LOCAL_MAX_MONTHS,
     +               LOCAL_MAX_DAYS,
     +               LOCAL_MAX_YEARS),stat=stv_er)
            call check_alloc("gas_objt:0475","GW_INDEX",stv_er)
...
!     
            ...
            MANAGE_GAS_WEATHER_DEMAND_FCSTS = .TRUE.
         ENDIF
!
      RETURN
!      
...
! 060107. 
!-----
! B2LogRF is the base-2 logarithm of the max allowable bounds-relaxation factor
...
!        TEMP_L = GregGetDaysDemand(ErrOrEnd,NodeQty,nNode)
        if(ErrOrEnd/=0) then 
          exit ! error or EoF reading demand data into NodeQty
        endif
        nDays=nDays+1
!
...
!cc       call WriteLpInputs() ! using only internal variables and arrays
     ...
     +      write_PCX_input)
        ...
        ! after 20080103, always expect Feasible when using AmortizeUE
          if(Feasible.or.AmortizeUE) then 
            exit ! else try again with relaxed upper-bounds
          endif
        end do
...
!        
!       call WriteStrAt(10,5,'solved LP using PCX.exe')
...
!           write(4,'(2i4,2f12.2,a)') iLink,iSeg,LinkQtySeg(iSeg,iLink),
!    +        LinkAvQSeg(iSeg,iLink),' LAQ'
          end do
        end do
...
!        end do
!      end do
!     nDays=365 ! extend the averages to cover a whole year
...
!      PrtDetail=0 ! for normal operation of the model by a customer
!     PrtDetail=1 ! for debugging details (some)
!     PrtDetail=2 ! for debugging details (more)
!         call GetDaysDemand(ErrOrEnd,NodeQty)
!        TEMP_L = GregGetDaysDemand(ErrOrEnd,NodeQty,nNode)
        do i = 1, nNode
...
!-----
      ...
      allocate(NodeID  (nNode),stat=stv_er)
      call check_alloc("gas_objt:0476","NodeID",stv_er)
     
      allocate(NodeType(nNode),stat=stv_er)
      call check_alloc("gas_objt:0477","NodeType",stv_er)
     
      allocate(NodeQtyLoB(nNode),stat=stv_er) ! min quantity at supply MMCF/D
      call check_alloc("gas_objt:0478","NodeQtyLoB",stv_er)
     
      allocate(NodeQtyLim(nNode),stat=stv_er) ! max quantity at supply MMCF/D
      call check_alloc("gas_objt:0479","NodeQtyLim",stv_er)
     
      allocate(NodeQtyHardLim(nNode),stat=stv_er) ! max quantity at supply MMCF/D
      call check_alloc("gas_objt:0480","NodeQtyHardLim",stv_er)
     
      allocate(NodeQty (nNode),stat=stv_er) ! quantity demand or supply MMCF/D
      call check_alloc("gas_objt:0481","NodeQty",stv_er)
     
      allocate(NodeQtyOfLR(nNode),stat=stv_er) ! to/from LastResort; useful only if AmortizeUE
      call check_alloc("gas_objt:0482","NodeQtyOfLR",stv_er)
     
      allocate(NodePri (nNode),stat=stv_er) ! $/MCF
      call check_alloc("gas_objt:0483","NodePri",stv_er)
     
      allocate(NodeMgC (nNode),stat=stv_er) ! $/MCF
      call check_alloc("gas_objt:0484","NodeMgC",stv_er)
     
      allocate(NodeName(nNode),stat=stv_er)
      call check_alloc("gas_objt:0485","NodeName",stv_er)
...
!-----
      ...
      allocate(LinkID (nLink),stat=stv_er)
      call check_alloc("gas_objt:0486","LinkID",stv_er)
     
      allocate(LinkCap(nLink),stat=stv_er) ! A-to-B capacity (nominal 100%)
      call check_alloc("gas_objt:0487","LinkCap",stv_er)
     
      allocate(LinkChg(nLink),stat=stv_er) ! A-to-B commodity charge at 0%
      call check_alloc("gas_objt:0488","LinkChg",stv_er)
     
      allocate(LinkEff(nLink),stat=stv_er) ! A-to_B fractional retention of inlet quantity
      call check_alloc("gas_objt:0489","LinkEff",stv_er)
     
      allocate(LinkAvC(nLink),stat=stv_er) ! weighted average cost $/MCF
      call check_alloc("gas_objt:0490","LinkAvC",stv_er)
     
      allocate(LinkMgC(nLink),stat=stv_er) ! marginal cost $/MCF
      call check_alloc("gas_objt:0491","LinkMgC",stv_er)
     
      allocate(LinkNdID(0:1,nLink),stat=stv_er) ! A-to-B IDs
      call check_alloc("gas_objt:0492","LinkNdID",stv_er)
     
      allocate(LinkNdSN(0:1,nLink),stat=stv_er) ! A-to-B IDs' serial numbers (contiguous)
      call check_alloc("gas_objt:0493","LinkNdSN",stv_er)
     
      allocate(LinkQty(nLink),stat=stv_er) ! A-to-B quantity at inlet in MMCF
      call check_alloc("gas_objt:0494","LinkQty",stv_er)
     
      allocate(LinkQOF(nLink),stat=stv_er) ! A-to-B over-limit quantity at inlet in MMCF
      call check_alloc("gas_objt:0495","LinkQOF",stv_er)
     
      allocate(LinkExpQty(nLink),stat=stv_er) ! economic expansion quantity
      call check_alloc("gas_objt:0496","LinkExpQty",stv_er)
     
      allocate(LinkQtySeg(nCCSegs,nLink),stat=stv_er) ! segment usage in MMCF
      call check_alloc("gas_objt:0497","LinkQtySeg",stv_er)
     
      allocate(LinkAvQSeg(nCCSegs,nLink),stat=stv_er) ! segment usage in MMCF, accumulated
      call check_alloc("gas_objt:0498","LinkAvQSeg",stv_er)
     
      allocate(LinkFunc(nCCSegs,0:2,nLink),stat=stv_er) ! segment x(EoS fract cap),y($/MCF),DeltaX
      call check_alloc("gas_objt:0499","LinkFunc",stv_er)
...
!-----
!     subroutine ReplacdDelims(s)
!     character*(*) s
!     integer*2 j,LengNB
!
!     do j=LengNB(s),1,-1
!       if((s(j:j)==' ').or.(s(j:j)=='/')) s(j:j)='_'
!     end do
!     end
!----
...
!-----
!     real*4 function SupValue(n,a)
!   ! returns the largest algebraic value in array a
!     integer*2 n,i
!     real*4 aSup,a(*)
!
!     aSup=-1.0e32
!     do i=1,n
!       if(aSup<a(i)) aSup=a(i)
!     end do
!     SupValue=aSup
!     end
!-----
...
!
      do i=1,n
!       write(4,'(4i6,a)') i,n,i2Array(i),ThisItem,' II'
        if(i2Array(i)==ThisItem) then 
          exit
        endif
      end do
!     if(i>n) call ps(1,'specified index not found on list (IndexIn)')
...
!
      ...
      do while(Gap>0)
        do i=Gap+1,nItems
          ...
          do while(j>0)
            k=j+Gap
!           write(4,'(9i9,a)') nItems,Gap,i,j,k,
!    +        MinorKey(j),MajorKey(j),MinorKey(k),MajorKey(k),' SBAKbef'
!           jFull=FullKey(j)
!           kFull=FullKey(k)
!           if(jFull<=kFull) then
            if(FullKey(j)<=FullKey(k)) then
              ...
              Val(k)=rHold
!           write(4,'(9i9,a)') nItems,Gap,i,j,k,
!    +        MinorKey(j),MajorKey(j),MinorKey(k),MajorKey(k),' SBAKaft'
            end if
            j=j-Gap
          end do
        end do
...
!   ! verify ascending order of Key array
!     kFull=0
!     do i=1,nItems
!       jFull=FullKey(i)
!       if(jFull<kFull) then
!         write(4,'(3i9,2i17,a)')
!    +      i,MinorKey(i),MajorKey(i),kFull,jFull,' SBAK final'
!         call ps(1,'SBAK out of sorts')
!       end if
!       kFull=jFull
!     end do
...
!      if(CallDepth>48) then
    ...
    ! weight sourcing nodes' prices by the net Qty into the node at iDNod
      ...
      ! 20080122 expedient:  ignore noisy nodes causing cycling
        do kLink=1,nLinkInto(iDNod)
          iLink=LkUsdInto(kLink,iDNod)
          if(LinkQty(iLink)<=0.0) then 
            cycle ! skip links with 0 weight
          endif
...
!             DNodQty=DNodQty-LinkQty(iLink)*LinkEff(iLink)
              LinkQty(iLink)=0.0 ! preclude cycling through iLink at later calls
!             InfRation=0.999 ! allow next-discovered cycle to have a larger InfRation
              Cyclic=.true.
              return
!           else
!           write(DbgU,'(4i4,i3,3i4,f7.4,a)') CallDepth,iDNod,iNode,
!    +        iLink,kLink,InfCycOrg,CycOrg,MinTestDepth,InfRation,
!    +        ' cycle orig>0 & testD'
            end if
!         else
!           write(DbgU,'(4i4,i3,3i4,f7.4,a)') CallDepth,iDNod,iNode,
...
!cc   end if
     ...
     +      )
          if(Cyclic) return
!         SourMgCst=(NodeMgC(iNode)+LinkMgC(iLink)/1000.)/LinkEff(iLink) ! $/MCF
...
!    +      (LinkAvC(iLink)+NodePri(iNode)*1000.)                ! Taudit 22.5342
!    +      (LinkAvC(iLink)/LinkEff(iLink)+NodePri(iNode)*1000.) ! Taudit 22.4479
!    +      (LinkAvC(iLink)+NodePri(iNode)*1000.)/LinkEff(iLink) ! Taudit  0.1553
...
!-----
!Small: NumSegs 2
!Small: (113+2*10) 133 needed if_NumSegs<10 in GasNetwk
...
!cc     if(iInpCsv==2) call AlignCols(InpU)
!       call SkipLines(InpU,1) ! the "9," line
...
!-----
     ...
     +           (CI_index(OneLine,'PRICE POINTS')>0)) then
        ...
        if((FirstCh/='1').or.Negat(cDummy).or.(NodeDir<-1)) then 
          cycle
        endif
        nNode=nNode+1
        if    (NodeDir== 1) then
!Small:   if(nSNod>=3) cycle ! 9 20080131
          nSNod=nSNod+1
        elseif(NodeDir==-1) then
!Small:   if(nDNod>=2) cycle ! 5 20080131
          nDNod=nDNod+1
        end if
      end do
  410 write(DbgU,'(i4," EoF"/)') InpU
      ...
      allocate(NodeSNofDFRank(nNode),stat=stv_er)
      call check_alloc("gas_objt:0500","NodeSNofDFRank",stv_er)
...
!Small: if((NodeDir== 1).and.(iNode==3)) cycle ! 14 20080131
!Small: if((NodeDir==-1).and.(iNode==2)) cycle !  5 20080131
        iNode=iNode+1
        NodeID(iNode)=j
...
!
      ...
      do
        ...
        if((FirstCh/='1').or.Negat(cDummy)) then
          cycle
        endif
        ignored=B0
...
!-----
      ...
      allocate(DNodName(nNode),stat=stv_er)
      call check_alloc("gas_objt:0501","DNodName",stv_er)
  ...
  428 continue ! write(DbgU,'(1x,3a)') ('[',FiveLabels(j),']',j=1,5)
      do j=1,nNode
        if(LengNB(DNodName(j))<1) then 
          exit ! should exit at j>nDNod
        endif
        ThisNdName=DNodName(j)
        do iNode=1,nNode
          if(trim(NodeName(iNode))==trim(ThisNdName)) then 
            exit
          endif
        end do
!Small: if(iNode>nNode) cycle ! 20080131
...
!
    ! read network data from Midas-formatted CSV files
      allocate(LinkExpCst(nLink),stat=stv_er)
      call check_alloc("gas_objt:0502","LinkExpCst",stv_er)
     
      allocate(LinkExpLim(nLink),stat=stv_er)
      call check_alloc("gas_objt:0503","LinkExpLim",stv_er)
     
      allocate(LinkCCRF  (nLink),stat=stv_er)
      call check_alloc("gas_objt:0504","LinkCCRF",stv_er)
     
      allocate(DFValue(SupDFRank),stat=stv_er)
      call check_alloc("gas_objt:0505","DFValue",stv_er)
...
!cc     if(FirstCh/='1') ToDmdNod=(CI_index(OneLine,'DEMAND')>0)
        if(FirstCh/='1') then 
          cycle
        endif
...
!       if(iLink>nSNod) r4=50.0 ! to exacerbate pricing errors
     ...
     +        'ID not active B'
        ...
        ! read Cap(MMCF/D) and CommodityCost($/MCF)
          j=j+2
          read(OtherField(j),*) r4
!cc       if(B0.and.ToDmdNod) r4=r4*100.0 ! kludge for B0
          j=j+2
! 093008.
          if(ABS(r4) <= 0.1) THEN
...
!
     ...
     +      LinkCap(iLink),r4,' cap,chg'
        ...
        if    (iLink==PreviLink) then
          ...
          nZcpLink=nZcpLink+1
          cycle
!       elseif(iLink> PreviLink+1) then
!         write(DbgU,'(i4,a)') iLink,' link is bidirectional'
!         nBidLink=nBidLink+1
!         LinkEff(iLink)=LinkEff(iLink-1)
!         LinkID (iLink)=LinkID (iLink-1)
!         do j=0,1 ! copy IDs and their serial numbers in reverse order
!           k=mod(j+1,2)
!           LinkNdID(j,iLink)=LinkNdID(k,iLink-1)
!           LinkNdSN(j,iLink)=LinkNdSN(k,iLink-1)
!         end do
        end if
...
!cc         if(yDpMCF(i)<1.0) yDpMCF(i)=1.0 ! kludge for B0 while testing
          end if
!         write(DbgU,'(3i4,2f15.3)') i,j,k,xFrCap(i),yDpMCF(i)
...
!       do j=0,1 ! same PWL function applies to both directions (if allowed)
...
!         if(iLink==PreviLink+1) exit ! this link is 1-way
!       end do
        r4=0.0
        CapExpCostRate=0.0
...
! 030614. gt.
  ...
  439   write(DbgU,'(i4,3f9.2,4(1x,a))') iLink,r4,LinkCCRF(iLink),
     ...
     +    'LXC,GLXRF,LXL'
!
!       if(yDpMCF(nSegUsd)<=0.0) write(DbgU,'(i4,a)') iLink,' 0-cost link'
...
!     call DisplayField(12) ! judged useless after 20070827
...
!       r4=0.0
!       read(OtherField(12),*,err=442,end=442) r4 ! capacity MMCF/D
! 442   if(r4<0.0) r4=0.0
!       NodeQty(iNode)=r4 ! NodeQty is used to report Qty supplied by sources
!
        r4=-1.0
        read(OtherField(52),*,err=443,end=443) r4 ! nominal capacity limit MMCF/D [supply]
! 443   if(r4<0.0) r4=0.0 ! negative value implies 'revert to mominal capacity'
  443   NodeQtyLim(iNode)=r4
!cc     if(B0.and.(NodeQty(iNode)==0.0).and.(r4>0.0))
!cc  +    NodeQty(iNode)=r4 ! kludge for B0
!
        r4=-1.0
        read(OtherField(132),*,err=444,end=444) r4 ! emergency (hard) capacity limit MMCF/D [supply]
! 444   if(r4<0.0) r4=0.0 ! negative value implies 'revert to nominal capacity'
...
!         call EnumerateFields(OneLine,(113+2*10),1,DbgU)
...
!     +  ' ---------------------------'
      do while(iNode<=mNode)
        ...
        ! NodeQty is not yet available for testing
          do j=1,SupDFRank
            if(NodeSNofDFRank(j)==iNode) then 
              exit
            endif
          end do
!         if(NodeQty(iNode)<=0.0) then ! iNode cannot function as a Demand node
...
!           NodeQty (j)=NodeQty (k) ! not yet available for Demand nodes
...
!
      jCol=int4(nNode)
      allocate(LkCstNode(nNode),stat=stv_er)
      call check_alloc("gas_objt:0506","LkCstNode",stv_er)
     
      allocate(SpQtyNode(nNode),stat=stv_er)
      call check_alloc("gas_objt:0507","SpQtyNode",stv_er)
     
      allocate(LkUsdFrom(nNode),stat=stv_er)
      call check_alloc("gas_objt:0508","LkUsdFrom",stv_er)
     
      allocate(nLinkInto(nNode),stat=stv_er)
      call check_alloc("gas_objt:0509","nLinkInto",stv_er)
     
      allocate(NdLkdInto(SupnLkAtNd,nNode),stat=stv_er)
      call check_alloc("gas_objt:0510","NdLkdInto",stv_er)
     
      allocate(LkUsdInto(SupnLkAtNd,nNode),stat=stv_er)
      call check_alloc("gas_objt:0511","LkUsdInto",stv_er)
     
      allocate(nPriorSegs(nLink),stat=stv_er)
      call check_alloc("gas_objt:0512","nPriorSegs",stv_er)
     
      allocate(LinkType  (nLink),stat=stv_er)
      call check_alloc("gas_objt:0513","LinkType",stv_er)
     
      allocate(LinkChgOrg(nLink),stat=stv_er)
      call check_alloc("gas_objt:0514","LinkChgOrg",stv_er)
     
      allocate(LnkQtyP   (nLink),stat=stv_er)
      call check_alloc("gas_objt:0515","LnkQtyP",stv_er)
     
      allocate(LinkNdBi  (nLink),stat=stv_er)
      call check_alloc("gas_objt:0516","LinkNdBi",stv_er)
    ...
    ! list all nodes connected by a single link
     ...
     +    (LinkNdID(j,iLink),j=0,1),LinkEff(iLink)
        LinkDir=0 ! default, valid for Intermed links
!cc     do j=1,1 ! 0 => LinksFrom, 1 => LinksInto
!cc       k=mod(1+j,2)
          j=1
          k=0
          ...
          if((iNode>nNode).or.(kNode>nNode)) then 
...
!       write(DbgU,'( i12)',advance='no') LinkNdBi(iLink)
     ...
     +  ' >= # links at any node'
!cc   PriceSoLR=PriceSoLR*2.0 ! sufficient to avoid economic use except to gain feasibility
...
!       if    (NodeType(iSNod)== 1) then
!         write(DbgU,'( f10.0)',advance='no') NodeQty(iSNod)
!       elseif(NodeType(iSNod)==-1) then
!       else
!       end if
        write(DbgU,'(1x)',advance='yes') ! writes CrLf after blank
        if((NodeType(iSNod)== 1).and.(nLinkFrTo(0)==0)) call ps(1,
...
!
!      write(DbgU,'(/a)')
!     +  ' source nodes one path-step from destination on left'
!      do iNode=1,nNode
!        write(DbgU,'(1x)',advance='no') ! expect advance=no to follow
!        write(DbgU,'( i3,i4,a)',advance='no')NodeType(iNode),iNode,':'
!        do k=1,nLinkInto(iNode)
!          write(DbgU,'( i4)',advance='no') NdLkdInto(k,iNode)
!        end do
!        write(DbgU,'(1x)',advance='yes') ! writes CrLf after blank
!      end do
      write(DbgU,'(1x)')
...
!    +  (NodeQty(NodeSNofDFRank(j)),j=1,SupDFRank)
...
!       write(DbgU,'(2i4,f10.1,a)') j,k,DFValue(j),' Demand as read'
      end do
      if(DayStr(1:2)=='  ') DayStr=DayStr(3:4) ! trim() doesn't cut leading blanks
      return
...
!
      allocate(A4Equiv(nVar),stat=stv_er)
      call check_alloc("gas_objt:0517","A4Equiv",stv_er)
     
      allocate(VarDesc(nVar),stat=stv_er)
      call check_alloc("gas_objt:0518","VarDesc",stv_er)
     
      allocate(TNofVar(nTVar),stat=stv_er)
      call check_alloc("gas_objt:0519","TNofVar",stv_er)
     
      allocate(cVect(nVar),stat=stv_er)
      call check_alloc("gas_objt:0520","cVect",stv_er)
     
      allocate(xVect(nVar),stat=stv_er)
      call check_alloc("gas_objt:0521","xVect",stv_er)
     
      allocate(bVect(mReq),stat=stv_er)
      call check_alloc("gas_objt:0522","bVect",stv_er)
     
      allocate(SpVec(mReq),stat=stv_er)
      call check_alloc("gas_objt:0523","SpVec",stv_er)
     
      allocate(ReVec(mReq),stat=stv_er)
      call check_alloc("gas_objt:0524","ReVec",stv_er)
     
      allocate(ConsMtxNze(ConsMtSLim),stat=stv_er)
      call check_alloc("gas_objt:0525","ConsMtxNze",stv_er)
     
      allocate(zbColOfNze(ConsMtSLim),stat=stv_er)
      call check_alloc("gas_objt:0526","zbColOfNze",stv_er)
     
      allocate(zbRowOfNze(ConsMtSLim),stat=stv_er)
      call check_alloc("gas_objt:0527","zbRowOfNze",stv_er)
     
      allocate(jUbVec(nUbV),stat=stv_er)
      call check_alloc("gas_objt:0528","jUbVec",stv_er)
     
      allocate(UbValu(nUbV),stat=stv_er)
      call check_alloc("gas_objt:0529","UbValu",stv_er)
     
      allocate(UbVOrg(nUbV),stat=stv_er)
      call check_alloc("gas_objt:0530","UbVOrg",stv_er)
     
      if(nLbV>0) then
      allocate(jLbVec(nLbV),stat=stv_er)
      call check_alloc("gas_objt:0531","jLbVec",stv_er)
     
      allocate(LbValu(nLbV),stat=stv_er)
      call check_alloc("gas_objt:0532","LbValu",stv_er)
     
      end if
      allocate(NodeSrcNip(mNode,nSNod),stat=stv_er) ! number of source paths in cost pool for iNode with iSNod as Supply
      call check_alloc("gas_objt:0533","NodeSrcNip",stv_er)
     
      allocate(NodeSrcQty(mNode,nSNod),stat=stv_er)
      call check_alloc("gas_objt:0534","NodeSrcQty",stv_er)
     
      allocate(NodeSrcCst(mNode,nSNod),stat=stv_er)
      call check_alloc("gas_objt:0535","NodeSrcCst",stv_er)
     
      allocate(CycleOrgD(mNode),stat=stv_er)
      call check_alloc("gas_objt:0536","CycleOrgD",stv_er)
...
!         write(DbgU,'(2i4,2e10.3,a)') iLink,iNode,NodePri(iNode),
!    +      LinkChg(iLink),' LinkChg after NodePri zeroing'
          NodePri(iNode)=NodePri(iNode)*1.0e-12 ! until LP is solved
        end if
        cVect(jVar)=NodePri(iNode)*1000. ! in ($/MMCF)
!       write(DbgU,'(3i4,f9.3,2(1x,a))') iNode,jVar,nSNod,cVect(jVar),
!    +    VarDesc(jVar),NodeName(iNode)
      end do
...
!       write(DbgU,'(3i4,i9,2(1x,a))') iNode,jVar,nSNod,
!    +    nint(cVect(jVar)),VarDesc(jVar),NodeName(iNode)
      end do
    ! each Demand node adds one variable (not upper-bounded)
      do iNode=1,mNode
        if(NodeType(iNode)/=-1) then 
          cycle ! skip non-Demand nodes
        endif
        call GNWIncrVar()
...
!       write(DbgU,'(3i4,i9,2(1x,a))') iNode,jVar,nDNod,
!    +    nint(cVect(jVar)),VarDesc(jVar),NodeName(iNode)
      end do
...
!
    ! each directional link has nSegUsd variables and upper-bounds
     ...
     +      LinkNdId(1,iLink)
!         if(j<nSegUsd) then ! after 20070615, let last segment run unbounded
            call GNWIncrUpB()
...
!           if(NodeSupplyLtd>=0.0)
!           write(DbgU,'(5i6,3e12.5,2(1x,a))') iLink,j,iUbV,jVar,iNode,
!    +        r4,NodeQtyHardLim(iNode),
!    +        UbValu(iUbV),'UbValu',trim(VarDesc(jVar))
!         end if
          cVect(jVar)=1000.*
...
!
    ! each node generates one constraint (row in ConsMtx)
      ...
      do iNode=1,mNode
        call GNWIncrReq()
!       write(DbgU,'(1x)',advance='no') ! expect advance=no to follow
!       write(DbgU,'( 2i4,a)',advance='no') iNode,iReq,' jVar: '
...
!           write(DbgU,'( i7.5)',advance='no') -(jCol-nSegUsd)
...
!         write(DbgU,'(6i5,f5.1,a)') iNode,iReq,jCol,iLink,
!    +      (LinkNdSN(j,iNode),j=0,1),r4,' FinalSegAMtxCoef'
...
!     call WriteStrAt(8,5,'after  sorting constraint matrix')
!
...
!-----
      ...
      if(ConsMtSize==ConsMtSLim) then ! move vectors' contents into larger space
        ...
        allocate(zbColHold(ConsMtSLim),stat=stv_er) ! temporary swap-space
        call check_alloc("gas_objt:0537","zbColHold",stv_er)
     
        allocate(zbRowHold(ConsMtSLim),stat=stv_er)
        call check_alloc("gas_objt:0538","zbRowHold",stv_er)
     
        allocate(ConsEHold(ConsMtSLim),stat=stv_er)
        call check_alloc("gas_objt:0539","ConsEHold",stv_er)
      ...
      ! save copies of full vectors in ~Hold
        ...
        allocate(zbColOfNze(ConsMtSLim),stat=stv_er)
        call check_alloc("gas_objt:0540","zbColOfNze",stv_er)
     
        allocate(zbRowOfNze(ConsMtSLim),stat=stv_er)
        call check_alloc("gas_objt:0541","zbRowOfNze",stv_er)
     
        allocate(ConsMtxNze(ConsMtSLim),stat=stv_er)
        call check_alloc("gas_objt:0542","ConsMtxNze",stv_er)
...
!     OneBasedOfs=OneBasedCol+(OneBasedRow-1)*nVar ! for column=major order
!     OneBasedOfs=OneBasedRow+(OneBasedCol-1)*mReq ! for row-major order
...
!     OneBasedOfs=OneBasedCol+(OneBasedRow-1)*nVar ! for column=major order
!     OneBasedOfs=OneBasedRow+(OneBasedCol-1)*mReq ! for row-major order
!     if(OneBasedOfs>ConsMtxOfs(ConsMtSize)) return
!     if(OneBasedOfs<ConsMtxOfs(1         )) return ! unlikely
!     do MidIndex=1,ConsMtSize
!       if(ConsMtxOfs(MidIndex)==OneBasedOfs) exit
!     end do
!     if(MidIndex<=ConsMtSize) RCMvalue=ConsMtxNze(MidIndex) ! e;se retain 0.0
    ! after 20070919 search separate row-major arrays for zbCol and zbRow
      ...
      do iNze=1,ConsMtSize
        if(zbColOfNze(iNze)==zbCol) then ! Nze for this column are contiguous
          ...
          if(OnTgtCol) then 
            exit ! early, since loop has passed all possible matches
          endif
        end if
      end do
...
!       j=0
!     ! above excludes final-segment costs
!       if(jVar>nTVar) then
!         iUbV=jVar-nTVar ! ignoring nLbV within nSNod
!         if((iLink<mLink).and.(iUbV>nPriorSegs(iLink))) iLink=iLink+1
!         iSeg=iUbV-nPriorSegs(iLink)
!         if(iSeg==nCCSegs) j=1 ! all but final-segment costs
!       end if
!       ObjectvEF(j)=ObjectvEF(j)+r4 ! if objective can be allocated to excluded/final-seg
...
!
  600 continue
!     if(.not.OrigBounds)
!cc   call ps(1,'not-optimal error; check *.dbg')
      write(DbgU,'(a)') ' no OPTIMAL solution; relaxing upper-bounds'
    ! relax the upper bound on the last segment of each cost curve
!     OrigBounds=B0 ! give PCX one more try at converging to Optimal Feasible solution
      do iUbV=1,nLbV ! each of nLbV was assigned an upper bound
        iNode=TNofVar(iUbV)
        if(NodeQtyHardLim(iNode)<0.0) then 
          cycle ! negative => immutable
        endif
        UbValu(iUbV)=UbValu(iUbV)*2.0
!       write(DbgU,'(i6,f16.1,a)') iUbV,UbValu(iUbV),' relaxed UB'
      end do
      iUbV=nLbV
      do iLink=1,mLink
...
!       write(DbgU,'(i6,f16.1,a)') iUbV,UbValu(iUbV),' relaxed UB'
      ...
      call flush(DbgU)
!     FileName='Presumed INFEASIBLE solution; retrying with UB=UB*10'
!     call WriteStrAt(20,5,trim(FileName))
!     write(DbgU,'(1x,a)') trim(FileName)
...
!
      do iReq=1,mReq ! cover the entire constraint set (all are equations in GasNetwk)
        ...
        do jVar=1,nVar
          ...
          QtySum=QtySum+f4*r4
          if(PrtDetail>1) then
!         ! assignments below needed to preclude 'initiating I/O during I/O' error
!           YearStr=R4toA4(f4) ! very slow
            DayStr =R4toA4(r4) ! very slow
!           write(DbgU,'( i6.6,2a5,a)',advance='no')
!    +        jVar,YearStr,DayStr,AComma
...
!
!cc      if(Skeptical)call CheckSolution(NodeType,NodeName,NodeQty,LinkEff)
...
!    +    call AppendFloat9(SnvU,r4)
!    +    call AppendFloat7(SnvU,cVect(jVar)/1000.) ! NodePri(iNode)
     ...
     +        trim(ErrMsg)
          ...
          iUbV=jVar-nTVar ! ignoring nLbV within nSNod ...
!         iLink=1+(iUbV-1)/nSegUsd
          if((iLink<mLink).and.(iUbV>nPriorSegs(iLink+1))) then
            iLink=iLink+1
            nSegUsd=nSegsOI(LinkType(iLink))
          end if
!         iSeg=iUbV-(iLink-1)*nSegUsd ! 1+mod(iUbV-1,nSegUsd)
          iSeg=iUbV-nPriorSegs(iLink)
...
!         if(iSeg<nSegUsd) then ! ONLY if nUbV=mLink*(nSegUsd-1)
!           iUbV=nLbV+jVar-nTVar-(iLink-1) ! reduced by count of skipped bounds
!           OrigUB=UbValu(iUbV)
!         else ! UbValu may have been increased by 2.0**n
!           OrigUB=LinkCap(iLink)*LinkFunc(iSeg,2,iLink) ! not used in LP
!           iNode=LinkNdSN(0,iLink)
!           if(NodeType(iNode)==1) then ! iLink drains a supply node
!             iUbV=nLbV+jVar-nTVar-(iLink-1) ! reduced by count of skipped bounds
!             NodeSupplyLtd=NodeQtyLim(iNode)
!             if(NodeSupplyLtd>=0.0) OrigUB=NodeSupplyLtd
!           end if ! re supply-node link
!         end if ! re iSeg
!         if((iSeg==nSegUsd).and.(.not.OrigBounds)) OrigUB=OrigUB/2.0
          OrigUB=UbVOrg(iUbV)
          if((r4>OrigUB*1.001).and.(r4>Near0Qty))
     +      cDummy='X' ! allow for some numerical slop
          if(iSeg==nSegUsd) then
            if(WrtDetail>0)
!    +      call AppendFloat9(LkvU,QtySum)
...
!           write(DbgU,'(i4,a,11f9.2)') iLink,' LQS',LinkQty(iLink),
!    +        (LinkQtySeg(iSeg,iLink),iSeg=1,nSegUsd)
!           write(DbgU,'(i4,i9,10f9.0,a)') iLink,jVar,(cVect(iUbV),
!    +        iUbV=jVar-nSegUsd+1,jVar),' cVect'
     ...
     +      LinkID(iLink),NodeType(LinkNdSN(0,iLink))
      ...
      call flush(DbgU)
!cc   if(B0) goto 8900 ! return ! pricing is N.A.
    ! preclude cycling on closed paths due to numerical noise
!     write(DbgU,'(2e12.5,a)') 0.0001,SupLinkQty,' SupLinkQty'
...
!         LinkAvC(iLink)=0.0 ! these two may ...
!         LinkMgC(iLink)=0.0 ! ... remain non-zero
        end if
      end do
!
!
      ...
      do iNode=1,mNode
        if(NodeType(iNode)/=1) then ! non-Supply node
          ...
          if(NodeQtyLim(iNode)>=0.0) then
            ...
            iLink=LkUsdFrom(iNode)
!           if(LinkNdSN(0,iLink)/=iNode) call ps(1,'logical error')
...
!           nSegUsd=nSegsOI(LinkType(iLink))
!           LnkQCEF(1,1)=LnkQCEF(1,1)-r4*LinkQtySeg(nSegUsd,iLink)
            if(LinkType(iLink)==1)
     +      LnkQCEF(1,1)=LnkQCEF(1,1)-r4*LinkQtySeg(nCCSegs,iLink)
!           write(DbgU,'(2i5,f7.3,e9.2,f17.3,a)')iNode,iLink,NodePri
!    +        (iNode),LinkChg(iLink),LnkQCEF(1,0),
!    +        ' NP,LC,LCS aft restore'
          end if
    ...
    ! recursively trace paths to all sources for each Demand node
     ...
     +        )
            if(.not.Cyclic) then 
              exit ! else restart at i=0 with revised LinkQtyP
            endif
          end do
! 060708
...
!       write(DbgU,'(i4,4e12.5,a)') iNode,NodeQty(iNode),NodePri(iNode),
!    +    DmdQCEF(:,2),' DS'
      end do ! iNode
!
      i=0
        ...
        do iDNod=1,mNode
          ...
          ! average price logic as used in DerivePriceFromSources
            ...
            do kLink=1,nLinkInto(iDNod)
              ...
              iNode=NdLkdInto(kLink,iDNod)
              if(NodePri(iNode)<=0.0) then 
                cycle ! iNode is possibly an Interm node
              endif
              nNZPSour=nNZPSour+1
...
!     +          NodePri(iDNod),NodeMgC(iDNod),' NodePri&MgC_rem'
        ...
        if(nZPNodesRem==nZPNodesOrg) then 
          exit ! no progress in reducing # of 0-priced nodes
        endif
...
!       LinkMgC(iLink)=(NodeMgC(iNode)+LinkMgC(iLink)/1000.)/
!    +    LinkEff(iLink) ! formula used in DerivePriceFromSources() for SourMgCst
!       LinkAvC(iLink)=(NodePri(iNode)+LinkAvC(iLink)/1000.)/
!    +    LinkEff(iLink) ! same form as above
...
!         write(DbgU,'(2i2,f4.1,-6p,4f11.4)') j,k,r4,r8,
!    +      SpyQCEF(j,k),LnkQCEF(j,k),DmdQCEF(j,k)
        end do
...
!-----
    ! NEED to incorporate LinkExpLim below
     ...
     +      cVect(jVar-nSegUsd+iSeg)
          if(ThisQtySeg(iSeg)<Near0Qty) then 
            cycle
          endif
          if(iSeg==nSegUsd) then
...
!         if(ExpansionMultiple==1.0) write(DbgU,'(1x)')
     ...
     +      ExpansionMultiple,' EEM'
          ...
          if(ExpDollarGain<0.0) then 
            exit ! expansion is not economic
          endif
...
!     write(*,'(i3,2(1x,a))') LengNB(FileName),trim(FileName),FileName
      do j=255,1,-1 ! blank any control characters, since stack-var's tail ...
        iCh=iChar(FileName(j:j))
        if((32<iCh).and.(iCh<123)) then 
          exit ! accept ASCII chars on [33,122]
        endif
        FileName(j:j)=' ' ! ... from concatenation may contain garbage bytes
      end do
!     write(*,'(i5,3(i3,1x,a),a)') iUnit,LengNB(FileName),FileName,
!    +  iFS,FSType(iFS),iCC,CCType(iCC),' before open()'
...
!       if(iChar(s(i:i))>32) exit ! len(s)=16384 caused 'program stack exhausted'
...
!
  ...
  101   write(OutUnit,'(3i5,a,i3.3)')jOfs,jSupFoI,j,
     +    ' Ofs,Sup,Max in Enum with #Truncated=',nTrunc
        if(j>0) then 
          exit ! with the lowest feasible nTrunc (trailing fields missing)
        endif
        if(jLimit<=0) then 
          exit
        endif
...
!     jLimit=j-1 ! if the above loop exited with error or EoF
