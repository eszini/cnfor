...
! CLREPORT
     ...
     +         CL_UNIT_MONTHLY_FIXED_COST)
      ...
      use derivatives_report_handler
      use monthly_trans_report_handler
      use transact_fuel_report_handler
      use contract_handler
      use block_summary_items_handler
      use monthly_pw_report_handler
      use annual_trans_report_handler
      use fiscal_reporting_season_handler
...
!
     ...
     +               FISCAL_UNIT_START_COSTS
      ...
      INTEGER (kind=2) ::  TRANS_PROD_VARIABLES    ! had a _ instead of a +
...
!
!
! 040905. NUNITS MAY BE TOO SHORT IF WE INNCLUDE DERIVATIVES
!
...
! 102409.
!
! TODO: EXTRACT_METHOD prepare_icap_summary_report (done)
      CALL prepare_icap_summary_report(ICAP_TOTAL_CAPACITY,
     ...
     +ANNUAL_ICAP_DEMAND_POINT,ANNUAL_ICAP_VALUE_POINT,
     +ANNUAL_ICAP_MRX_CONE,ANNUAL_ICAP_MRX_EAS,ANNUAL_ICAP_VAR_COST,
     +ANNUAL_ICAP_FUEL_COST,ANNUAL_ICAP_START_COST,ANNUAL_ICAP_VOM_COST,
     +ANNUAL_ICAP_EMISSION_COST,ANNUAL_ICAP_EAS_REVENUE_OFFSET,
     +MONTHLY_ICAP_VALUE,ANNUAL_ICAP_TOTAL_COST,ANNUAL_ICAP_NET_MARGIN,
...
! TODO: EXTRACT_METHOD collect_nox_data (done)
      CALL nox_data_collector(I,UNIT_EMISSIONS, 
     +      ANNUAL_NOX_SEASON_EMISSIONS,ANNUAL_NOX_SEASON_THERMAL,HEAT, 
     +      FISCAL_NOX_SEASON_EMISSIONS,FISCAL_NOX_SEASON_THERMAL,
     +      TOTAL_NOX_HEAT)
...
!
     ...
     +                              VCPMWH(UNITNO) *
     +                          MON_ECO_SALES_ENRG_FROM(UNITNO)/1000000.
! TODO: EXTRACT_METHOD collect_fiscal_data (Done)
          CALL collect_fiscal_data(I,UNITNO,FISCAL_ECO_SALES_REV_FROM,
     +         FISCAL_ECO_SALES_ENRG_FROM,FISCAL_ECO_PUCH_COST_FROM, 
     +         FISCAL_ECO_PUCH_ENRG_FROM,FISCAL_P_FUEL_CONSUMPTION,
     +         FISCAL_S_FUEL_CONSUMPTION,FISCAL_E_FUEL_CONSUMPTION,
     +         P_FUEL_CONSUMPTION,FISCAL_UNIT_STARTS,
     +         FISCAL_UNIT_START_COSTS,MONTH_UNIT_STARTS,
     +         MONTH_UNIT_START_COSTS,FISCAL_WHOLESALE_PROD_COST,
     +         WHOLESALE_PRODUCTION_COST,FISCAL_UNIT_EMISSIONS_COST,
     +         UNIT_EMISSIONS_COST)

!
! TODO: EXTRACT_METHOD collect_fiscal_data (end)

! TODO: EXTRACT_METHOD collect_emissions_costs (done)
      CALL collect_emissions_costs(EMISSIONS_COST_BY_SP,
     +                             UNIT_EMISSIONS_COST,
     +                             SP)

! TODO: EXTRACT_METHOD collect_emissions_costs (end)

! TODO: EXTRACT_METHOD collect_group_data (Done)

      CALL collect_group_data(I,SP,CAPBLK,ENRG,HEAT,UNITNO,FUEL_COST,
     +                         GROUP_VAR_BY_SP,ANNUAL_GROUP_VAR_BY_SP, 
     +                         CL_UNIT_MONTHLY_FIXED_COST,
     +                         UNIT_EMISSIONS,TOTAL_UNIT_COST) 
! TODO: EXTRACT_METHOD collect_group_data (end)
...
! TODO: EXTRACT_METHOD handle_monthly_pw_report
                call handle_monthly_pw_report(MONTHLY_PW_REPORT,
     + MONTHLY_NEW_PW_REPORT,UNITNO,CL_UNITS_B4_ADDITIONS,
     + UNIT_START_UP_COSTS,ISEAS,TEMP_L1,
     + EIA_PLANT_CODE,
     + RDI_MARKET_AREA_NAME,RMK,
     + EV_TRANS_AREA_NAME,TG_POSITION,A_CO,
     + B_CO,C_CO,D_CO,MIN_UPTIME,MIN_DOWNTIME,
     + SAC_UNIT_NAME,CL_TRANS_NAME,SAC_STATION_GROUP,
     + LOCAL_MW1,I,LOCAL_MW2,FIXED_COST_PER_UNIT,
     + CL_UNIT_MONTHLY_FIXED_COST,
     + MONTHLY_MOR,CAPBLK,MONTHLY_FOR,TRANS_EQUIV_AVAIL,
     + HEAT,DISP_COST_1,
     + DISP_COST_2,PW_AVE_FUEL_COST,AVERAGE_FUEL_COST,AVE_FUEL_MULT,
     + SAC_ONLINE_MO,ONLINE_DATE,LAST_RESOURCE_UPDATE_NO,
     + INSERVICE_STATUS,
     + SAC_ONLINE_DATE,PW_REC,LOCAL_YEAR,CL_MONTH_NAME,
     + MULTI_AREA_NAME,RTG,AVERAGE_NOX_RATE,AVERAGE_SOX_RATE,
     + EMIS_DISPATCH_2,EMIS_DISPATCH_1,EMIS_DISPATCH_3,AVERAGE_CO2_RATE,
     + EMIS_DISPATCH_4,AVERAGE_HG_RATE,SAC_DATE,
     + RFT)
...
!
           call handle_derivatives_report_iseas(DERIVATIVES_REPORT,
     + ISEAS,CHARGING_ENERGY,RSP,RPM,TRANS_OFF_LINE,
     + TRANS_ON_LINE,NEWGEN_UNIT_STATUS,TRANS_ASSET_CLASS,
     + TOTAL_EMISSION_COSTS,POWERDAT_PLANT_ID,TRANS_UNIT_ID,RFT,RMK,RTG,
     + RETAIL_SALES,WHOLESALE_PRODUCTION_COST,UNIT_EMISSIONS_COST,
     + TRANS_REV_MWH,TRANS_REV,TOTAL_UNIT_COST,TRANS_FIX_EXP,
     + UNIT_EMISSIONS,TRANS_VAR_MWH,TRANS_VAR_EXP,TRANS_STRIKES,ENRG,
     + TRANS_CAP_FACTOR,TRANS_HOURS,PRODUCT_HOURS,TRANS_CAP,
     + I,CL_TRANS_NAME,CL_MONTH_NAME,LOCAL_YEAR)
            ENDIF
         ENDDO ! TRANSACTIONS
!
...
! TODO: EXTRACT_METHOD handle_market_data (end)
         NEWGEN_UNIT_STATUS = 0.
!
! TODO: EXTRACT_METHOD handle_monthly_trans_report (done)
       call handle_monthly_trans_report(MONTHLY_UNIT_EMISSIONS_COST,
     + TOTAL_EMISSION_COSTS_BY_TYPE,MONTHLY_TRANS_CAPACITY,
     + TRANS_EQUIV_AVAIL,
     + TRANS_CAP_FACTOR,ISEAS,MONTHLY_CHARGING_ENERGY,MONTHLY_EBITDA,
     + MONTHLY_CAPITAL_COST_OF_BUILD,MONTHLY_CAPACITY_REVENUE,
     + NEWGEN_UNIT_STATUS,MONTHLY_START_COSTS,MONTHLY_STARTS,
     + AVERAGE_VAR_OM,
     + AVERAGE_FUEL_COST,AVERAGE_SOX_RATE,MONTHLY_E_HEAT,MONTHLY_S_HEAT,
     + MONTHLY_P_HEAT,AVE_REV_FROM_SALES,TOTAL_UNIT_COST,
     + MONTHLY_TRANS_FIXED_COST,MONTHLY_UNIT_EMISSIONS,
     + AVERAGE_PRODUCTION_COSTS,MONTHLY_TRANS_VAR_COST,
     + MONTHLY_TRANS_FUEL_COST,
     + AVERAGE_HEATRATE,MONTHLY_TRANS_HEAT,MONTHLY_TRANS_ENERGY,
     + MONTHLY_TRANS_EFFECTIVE_CAP,MONTHLY_TRANS_REPORT,I,CL_TRANS_NAME,
     + CL_MONTH_NAME,LOCAL_YEAR,AVERAGE_NOX_RATE)

! TODO: EXTRACT_METHOD handle_monthly_trans_report (end)

! TODO: EXTRACT_METHOD handle_transact_fuel_report
       CALL handle_transact_fuel_report(TRANSACT_FUEL_REPORT_ACTIVE,
     + I,FUEL_COST_PER_MMBTU,J,FUEL_BTUS_BY_TG_BY_FUEL,
     + FUEL_COST_BY_TG_BY_FUEL,TRANS_FUEL_REC,CL_MONTH_NAME,
     + ISEAS,MULTI_AREA_NAME,RESOURCE_RPS_VARS,
     + FUEL_BTUS_BY_SP_BY_FUEL,FUEL_COST_BY_SP_BY_FUEL,
     + GROUP_VAR_BY_SP,UNIT_EMISSIONS,EMISSION_COST_PER_TON,
     + EMISSIONS_COST_BY_SP,ANNUAL_EMISSIONS_COST_BY_SP,TEMP_I2,
     + STATE_PROVINCE,ST_TG,
     + STATE_RPS_VARS,TEMP_L1,
     + TEMP_NAME,TRANS_STATE_REC)
...
!
!
         call handle_fiscal_reporting_season(YES_FISCAL_REPORTING,
     + ISEAS,FISCAL_SEASON,MONTHLY_STARTS,MONTHLY_START_COSTS,
     + MONTHLY_CAPITAL_COST_OF_BUILD,MONTHLY_EBITDA,
     + MONTHLY_CHARGING_ENERGY,MONTHLY_CAPACITY_REVENUE,
     + MONTHLY_NOX_SEASON_THERM,MONTHLY_TRANS_CAPACITY,
     + MONTHLY_TRANS_EFFECTIVE_CAP,MONTHLY_TRANS_AVAIL_CAP,
     + MONTHLY_TRANS_ENERGY,MONTHLY_P_HEAT,MONTHLY_S_HEAT,
     + MONTHLY_E_HEAT,MONTHLY_TRANS_HEAT,MONTHLY_TRANS_FUEL_COST,
     + MONTHLY_TRANS_VAR_COST,MONTHLY_UNIT_EMISSIONS,
     + MONTHLY_TRANS_FIXED_COST,I,UNITNO,CL_TRANS_NAME,
     + CL_UNIQUE_RPT_STR,ENRG,FISCAL_CL_UNIT_ENERGY,
     + FISCAL_REV_GEN_CAPACITY,CAPBLK,FISCAL_EFFECTIVE_CAP,
     + FISCAL_MONTHS_ACTIVE,FISCAL_CAP,CLASS,P_FUEL_DELIVERY,
     + PBTUCT_SAVE,HEAT,FISCAL_CL_UNIT_MMBTUS,FUEL_COST,
     + FISCAL_CL_UNIT_FUEL_COST,UNIT_EMISSIONS,
     + FISCAL_CL_UNIT_EMISSIONS,UNIT_EMISSIONS_COST,
     + FISCAL_UNIT_EMISSIONS_COST,CANADA,UNIT_NAME,
     + AVERAGE_HEATRATE,HEAT_CONVERSION,AVERAGE_PRODUCTION_COSTS,
     + TRANS_EQUIV_AVAIL,TRANS_CAP_FACTOR,TOTAL_UNIT_COST,
     + FISCAL_CL_UNIT_FIXED_COST,FISCAL_ECO_SALES_ENRG_FROM,
     + AVE_REV_FROM_SALES,FISCAL_ECO_SALES_REV_FROM,
     + FISCAL_NOX_SEASON_THERMAL,FISCAL_ECO_PUCH_COST_FROM,
     + FISCAL_ECO_PUCH_ENRG_FROM,FISCAL_P_FUEL_CONSUMPTION,
     + FISCAL_S_FUEL_CONSUMPTION,FISCAL_E_FUEL_CONSUMPTION,
     + FISCAL_CL_UNIT_VAR_COST,MONTHLY_UNIT_EMISSIONS_COST,
     + FT,SP,TG,TEMP_L1,MARKET_ID,MK,PM,RTG,RMK,RFT,RSP,
     + CHARGING_ENERGY,RPM,HESI_ID_NUM_4_UNIT,
     + POWERDAT_PLANT_ID,NEWGEN_UNIT_STATUS,AVERAGE_SOX_RATE,
     + TONS_CONVERSION,AVERAGE_FUEL_COST,AVERAGE_VAR_OM,
     + AVERAGE_NOX_RATE,UNIT_GROSS_MARGIN,RETAIL_SALES,
     + FISCAL_WHOLESALE_PROD_COST,TRANS_GROUP_REPORTING_ACTIVE,
     + MONTHLY_TRANS_REPORT,ANNUAL_TRANS_REPORT,
     + TOTAL_EMISSION_COSTS,UNECONOMIC_RATE,MON_CL_TRANS_UNIT_REC,
     + MON_CL_TRANS_UNIT_NO,LOCAL_YEAR,CL_MONTH_NAME,
     + HESI_SECOND_UNIT_ID_NUM,EM,FISCAL_EBITDA,
     + FISCAL_CAPITAL_COST_OF_BUILD,FISCAL_UNIT_START_COSTS,
     + FISCAL_UNIT_STARTS,SEAS_HOURS)
!
! ANNUAL TRANS REPORT
!
!
! TODO: EXTRACT_METHOD HANDLE_ANNUAL_TRANS_REPORT
       call handle_annual_trans_report(ISEAS,
     + TRANSACT_FUEL_REPORT_ACTIVE,I,FUEL_COST_PER_MMBTU,J,
     + ANNUAL_FUEL_BTUS_BY_TG_BY_FUEL,ANNUAL_FUEL_COST_BY_TG_BY_FUEL,
     + TRANS_FUEL_REC,CL_MONTH_NAME,MULTI_AREA_NAME,
     + RESOURCE_RPS_VARS,ANNUAL_FUEL_BTUS_BY_SP_BY_FUEL,
     + ANNUAL_FUEL_COST_BY_SP_BY_FUEL,ANNUAL_GROUP_VAR_BY_SP,
     + UNIT_EMISSIONS,GROUP_VAR_BY_SP,EMISSION_COST_PER_TON,
     + ANNUAL_EMISSIONS_COST_BY_SP,TEMP_I2,STATE_PROVINCE,
     + TEMP_NAME,ST_TG,STATE_RPS_VARS,
     + TEMP_L1,TRANS_STATE_REC,MAX_LOAD_TYPES,TRAN_PROD_FUEL_REC,
     + MFT1,MONTHLY_STARTS,MONTHLY_START_COSTS,
     + MONTHLY_CAPITAL_COST_OF_BUILD,MONTHLY_EBITDA,
     + MONTHLY_CHARGING_ENERGY,MONTHLY_CAPACITY_REVENUE,
     + MONTHLY_NOX_SEASON_THERM,MONTHLY_TRANS_CAPACITY,
     + MONTHLY_TRANS_EFFECTIVE_CAP,MONTHLY_TRANS_AVAIL_CAP,
     ...
     + MONTHLY_E_HEAT,MONTHLY_TRANS_HEAT,MONTHLY_TRANS_FUEL_COST,
     + MONTHLY_TRANS_VAR_COST,MONTHLY_UNIT_EMISSIONS,
     + ANNUAL_LOCAL_EMIS_COST,MONTHLY_TRANS_FIXED_COST,
     + UNIT_GROSS_MARGIN,UNITNO,
     + CL_TRANS_NAME,ENRG,ANNUAL_REV_GEN_CAPACITY,CAPBLK,
     + ANNUAL_EFFECTIVE_CAP,MONTHS_ACTIVE,ANNUAL_CAP,
     + P_FUEL_DELIVERY,PBTUCT_SAVE,HEAT,FUEL_COST,UNIT_EMISSIONS_COST,
     + ANNUAL_UNIT_EMISSIONS_COST,CANADA,UNIT_NAME,AVERAGE_HEATRATE,
     + HEAT_CONVERSION,AVERAGE_PRODUCTION_COSTS,TRANS_EQUIV_AVAIL,
     + TRANS_CAP_FACTOR,TOTAL_UNIT_COST,AVE_REV_FROM_SALES,
     + ANNUAL_NOX_SEASON_THERMAL,ANNUAL_UNIT_STARTS,
     + ANNUAL_UNIT_START_COSTS,ANNUAL_CAPITAL_COST_OF_BUILD,
     + ANNUAL_EBITDA,ANNUAL_UNIT_CAPACITY_REVENUE,
     + ANNUAL_P_FUEL_CONSUMPTION,ANNUAL_S_FUEL_CONSUMPTION,
     + ANNUAL_E_FUEL_CONSUMPTION,FT,SP,TG,
     + ONLINE_DATE,OFLINE_DATE,MARKET_ID,MK,MARKET_AREA_LOOKUP,
     + PM,RTG,RMK,RFT,RSP,CHARGING_ENERGY,
     + RPM,HESI_ID_NUM_4_UNIT,HESI_SECOND_UNIT_ID_NUM,
     + POWERDAT_PLANT_ID,NEWGEN_UNIT_STATUS,AVERAGE_SOX_RATE,
     + TONS_CONVERSION,AVERAGE_FUEL_COST,AVERAGE_VAR_OM,
     + AVERAGE_NOX_RATE,TEMP_R4,CO2_COST,CAP_COST,
     + RETAIL_SALES,ANNUAL_WHOLESALE_PROD_COST,
     + TRANS_GROUP_REPORTING_ACTIVE,
     + FISCAL_ONLY,MONTHLY_TRANS_REPORT,
     + ANNUAL_TRANS_REPORT,TOTAL_EMISSION_COSTS,UNECONOMIC_RATE,
     + AllHoursMonth,CapHours,EffectiveCapHours,AvailHoursMonth,
     + MON_CL_TRANS_UNIT_REC,MON_CL_TRANS_UNIT_NO,LOCAL_YEAR,
     + BEGIN_DATE,MON_NU_TRANS_UNIT_REC,MON_NU_TRANS_UNIT_NO,
     + SUM_ANNUAL,TRANS,VOID_LOGICAL,TRANS_CAP,TRANS_VAR_EXP,
     + TRANS_VAR_MWH,TRANS_FIX_EXP,TRANS_REV,TRANS_REV_MWH,
     + TRANS_HOURS,PRODUCT_HOURS,TRANS_STRIKES,TRANS_NAME,
     + YES_REPORT_PRODUCT,TRANS_ON_LINE,TRANS_OFF_LINE,
     + TRANS_UNIT_ID,TRANS_ASSET_CLASS,TRANS_NOT_ACTIVE,TEMP_YEAR,
     + TG_POSITION,WHOLESALE_PRODUCTION_COST,TransAllHoursMonth,
     + TransCapHours,TransEffectiveCapHours,TransAvailHoursMonth,
     + MONTHLY_UNIT_EMISSIONS_COST,CLASS,EM,SEAS_HOURS,
     + TRANS_TOTAL_GROSS_MARGIN,CL_UNIQUE_RPT_STR)
...
! TODO: EXTRACT_METHOD handle_contracts
       call handle_contracts(I,TOTAL_CONTRACT_COST,ENRG,SEAS_HOURS,
     + CONTRACT_ENERGY,CNTR_ENRG,CNTR_EMISSIONS,CNTR_MIN_FIXED_COST,
     + MINIMUM_CAPACITY,CONTRACT_CAPACITY,CNTR_MAX_FIXED_COST,KEPCO,
     + REALLY_KEPCO,MAXIMUM_CAPACITY,CONTRACT_BILLING_CAPACITY,
     + RATCHET_CAPACITY_BASIS,MIN_RATCHET_CAPACITY,
     + CAPACITY_BILLED_AT_MIN,
     + CAPACITY_BILLED_AT_MAX,MONTHLY_KEPCO_SALES,
     + MONTHLY_KEPCO_SALES_REV,ISEAS,PEAK_MONTH,MONTHLY_TOTAL_EMISSIONS,
     + MONTHLY_CONTRACT_REPORT_ACTIVE,
     + CONTRACTS_IN_PERIOD,UNIT_NAME,REPORTING_CAPACITY,DIVIDE_BY_ENRG,
     + AVERAGE_TOTAL_COST,MONTHLY_REPORTING_CAPACITY,
     + MONTHLY_CONTRACT_CAPACITY,MONTHLY_MINIMUM_CAPACITY,MONTHLY_ENRG,
     + MONTHLY_CONTRACT_VARIABLE_COST,MONTHLY_CNTR_MAX_FIXED_COST,
     + MONTHLY_CNTR_MIN_FIXED_COST,MONTHLY_TOTAL_FIXED_COST,
     + MONTHLY_TOTAL_CONTRACT_COST,MONTHLY_CNTR_EMISSIONS,
     + MON_CT_UNIT_REC,MON_CT_UNIT_NO,CL_MONTH_NAME,
     + MONTHLY_CONTRACT_AVE_VAR_COST,MONTHLY_AVERAGE_TOTAL_COST,
     + ANNUAL_REPORTING_CAPACITY,ANNUAL_CNTR_CAPACITY,
     + ANNUAL_MINIMUM_CAPACITY,ANNUAL_ENRG,ANNUAL_CNTR_VARIABLE_COST,
     + ANNUAL_CNTR_MAX_FIXED_COST,ANNUAL_CNTR_MIN_FIXED_COST,
     + ANNUAL_TOTAL_FIXED_COST,ANNUAL_TOTAL_CONTRACT_COST,
     + ANNUAL_CNTR_EMISSIONS)
     ...
     +                               CNTR_OFF_LI(I) < FIRST_MONTH) THEN 
              CYCLE
            ENDIF
...
! TODO: EXTRACT_METHOD handle_block_and_summary_report_items
      call handle_block_summary_report(MONTHLY_BLOCK_REPORT,
     + MONTHLY_SUMMARY_REPORT_ACTIVE,CONTRACTS_IN_PERIOD,
     + MONTHLY_CONTRACT_REPORT_ACTIVE,P_PUR_ENRG,SEAS_HOURS,
     + PVARCOST,P_SALES_ENERGY,TOTAL_PRODUCTION_COSTS,PFUELCST,
     + P_NUCLEAR_FUEL_COST,PFIXCOST,P_PUR_POWER_COST,
     + MONTHLY_ECONOMY_COST,MONTHLY_ECONOMY_REVENUE,P_SALES_REVENUE,
     + MONTHLY_HEAT,PMMBTUS,TEMP_SNGL,MON_CL_SUM_REC,MON_CL_SUM_NO,
     + CL_MONTH_NAME,ISEAS,DEMAND,MONTHLY_ECONOMY_SOLD,PENRG,
     + MONTHLY_ECONOMY_BOUGHT,PEAK,BASE,MONTHLY_TOTAL_EMISSIONS,I)
...
!
      ...
      use poolcom
      use clreport_routines
      use annual_output_report_preparer
!
...
! TODO: EXTRACT_METHOD prepare_annual_output_report
      call prepare_annual_output_report(ANNUAL_OUTPUT_REPORT,
     + ANN_CL_SUM_NO,ANN_CL_SUM_REC,SAVE_YEAR,CAP_LIMITED_DEMAND,YR,
     + TOTAL_PRODUCTION_COSTS,PEAK,BASE,ANNUAL_TOTAL_EMISSIONS,I,
     + TEMP_DBLE)
...
! MATCH UNIT NAME AND ZONE LOCATION
     ...
     +                          GET_GROUP_NAME(TG_POSITION)) == 0) THEN 
                          CYCLE
                        ENDIF
