...
!
      ...
      IF(ACCEPTANCE_ACTIVE(DSM_DEVICE_RECORDS,MAX_DSM_DEVICE_NUM)) THEN
         IF(.NOT. ALLOCATED(DEVICE_APPLICATION)) THEN
            allocate(DEVICE_APPLICATION(MAX_DSM_DEVICE_NUM),stat=stv_er)
            call check_alloc("dsm_objt:0001",
     +                                    "DEVICE_APPLICATION",stv_er)
            allocate(CUSTOMERS_BY_DAY_TYPE(3,DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0002",
     +                                 "CUSTOMERS_BY_DAY_TYPE",stv_er)
            allocate(APPLICATION_ID(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0003","APPLICATION_ID",stv_er)
            allocate(APPLICATION_CLASS(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0004","APPLICATION_CLASS",stv_er)
            allocate(OPERATING_METHOD(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0005","OPERATING_METHOD",stv_er)
            allocate(DSM_CAP_PLAN_FACTOR(DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0006",
     +                                   "DSM_CAP_PLAN_FACTOR",stv_er)
            allocate(DSM_ENERGY_CLASSIFICATION(DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0007",
     +                             "DSM_ENERGY_CLASSIFICATION",stv_er)
            allocate(FREE_RIDERS(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0008","FREE_RIDERS",stv_er)
            allocate(FREE_DRIVERS(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0009","FREE_DRIVERS",stv_er)
            allocate(PROGRAM_LOSS_FACTOR(DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0010",
     +                                   "PROGRAM_LOSS_FACTOR",stv_er)
            allocate(DSM_START_YEAR(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0011","DSM_START_YEAR",stv_er)
            allocate(DSM_DEVICE_NAME(DSM_DEVICE_RECORDS),stat=stv_er)
            call check_alloc("dsm_objt:0012","DSM_DEVICE_NAME",stv_er)
            allocate(RESPONSE_POINTER_FOR(3,DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0013",
     +                                  "RESPONSE_POINTER_FOR",stv_er)
            allocate(PROGRAM_DSM_ALLOCATION_METHOD(DSM_DEVICE_RECORDS),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0014",
     +                         "PROGRAM_DSM_ALLOCATION_METHOD",stv_er)
      
         ELSE
            DEALLOCATE(DEVICE_APPLICATION)
            ALLOCATE(DEVICE_APPLICATION(MAX_DSM_DEVICE_NUM),stat=stv_er)
            call check_alloc("dsm_objt:0015",
     +                                    "DEVICE_APPLICATION",stv_er)
...
!
      IF(RESPONSE_ACTIVE(DSM_RESPONSE_CURVES,MAX_RESPONSE_NUM)) THEN
         IF(.NOT. ALLOCATED(BASDEV)) THEN
            allocate(BASDEV(24,DSM_RESPONSE_CURVES),stat=stv_er)
            call check_alloc("dsm_objt:0016","BASDEV",stv_er)
            allocate(LMGDEV(24,DSM_RESPONSE_CURVES),stat=stv_er)
            call check_alloc("dsm_objt:0017","LMGDEV",stv_er)
            allocate(BEFORE_LM_ADJUSTMENT(DSM_RESPONSE_CURVES),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0018",
     +                                  "BEFORE_LM_ADJUSTMENT",stv_er)
            allocate(AFTER_LM_ADJUSTMENT(DSM_RESPONSE_CURVES),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0019",
     +                                   "AFTER_LM_ADJUSTMENT",stv_er)
            allocate(LM_LOAD_UNITS(DSM_RESPONSE_CURVES),stat=stv_er)
            call check_alloc("dsm_objt:0020","LM_LOAD_UNITS",stv_er)
            allocate(RESPONSE_LOCATION_POINTER(MAX_RESPONSE_NUM),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0021",
     +                             "RESPONSE_LOCATION_POINTER",stv_er)
            allocate(RESPONSE_DATA_LOCATION(12,MAX_RESPONSE_NUM),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0022",
     +                                "RESPONSE_DATA_LOCATION",stv_er)
            allocate(AFTER_LM_FACTOR(DSM_RESPONSE_CURVES),stat=stv_er)
            call check_alloc("dsm_objt:0023","AFTER_LM_FACTOR",stv_er)
            allocate(BEFORE_LM_FACTOR(DSM_RESPONSE_CURVES),stat=stv_er)
            call check_alloc("dsm_objt:0024","BEFORE_LM_FACTOR",stv_er)
     ...
     +                 RESPONSE_LOCATION_POINTER)
            allocate(RESPONSE_DATA_LOCATION(12,MAX_RESPONSE_NUM),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0025",
     +                                "RESPONSE_DATA_LOCATION",stv_er)
            allocate(RESPONSE_LOCATION_POINTER(MAX_RESPONSE_NUM),
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0026",
     +                             "RESPONSE_LOCATION_POINTER",stv_er)
...
!
         ...
         DO APPLICATION = 1, APPLICATION_NUM
            IF(DSM_START_YEAR(APPLICATION) > CURRENT_YEAR) THEN 
              CYCLE
            ENDIF
...
!
            IF(CUSCAL == 0. .OR. DEVICE_NUM == 0) THEN 
              CYCLE
            ENDIF
            LOCATION = RESPONSE_LOCATION_POINTER(DEVICE_NUM)
...
!
     ...
     +              LF_DELETE)
      ENDIF
      allocate(BASE_PROG_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0027","BASE_PROG_EXP_SAVE",stv_er)
      allocate(ONGO_PROG_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0028","ONGO_PROG_EXP_SAVE",stv_er)
      allocate(BASE_CUST_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0029","BASE_CUST_EXP_SAVE",stv_er)
      allocate(ONGO_CUST_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0030","ONGO_CUST_EXP_SAVE",stv_er)
      allocate(ONGO_NEW_CUST_EXP_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0031","ONGO_NEW_CUST_EXP_SAVE",stv_er)
      allocate(ONGO_KWH_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0032","ONGO_KWH_EXP_SAVE",stv_er)
      allocate(ONGO_KW_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0033","ONGO_KW_EXP_SAVE",stv_er)
      allocate(BASE_PROG_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0034","BASE_PROG_CAP_SAVE",stv_er)
      allocate(ONGO_PROG_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0035","ONGO_PROG_CAP_SAVE",stv_er)
      allocate(BASE_CUST_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0036","BASE_CUST_CAP_SAVE",stv_er)
      allocate(ONGO_CUST_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0037","ONGO_CUST_CAP_SAVE",stv_er)
      allocate(ONGO_NEW_CUST_CAP_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0038","ONGO_NEW_CUST_CAP_SAVE",stv_er)
      allocate(ONGO_KWH_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0039","ONGO_KWH_CAP_SAVE",stv_er)
      allocate(ONGO_KW_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0040","ONGO_KW_CAP_SAVE",stv_er)
      allocate(REBATE_CUST_EXP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0041","REBATE_CUST_EXP_SAVE",stv_er)
      allocate(REBATE_NEW_CUST_EXP_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0042",
     +                              "REBATE_NEW_CUST_EXP_SAVE",stv_er)
      allocate(REBATE_CUST_CAP_SAVE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0043","REBATE_CUST_CAP_SAVE",stv_er)
      allocate(REBATE_NEW_CUST_CAP_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0044",
     +                              "REBATE_NEW_CUST_CAP_SAVE",stv_er)
      allocate(PARTICIPANT_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0045",
     +                            "PARTICIPANT_CUST_COST_SAVE",stv_er)
      allocate(PARTICIPANT_NEW_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0046",
     +                        "PARTICIPANT_NEW_CUST_COST_SAVE",stv_er)
      allocate(PARTICIPANT_COSTS_BY(APPLICATION_NUM),stat=stv_er)
      call check_alloc("dsm_objt:0047","PARTICIPANT_COSTS_BY",stv_er)
      allocate(UTIL_NON_ELEC_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0048",
     +                          "UTIL_NON_ELEC_CUST_COST_SAVE",stv_er)
      allocate(UTIL_NON_ELEC_NEW_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0049",
     +                           "UTIL_NON_ELEC_NEW_COST_SAVE",stv_er)
      allocate(THIRD_PARTY_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0050",
     +                            "THIRD_PARTY_CUST_COST_SAVE",stv_er)
      allocate(THIRD_PARTY_NEW_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0051",
     +                        "THIRD_PARTY_NEW_CUST_COST_SAVE",stv_er)
      allocate(OTH_PARTICIPANT_CUST_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0052",
     +                        "OTH_PARTICIPANT_CUST_COST_SAVE",stv_er)
      allocate(OTH_PARTICIPANT_NEW_COST_SAVE(DSM_FINANCIAL_RECORDS),
     +                                                    stat=stv_er)
      call check_alloc("dsm_objt:0053",
     +                         "OTH_PARTICIPANT_NEW_COST_SAVE",stv_er)
      allocate(DSM_ASSET_CLASS_POINTER(1024),stat=stv_er)
      call check_alloc("dsm_objt:0054","DSM_ASSET_CLASS_POINTER",stv_er)
      allocate(LF_DELETE(DSM_FINANCIAL_RECORDS),stat=stv_er)
      call check_alloc("dsm_objt:0055","LF_DELETE",stv_er)
...
!
         IF(IOS /= 0) THEN 
           EXIT
         ENDIF
         IF(LF_DELETE(DEVICE_NUM) > 7) THEN 
           CYCLE
         ENDIF
!
...
!     ******************************************************************
     ...
     +                                                    stat=stv_er)
            call check_alloc("dsm_objt:0056",
     +                                 "MONTHLY_DSM_DATA_BASE",stv_er)
...
!     ******************************************************************
         ...
         YR = YEAR + BASE_YEAR
         DO APPLICATION = 1 , APPLICATION_NUM
            IF(DSM_START_YEAR(APPLICATION) > YR) THEN 
              CYCLE
            ENDIF
            DO DAY = 1 , 3
...
!     ******************************************************************
     ...
     +                      PROGRAM_DSM_ALLOCATION_METHOD(APPLICATION))
            IF(ALLOC > 1 .OR.
     +                 R_CLASS /= APPLICATION_CLASS(APPLICATION)) THEN 
              CYCLE
            ENDIF
...
!
         ...
         DO APPLICATION = 1 , APPLICATION_NUM
            IF(DSM_DEVICE_PEAK(YR,APPLICATION) == 0.0) THEN 
              CYCLE
            ENDIF
            CLASS = APPLICATION_CLASS(APPLICATION)
            IF(CLASS > SYSTEM_CLASS_NUM .OR. CLASS < 1) THEN 
              CYCLE
            ENDIF
...
!
     ...
     +                                                 DEVICE_AVE_ENERGY
!
               IF(PROGRAM_DSM_ALLOCATION_METHOD(APPLICATION)/='L') THEN 
                 CYCLE
               ENDIF
               DEVICE_AVE_SALES = DEVICE_AVE_ENERGY *
     +                           (1. - PROGRAM_LOSS_FACTOR(APPLICATION))
...
!
      DO DSM_ACCTS = 1, MAX_DSM_FINANCIAL_RECORDS
         IF(.NOT. DSM_FINANCIAL_ACTIVE(DSM_ACCTS)) THEN 
           CYCLE  ! GOTO 100
         ENDIF
!
! SET VALUES TO ZERO
...
!           CALL GET_DSM_START_YEAR(ACCEPTANCE_NUM,DSM_START_YEAR)
            ...
            IF(DSM_YEAR < BEGIN_APP_YEAR) THEN 
              CYCLE
            ENDIF
!
!
...
!
     ...
     +                                   DEALLOCATE(ASSET_CLASS_POINTER)
            ALLOCATE(ASSET_CLASS_POINTER(MAX_DSM_CLASS_NUM),stat=stv_er)
            call check_alloc("dsm_objt:0057",
...
!
     ...
     +                                RB_DSM_AFUDC,
     +                                RB_DSM_TAX_EXPENSE)
         allocate(DSM_CLASS_EXPENSES(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0058","DSM_CLASS_EXPENSES",stv_er)
         allocate(DSM_CLASS_REBATES(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0059","DSM_CLASS_REBATES",stv_er)
         allocate(DSM_CLASS_PURCHASE_POWER(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0060",
     +                              "DSM_CLASS_PURCHASE_POWER",stv_er)
         allocate(DSM_CLASS_SALES_REVENUE(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0061",
     +                               "DSM_CLASS_SALES_REVENUE",stv_er)
         allocate(DSM_BTL_EXPENSE(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0062","DSM_BTL_EXPENSE",stv_er)
         allocate(DSM_BTL_REVENUE(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0063","DSM_BTL_REVENUE",stv_er)
         allocate(DSM_ADJ_CLAUSE_COLLECTIONS(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0064",
     +                            "DSM_ADJ_CLAUSE_COLLECTIONS",stv_er)
         allocate(DSM_CLASS_CAPITIAL(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0065","DSM_CLASS_CAPITIAL",stv_er)
         allocate(DSM_CLASS_CAPACITY(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0066","DSM_CLASS_CAPACITY",stv_er)
         allocate(DSM_CLASS_ENERGY(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0067","DSM_CLASS_ENERGY",stv_er)
         allocate(DSM_CLASS_PARTICIPANT_COSTS(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0068",
     +                           "DSM_CLASS_PARTICIPANT_COSTS",stv_er)
         allocate(DSM_CLASS_UTIL_NON_ELEC_COSTS(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0069",
     +                         "DSM_CLASS_UTIL_NON_ELEC_COSTS",stv_er)
         allocate(DSM_CLASS_THIRD_PARTY_COSTS(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0070",
     +                           "DSM_CLASS_THIRD_PARTY_COSTS",stv_er)
         allocate(DSM_CLASS_OTH_PARTICIPANT_COSTS(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0071",
     +                       "DSM_CLASS_OTH_PARTICIPANT_COSTS",stv_er)
         allocate(DSM_TAX_EXPENSE(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0072","DSM_TAX_EXPENSE",stv_er)
         allocate(DSM_AFUDC(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0073","DSM_AFUDC",stv_er)
         allocate(RB_DSM_AFUDC(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0074","RB_DSM_AFUDC",stv_er)
         allocate(RB_DSM_TAX_EXPENSE(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0075","RB_DSM_TAX_EXPENSE",stv_er)
...
!
     ...
     +                DEALLOCATE(ASSET_CLASS_LIST,ASSET_ALLOCATION_LIST)
         ...
         call check_alloc("dsm_objt:0076","ASSET_CLASS_LIST",stv_er)
         allocate(ASSET_ALLOCATION_LIST(AVAIL_DATA_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0077",
...
!
     ...
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0078","DSM_BOOK_DEP",stv_er)
         allocate(DSM_CUM_BOOK_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0079","DSM_CUM_BOOK_DEP",stv_er)
         allocate(DSM_GPV(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0080","DSM_GPV",stv_er)
         allocate(DSM_TAX_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0081","DSM_TAX_DEP",stv_er)
         allocate(DSM_AMORT(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0082","DSM_AMORT",stv_er)
         allocate(DSM_DEFERRED_DEBIT(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0083","DSM_DEFERRED_DEBIT",stv_er)
         allocate(DSM_DEFERRED_TAX_BASIS(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0084",
     +                                "DSM_DEFERRED_TAX_BASIS",stv_er)
         allocate(DSM_TAX_PREF_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0085","DSM_TAX_PREF_DEP",stv_er)
         allocate(RB_DSM_AMORT(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0086","RB_DSM_AMORT",stv_er)
         allocate(RB_DSM_DEFERRED_TAX_BASIS(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0087",
     +                             "RB_DSM_DEFERRED_TAX_BASIS",stv_er)
         allocate(DSM_REGULATED_BOOK_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0088",
     +                                "DSM_REGULATED_BOOK_DEP",stv_er)
         allocate(DSM_REGULATED_TAX_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0089",
     +                                 "DSM_REGULATED_TAX_DEP",stv_er)
         allocate(DSM_REGULATED_DEFERRED_DEBIT(0:NUM_OF_DSM_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("dsm_objt:0090",
     +                          "DSM_REGULATED_DEFERRED_DEBIT",stv_er)
         allocate(DSM_REGULATED_NPV(0:NUM_OF_DSM_CLASSES),stat=stv_er)
         call check_alloc("dsm_objt:0091","DSM_REGULATED_NPV",stv_er)
         allocate(DSM_GPV_PROPERTY_TAX(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0092","DSM_GPV_PROPERTY_TAX",stv_er)
         allocate(DSM_NPV_PROPERTY_TAX(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0093","DSM_NPV_PROPERTY_TAX",stv_er)
         allocate(DSM_SL_TAX_DEP(0:NUM_OF_DSM_CLASSES,
     +                    MAX_FINANCIAL_SIMULATION_YEARS),stat=stv_er)
         call check_alloc("dsm_objt:0094","DSM_SL_TAX_DEP",stv_er)
