...
!
     ...
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0001","TG_2_PLANNING_AREA",stv_er)
     ...
     +                              0:MAX_TRANS_GROUPS),stat=stv_er) 
      call check_alloc("cla_objt:0002",
     +                             "DAY_TYPE_TRANS_GROUP_PAIR",stv_er)
     
      ALLOCATE(DATA_BASE_POSITION(2*MAX_DAY_TYPES,MAX_CL_UNITS),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0003","DATA_BASE_POSITION",stv_er)
     
      IF(ALLOCATED(MAINT_DAYS)) DEALLOCATE(MAINT_DAYS)
      ALLOCATE(MAINT_DAYS(MAX_CL_UNITS),stat=stv_er)
      call check_alloc("cla_objt:0004","MAINT_DAYS",stv_er)
     ...
     +                                     CL_TG_CAP_MARKET_REV)
      ALLOCATE(CL_ANN_CAP(3,STUDY_PERIOD,2),stat=stv_er)
      call check_alloc("cla_objt:0005","CL_ANN_CAP",stv_er)
     
      ALLOCATE(CL_TG_CAP(0:6,0:MAX(1,UPPER_TRANS_GROUP),STUDY_PERIOD,2),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0006","CL_TG_CAP",stv_er)
     
      ALLOCATE(NEWGEN_CAP_BY_INDEX(MAX_NEWGEN_INDEX,
     +                                   0:MAX(1,UPPER_TRANS_GROUP),
     +                                    STUDY_PERIOD),stat=stv_er) 
      call check_alloc("cla_objt:0007","NEWGEN_CAP_BY_INDEX",stv_er)
     
      ALLOCATE(CL_TG_AFTER_PEAK(0:UPPER_TRANS_GROUP,STUDY_PERIOD),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0008","CL_TG_AFTER_PEAK",stv_er)
     
      allocate(CL_TG_CAP_MARKET_MW(0:UPPER_TRANS_GROUP),stat=stv_er)
      call check_alloc("cla_objt:0009","CL_TG_CAP_MARKET_MW",stv_er)
      allocate(CL_TG_CAP_MARKET_REV(0:UPPER_TRANS_GROUP),stat=stv_er)
      call check_alloc("cla_objt:0010","CL_TG_CAP_MARKET_REV",stv_er)
      
      ALLOCATE(CL_TG_RETIRE(0:UPPER_TRANS_GROUP,STUDY_PERIOD),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0011","CL_TG_RETIRE",stv_er)
     
      IF(ALLOCATED(CL_ANNUAL_LOAD_REDUCTION))
     +         DEALLOCATE(CL_ANNUAL_LOAD_REDUCTION)
      ALLOCATE(CL_ANNUAL_LOAD_REDUCTION(STUDY_PERIOD),stat=stv_er)
      call check_alloc("cla_objt:0012",
     +                              "CL_ANNUAL_LOAD_REDUCTION",stv_er)
...
!           ! comment. It should be a function call.
            call check_for_duplicate_name(UNIT_NAME,TEMP_UNIT_NAME,
     +      UNIT_NAME_COUNT)
...
! TO GENERATE MONTHLY CAPACITIES
     ...
     +                 RETIRE_RETRO_POSITION,
     +                 RETIRE_RETRO_ABATE_INDEX)
      allocate(RETIRE_RETRO_CO2_PRICE(RETIRE_RETRO_COUNTER),stat=stv_er)
      call check_alloc("cla_objt:0013","RETIRE_RETRO_CO2_PRICE",stv_er)
      allocate(RETIRE_RETRO_UPLIFT_PER_TON(RETIRE_RETRO_COUNTER),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0014",
     +                           "RETIRE_RETRO_UPLIFT_PER_TON",stv_er)
      allocate(RETIRE_RETRO_CUM_CO2(0:RETIRE_RETRO_COUNTER),stat=stv_er)
      call check_alloc("cla_objt:0015","RETIRE_RETRO_CUM_CO2",stv_er)
      allocate(NEW_UNIT_RETIRE_VALUE_BY_CM(UPPER_TRANS_GROUP),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0016",
     +                           "NEW_UNIT_RETIRE_VALUE_BY_CM",stv_er)
      allocate(RETIRE_RETRO_INDEX(RETIRE_RETRO_COUNTER),stat=stv_er)
      call check_alloc("cla_objt:0017","RETIRE_RETRO_INDEX",stv_er)
      allocate(RETIRE_OR_RETRO(RETIRE_RETRO_COUNTER),stat=stv_er)
      call check_alloc("cla_objt:0018","RETIRE_OR_RETRO",stv_er)
      allocate(RETIRE_RETRO_POSITION(RETIRE_RETRO_COUNTER),stat=stv_er)
      call check_alloc("cla_objt:0019","RETIRE_RETRO_POSITION",stv_er)
      allocate(RETIRE_RETRO_ABATE_INDEX(RETIRE_RETRO_COUNTER),
     +                                                    stat=stv_er)
      call check_alloc("cla_objt:0020",
     +                              "RETIRE_RETRO_ABATE_INDEX",stv_er)
...
! FIND THE BLKNO
            DO I = 1, NBLOK
               IF(UNIT(I) /= R_NUNITS) THEN 
                 CYCLE
               ENDIF
               IF(BLKNO(I) > 1) THEN 
                 CYCLE ! THIS SHOULD HAPPEN.
               ENDIF
!
               TEMP_R4 = GET_VAR(
...
! 021910 CHANGED TO GET CORRECT RETURN
         GET_I8_ID_TO_UNIT = 0
         DO I = 1, NUNITS
            IF(HESI_UNIT_ID_NUM(I) /= R_I8_ID) THEN 
              CYCLE
            ENDIF
            GET_I8_ID_TO_UNIT = I
            EXIT
...
!***********************************************************************
     ...
     +                      MRX_RPS_DV_UNIT_INDEX)
         allocate(MRX_RPS_CL_CURVE_MWH(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0021","MRX_RPS_CL_CURVE_MWH",stv_er)
         allocate(MRX_RPS_CL_CURVE_CUM_MWH(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0022",
     +                              "MRX_RPS_CL_CURVE_CUM_MWH",stv_er)
         allocate(MRX_RPS_CL_CURVE_MARGIN(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0023",
     +                               "MRX_RPS_CL_CURVE_MARGIN",stv_er)
         allocate(MRX_RPS_CL_CURVE_CUM_MARGIN(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0024",
     +                           "MRX_RPS_CL_CURVE_CUM_MARGIN",stv_er)
         allocate(MRX_RPS_CL_CURVE_INDEX(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0025",
     +                                "MRX_RPS_CL_CURVE_INDEX",stv_er)
         allocate(MRX_RPS_CL_UNIT_NO(PG),stat=stv_er)
         call check_alloc("cla_objt:0026","MRX_RPS_CL_UNIT_NO",stv_er)
         allocate(MRX_RPS_CL_UNIT_INDEX(NUNITS,PG),stat=stv_er)
         call check_alloc("cla_objt:0027",
     +                                 "MRX_RPS_CL_UNIT_INDEX",stv_er)
         allocate(MRX_RPS_DV_CURVE_MWH(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0028","MRX_RPS_DV_CURVE_MWH",stv_er)
         allocate(MRX_RPS_DV_CURVE_CUM_MWH(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0029",
     +                              "MRX_RPS_DV_CURVE_CUM_MWH",stv_er)
         allocate(MRX_RPS_DV_CURVE_MARGIN(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0030",
     +                               "MRX_RPS_DV_CURVE_MARGIN",stv_er)
         allocate(MRX_RPS_DV_CURVE_CUM_MARGIN(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0031",
     +                           "MRX_RPS_DV_CURVE_CUM_MARGIN",stv_er)
         allocate(MRX_RPS_DV_CURVE_INDEX(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0032",
     +                                "MRX_RPS_DV_CURVE_INDEX",stv_er)
         allocate(MRX_RPS_DV_UNIT_NO(PG),stat=stv_er)
         call check_alloc("cla_objt:0033","MRX_RPS_DV_UNIT_NO",stv_er)
         allocate(MRX_RPS_DV_UNIT_INDEX(NUM_TRANS,PG),stat=stv_er)
         call check_alloc("cla_objt:0034",
     +                                 "MRX_RPS_DV_UNIT_INDEX",stv_er)
...
!***********************************************************************
         ...
         DO I = 1, RETROFIT_COUNTER
            ...
            IF(RETROFIT_CANDIDATE(U) /= 'T') THEN 
              CYCLE
            ENDIF
            IF(RETIREMENTS_CO2(U) < 0.01 .OR.
     +                  RETIREMENTS_MWH(U) < 0.01 .OR.
     +                         RETIREMENTS_MW(U) < 0.0001) THEN 
              CYCLE
            ENDIF
            IF(.NOT. GRX_CONVERGED) THEN
...
!***********************************************************************
     ...
     +                          RETIRED_UNITS = RESET_CL_RETIRED_UNITS() !RESETS THE UNITS RETIRED IN THE LOOP
         ...
         DO U = 1, NUNITS
            ...
            IF(FIRST_RETIREMENT_YEAR(U) > CURRENT_YEAR) THEN 
              CYCLE
            ENDIF
...
! CHANGING INDEX DEFINITION
     ...
     +                       RETIREMENTS_CO2_PRICE(RETIREMENTS_INDEX(I))
         ...
         DO J = 1, ABATE_RETROFIT_COUNTER
            ...
            IF(.NOT. RETROFIT_UNIT_IS_ACTIVE(U)) THEN 
              CYCLE
            ENDIF
            IF(RETROFIT_CANDIDATE(U) /= 'T') THEN 
              CYCLE
            ...
            IF(ALLOCATED(GRX_BOP_RETROFIT_ACTIVE)) THEN
               IF(GRX_BOP_RETROFIT_ACTIVE(U)) THEN 
                 CYCLE
               ENDIF
            ENDIF
            IF(HardWiredRetrofitProject(U) .AND.
     +                  .NOT. HWRetrofitProjectThisYear(U)) THEN 
              CYCLE
            ENDIF
            IF(OFLINE(U) < 100*(BASE_YEAR + R_YEAR - 1900)) THEN 
              CYCLE
            ENDIF
            I = I + 1
            RETIRE_RETRO_POSITION(I) = I
     ...
     +                                   ANN_ECON_RETIRE_COUNTER))) THEN
         ...
         ELSE
            ...
            DO COUNTER = 1, ANN_ECON_RETIRE_COUNTER
               IF(R_CO2_REDUCTION >
     +              RETIREMENTS_CUM_CO2(
     +                              CO2_ABATEMENT_INDEX(COUNTER))) THEN 
                 CYCLE
               ENDIF
               EXIT
     ...
     +        RETIRE_RETRO_CUM_CO2(RETIRE_RETRO_COUNT_AFTER_SORT) ) THEN
         ...
         ELSE
            ...
            DO COUNTER = 1, RETIRE_RETRO_COUNT_AFTER_SORT
               IF(R_CO2_REDUCTION >
     +              RETIRE_RETRO_CUM_CO2(COUNTER) ) THEN 
                 CYCLE
               ENDIF
               EXIT
...
!
         ...
         DO COUNTER =  NEXT_MRX_RETIREMENT, ANN_ECON_RETIRE_COUNTER
            ...
            IF(CO2_ABATEMENT_INDEX(COUNTER) <= 0) THEN 
              CYCLE
            ENDIF
...
!***********************************************************************
     ...
     +                                                    stat=stv_er)
         call check_alloc("cla_objt:0035",
     +                             "TRANS_GROUP_FOR_DATA_BASE",stv_er)
...
!***************************************************************
     ...
     +                          CAPACITY_MARKET_TYPE(I) == 'NONE') THEN 
              CYCLE
            ENDIF
...
!
     ...
     +                     ANNUAL_CL_MAINT_MW,MAINT_DIVISIBLE)
         ALLOCATE(ANNUAL_CL_CAPACITY(2,NUNITS,12),stat=stv_er)
         call check_alloc("cla_objt:0036","ANNUAL_CL_CAPACITY",stv_er)
     
      ALLOCATE(ANNUAL_CL_MAINTENANCE(NUNITS,13),stat=stv_er)
      call check_alloc("cla_objt:0037","ANNUAL_CL_MAINTENANCE",stv_er)
     
      ALLOCATE(ANNUAL_CL_MAINT_MW(NUNITS),stat=stv_er)
      call check_alloc("cla_objt:0038","ANNUAL_CL_MAINT_MW",stv_er)
     
      ALLOCATE(MAINT_DIVISIBLE(NUNITS),stat=stv_er)
      call check_alloc("cla_objt:0039","MAINT_DIVISIBLE",stv_er)
...
!
     ...
     +                                         MAINT_INDEX_DAYS,
     +                                         MAINT_INDEX_MONTHLY_MW)
         ALLOCATE(MAINT_INDEX(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0040","MAINT_INDEX",stv_er)
     
         ALLOCATE(MAINT_INDEX_MW(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0041","MAINT_INDEX_MW",stv_er)
     
         ALLOCATE(MAINT_INDEX_DAYS(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0042","MAINT_INDEX_DAYS",stv_er)
     
         ALLOCATE(MAINT_INDEX_MONTHLY_MW(NUNITS,13),stat=stv_er)
         call check_alloc("cla_objt:0043",
     +                                "MAINT_INDEX_MONTHLY_MW",stv_er)
     
!
!
         IF(ALLOCATED(MAINTENANCE_DAYS)) DEALLOCATE(MAINTENANCE_DAYS)
         ALLOCATE(MAINTENANCE_DAYS(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0044","MAINTENANCE_DAYS",stv_er)
...
! MARKET_RESOURCE 7/5/02. FOR TVA
     ...
     +                                          MARKET_RESOURCE_COUNTER)
         ALLOCATE(MARKET_RESOURCE_INDEX(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0045",
     +                                 "MARKET_RESOURCE_INDEX",stv_er)
     
         ALLOCATE(MARKET_RESOURCE_COUNTER(NUNITS),stat=stv_er)
         call check_alloc("cla_objt:0046",
     +                               "MARKET_RESOURCE_COUNTER",stv_er)
...
!
               IF(REGIONAL_PA_MAINT_SCHEDULING) THEN
                  TG = TRANSACTION_GROUP_ID(I)
                  TG = GET_TRANS_GROUP_POSITION(TG)
                  IF(TG_2_PLANNING_AREA(TG) /= PA) THEN 
                    CYCLE
                  ENDIF
               ELSEIF(REGIONAL_TG_MAINT_SCHEDULING) THEN
                  TG = TRANSACTION_GROUP_ID(I)
...
!
     ...
     +          NUMBER_OF_EMISSION_TYPES,R_NUNITS,0:12),stat=stv_er) 
            call check_alloc("cla_objt:0047",
     +                             "MON_MDS_CL_UNIT_EMISSIONS",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_EMISSIONS_COST(
     +          NUMBER_OF_EMISSION_TYPES,R_NUNITS,0:12),stat=stv_er) 
            call check_alloc("cla_objt:0048",
     +                        "MON_MDS_CL_UNIT_EMISSIONS_COST",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_CAPACITY(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0049",
     +                              "MON_MDS_CL_UNIT_CAPACITY",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_ENERGY(R_NUNITS,0:12),stat=stv_er)
            call check_alloc("cla_objt:0050",
     +                                "MON_MDS_CL_UNIT_ENERGY",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_FUEL_COST(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0051",
     +                             "MON_MDS_CL_UNIT_FUEL_COST",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_VAR_COST(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0052",
     +                              "MON_MDS_CL_UNIT_VAR_COST",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_FIXED_COST(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0053",
     +                            "MON_MDS_CL_UNIT_FIXED_COST",stv_er)
     
            ALLOCATE(MON_MDS_NUC_FUEL_ADDER_COST(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0054",
     +                           "MON_MDS_NUC_FUEL_ADDER_COST",stv_er)
     
            ALLOCATE(MON_MDS_ECO_SALES_REV_FROM(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0055",
     +                            "MON_MDS_ECO_SALES_REV_FROM",stv_er)
     
            ALLOCATE(MON_MDS_ECO_SALES_ENRG_FROM(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0056",
     +                           "MON_MDS_ECO_SALES_ENRG_FROM",stv_er)
     
            ALLOCATE(MON_MDS_ECO_PUCH_COST_FROM(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0057",
     +                            "MON_MDS_ECO_PUCH_COST_FROM",stv_er)
     
            ALLOCATE(MON_MDS_ECO_PUCH_ENRG_FROM(R_NUNITS,0:12),
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0058",
     +                            "MON_MDS_ECO_PUCH_ENRG_FROM",stv_er)
     
            ALLOCATE(MON_MDS_ICAP_REVENUES(R_NUNITS,0:12),stat=stv_er)
            call check_alloc("cla_objt:0059",
     +                                 "MON_MDS_ICAP_REVENUES",stv_er)
     
            ALLOCATE(MON_MDS_CL_UNIT_MMBTUS(R_NUNITS,0:12),stat=stv_er)
            call check_alloc("cla_objt:0060",
     +                                "MON_MDS_CL_UNIT_MMBTUS",stv_er)
...
!
     ...
     +                                                    stat=stv_er)
            call check_alloc("cla_objt:0061",
     +                                   "ASSET_CLASS_POINTER",stv_er)
     ...
     +                                                  stat=stv_er) 
         call check_alloc("cla_objt:0062",
     +                                "CL_ANN_CLASS_FUEL_COST",stv_er)
     
         ALLOCATE(CL_ANN_MULT_FUEL_COST(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0063",
     +                                 "CL_ANN_MULT_FUEL_COST",stv_er)
     
         ALLOCATE(FE_ANN_MULT_FUEL_COST(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0064",
     +                                 "FE_ANN_MULT_FUEL_COST",stv_er)
     
         ALLOCATE(NF_FUEL_LEASED_BY_CLASS(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0065",
     +                               "NF_FUEL_LEASED_BY_CLASS",stv_er)
     
         ALLOCATE(NF_FUEL_OWNED_BY_CLASS(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0066",
     +                                "NF_FUEL_OWNED_BY_CLASS",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_VAR_COST(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0067",
     +                                 "CL_ANN_CLASS_VAR_COST",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_FIXED_COST(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0068",
     +                               "CL_ANN_CLASS_FIXED_COST",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_CAPACITY(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0069",
     +                                 "CL_ANN_CLASS_CAPACITY",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_ENERGY(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,4),stat=stv_er) 
         call check_alloc("cla_objt:0070","CL_ANN_CLASS_ENERGY",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_REVENUE(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,5),stat=stv_er) 
         call check_alloc("cla_objt:0071","CL_ANN_CLASS_REVENUE",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_PURCHASES(
     +               0:NUMBER_OF_CAP_LIMITED_CLASSES,5),stat=stv_er) 
         call check_alloc("cla_objt:0072",
     +                                "CL_ANN_CLASS_PURCHASES",stv_er)
     
         ALLOCATE(CL_ANN_CLASS_EMISSIONS(NUMBER_OF_EMISSION_TYPES,
     +                 0:NUMBER_OF_CAP_LIMITED_CLASSES),stat=stv_er) 
         call check_alloc("cla_objt:0073",
     +                                "CL_ANN_CLASS_EMISSIONS",stv_er)
     
         ALLOCATE(CL_ANN_MULT_VAR_COST(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0074","CL_ANN_MULT_VAR_COST",stv_er)
     
         ALLOCATE(
     +           CL_ANN_MULT_FIXED_COST(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0075",
     +                                "CL_ANN_MULT_FIXED_COST",stv_er)
     
         ALLOCATE(CL_ANN_MULT_REVENUE(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0076","CL_ANN_MULT_REVENUE",stv_er)
     
         ALLOCATE(CL_ANN_MULT_CAPACITY(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0077","CL_ANN_MULT_CAPACITY",stv_er)
     
         ALLOCATE(CL_ANN_MULT_ENERGY(0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0078","CL_ANN_MULT_ENERGY",stv_er)
     
         ALLOCATE(CL_ANN_MULT_PURCHASES(0:
     +  NUMBER_OF_CAP_LIMITED_CLASSES,
     +                             NUM_FUEL_CATEGORIES),stat=stv_er) 
         call check_alloc("cla_objt:0079",
     +                                 "CL_ANN_MULT_PURCHASES",stv_er)
     
         ALLOCATE(NUC_FUEL_LEASED_BURN_BY_CLASS(
     +                 0:NUMBER_OF_CAP_LIMITED_CLASSES),stat=stv_er) 
         call check_alloc("cla_objt:0080",
     +                         "NUC_FUEL_LEASED_BURN_BY_CLASS",stv_er)
     
         ALLOCATE(NUC_FUEL_OWNED_BURN_BY_CLASS(
     +                 0:NUMBER_OF_CAP_LIMITED_CLASSES),stat=stv_er) 
         call check_alloc("cla_objt:0081",
     +                          "NUC_FUEL_OWNED_BURN_BY_CLASS",stv_er)
     
         ALLOCATE(NUCLEAR_MWH_BY_CLASS(0:NUMBER_OF_CAP_LIMITED_CLASSES),
     +                                                    stat=stv_er)
         call check_alloc("cla_objt:0082","NUCLEAR_MWH_BY_CLASS",stv_er)
     
         ALLOCATE(NUCLEAR_MMBTU_BY_CLASS(
     +                 0:NUMBER_OF_CAP_LIMITED_CLASSES),stat=stv_er) 
         call check_alloc("cla_objt:0083",
     +                                "NUCLEAR_MMBTU_BY_CLASS",stv_er)
     
         ALLOCATE(INTRA_COMPANY_REVENUES(0:1024,0:3),stat=stv_er)
         call check_alloc("cla_objt:0084",
     +                                "INTRA_COMPANY_REVENUES",stv_er)
     
         ALLOCATE(INTRA_COMPANY_NF_BURN(0:1024),stat=stv_er)
         call check_alloc("cla_objt:0085",
     +                                 "INTRA_COMPANY_NF_BURN",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_FUEL_COST
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0086",
     +                            "MON_MDS_CL_CLASS_FUEL_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_FUEL_COST
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0087",
     +                             "MON_MDS_CL_MULT_FUEL_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_VAR_COST
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0088",
     +                              "MON_MDS_CL_MULT_VAR_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_FIXED_COST
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0089",
     +                            "MON_MDS_CL_MULT_FIXED_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_CAPACITY
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0090",
     +                              "MON_MDS_CL_MULT_CAPACITY",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_ENERGY
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0091",
     +                                "MON_MDS_CL_MULT_ENERGY",stv_er)
     
         ALLOCATE(MON_MDS_CL_MULT_PURCHASES
     +                         (0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                        NUM_FUEL_CATEGORIES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0092",
     +                             "MON_MDS_CL_MULT_PURCHASES",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_VAR_COST
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0093",
     +                             "MON_MDS_CL_CLASS_VAR_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_FIXED_COST
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0094",
     +                           "MON_MDS_CL_CLASS_FIXED_COST",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_REVENUE
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,5,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0095",
     +                              "MON_MDS_CL_CLASS_REVENUE",stv_er)
     
         ALLOCATE(MON_MDS_CL_CAP_REVENUE
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,5,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0096",
     +                                "MON_MDS_CL_CAP_REVENUE",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_CAPACITY
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0097",
     +                             "MON_MDS_CL_CLASS_CAPACITY",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_PURCHASES
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,5,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0098",
     +                            "MON_MDS_CL_CLASS_PURCHASES",stv_er)
     
         ALLOCATE(MON_MDS_CL_CAP_PURCHASES
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,5,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0099",
     +                              "MON_MDS_CL_CAP_PURCHASES",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_ENERGY
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0100",
     +                               "MON_MDS_CL_CLASS_ENERGY",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_EMISSIONS(NUMBER_OF_EMISSION_TYPES,
     +            0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0101",
     +                            "MON_MDS_CL_CLASS_EMISSIONS",stv_er)
     
         ALLOCATE(MON_MDS_CL_CLASS_EMISSIONS_COST(
     +         NUMBER_OF_EMISSION_TYPES,0:NUMBER_OF_CAP_LIMITED_CLASSES,
     +                                            0:12),stat=stv_er) 
         call check_alloc("cla_objt:0102",
     +                       "MON_MDS_CL_CLASS_EMISSIONS_COST",stv_er)
     
!
         ALLOCATE(MON_MDS_INTRA_COMPANY_REVENUES(0:1024,0:3,0:12),
     +                                                    stat=stv_er)
         call check_alloc("cla_objt:0103",
     +                        "MON_MDS_INTRA_COMPANY_REVENUES",stv_er)
     
         ALLOCATE(MON_MDS_INTRA_COMPANY_NF_BURN(0:1024,0:12),
     +                                                    stat=stv_er)
         call check_alloc("cla_objt:0104",
     +                         "MON_MDS_INTRA_COMPANY_NF_BURN",stv_er)
     
         ALLOCATE(MON_MDS_NF_FUEL_LEASED_BC
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0105",
     +                             "MON_MDS_NF_FUEL_LEASED_BC",stv_er)
     
         ALLOCATE(MON_MDS_NF_FUEL_OWNED_BC
     +         (0:NUMBER_OF_CAP_LIMITED_CLASSES,4,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0106",
     +                              "MON_MDS_NF_FUEL_OWNED_BC",stv_er)
     
         ALLOCATE(MON_MDS_NUC_FUEL_LEASE_BURN_BC
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0107",
     +                        "MON_MDS_NUC_FUEL_LEASE_BURN_BC",stv_er)
     
         ALLOCATE(MON_MDS_NUC_FUEL_OWNED_BURN_BC
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0108",
     +                        "MON_MDS_NUC_FUEL_OWNED_BURN_BC",stv_er)
     
         ALLOCATE(MON_MDS_NUCLEAR_MWH_BY_CLASS
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0109",
     +                          "MON_MDS_NUCLEAR_MWH_BY_CLASS",stv_er)
     
         ALLOCATE(MON_MDS_NUCLEAR_MMBTU_BY_CLASS
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0110",
     +                        "MON_MDS_NUCLEAR_MMBTU_BY_CLASS",stv_er)
     
         ALLOCATE(MON_MDS_ICAP_REV_BY_CLASS
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0111",
     +                             "MON_MDS_ICAP_REV_BY_CLASS",stv_er)
     
         ALLOCATE(MONTHLY_AC_WHOLESALE_PROD_COST
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0112",
     +                        "MONTHLY_AC_WHOLESALE_PROD_COST",stv_er)
     
         ALLOCATE(MONTHLY_AC_ECITY_VAR_PROD_COST
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0113",
     +                        "MONTHLY_AC_ECITY_VAR_PROD_COST",stv_er)
     
         ALLOCATE(MONTHLY_AC_ECITY_NEW_FIX_COST
     +           (0:NUMBER_OF_CAP_LIMITED_CLASSES,0:12),stat=stv_er) 
         call check_alloc("cla_objt:0114",
     +                         "MONTHLY_AC_ECITY_NEW_FIX_COST",stv_er)
...
!***********************************************************************
         ...
         call check_alloc("cla_objt:0117","ASSET_CLASS_LIST",stv_er)
     
         ALLOCATE(ASSET_ALLOCATION_LIST(AVAIL_DATA_YEARS),stat=stv_er)
         call check_alloc("cla_objt:0118",
     +                                 "ASSET_ALLOCATION_LIST",stv_er)
...
!
     ...
     +                CURRENT_YEAR_COMPARISON - OFLINE(UNUM) > 12) THEN 
              CYCLE
            ENDIF
...
!
               ASSET_CLASS = ASSET_CLASS + 1
               IF(ASSET_CLASS <= 0 .OR.
     +                 ASSET_CLASS > MAX_CAP_LIMITED_CLASS_ID_NUM) THEN 
                 CYCLE
               ENDIF
...
!***********************************************************************
         ...
         ALLOCATE(ASSET_CLASS_LIST(AVAIL_DATA_YEARS),stat=stv_er)
         call check_alloc("cla_objt:0119","ASSET_CLASS_LIST",stv_er)
     
         ALLOCATE(ASSET_ALLOCATION_LIST(AVAIL_DATA_YEARS),stat=stv_er)
         call check_alloc("cla_objt:0120",
     +                                 "ASSET_ALLOCATION_LIST",stv_er)
...
!***********************************************************************
      function GET_MAX_STATE_PROVINCE_NO()
!***********************************************************************
      use cl_unit_vars
      ...
      RETURN
      end function GET_MAX_STATE_PROVINCE_NO
!***********************************************************************
