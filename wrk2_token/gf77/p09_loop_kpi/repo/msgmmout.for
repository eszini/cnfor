...
!            CURRENT_PORTION_OF_LTD(0) = LTD_RETIREMENTS
!            DO MO = 0, 12
...
!            IF(VARIABLE == MonthlyEmissionCredits) CYCLE
            IF(VARIABLE == MonthlyEmissionCredits
     +                                         .AND. .NOT. WVPA()) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE == Monthly_LTD_Amort_Interest) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE == monthly_oth_taxes) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE == Monthly_Operating_Revenue_Tax) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE == mthy_nf_incm_tax_dfrls_cr) THEN 
              CYCLE
            ENDIF
!
            IF(VARIABLE == Monthly_Property_Taxes) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE==MonthlyExpFilePropertyTaxes) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE==MonthlyExpFileOtherTaxes) THEN 
              CYCLE
            ENDIF
            IF(VARIABLE==mthy_exp_file_opng_revnue_tax) THEN 
              CYCLE
            ENDIF
...
!            IF(VARIABLE == CashEmissionCredits) CYCLE
...
!
! TAL TAXES ITES 81-95; TOTAL EXPENSE 150 OP INCOME 151 INCOME_VARIABLES(0:12,mthy_fed_atl_income_tax_pd)
!
...
!
     ...
     +                 INCOME_VARIABLES(0,Monthly_LTD_Total_Interest)
     +                 + LTD_BOOKED_ISSUED_INTEREST(0)
     +                 + ANNUAL_VARS(630) ! was 384 but only debit file is inc. LTD INTEREST AMORTIZATION
            DO MO = 1, 12
...
!
            DO MO = 1, 12
               IF(LTD_ISSUED_DISTRIBUTION(MO) == 0.) THEN 
                 CYCLE
               ENDIF
...
!
     ...
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE==monthly_cash_lti) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE==monthly_debt_lti) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_short_lti) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_common_stock) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_retained_earnings_bal) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE ==  Monthly_TOTAL_COMMON_EQUITY) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_preferred_stock) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_long_term_debt) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_total_capital) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_short_term_debt_bal) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_total_liabilities) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_total_assets) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_customer_deposits) THEN 
                    CYCLE
                  ENDIF
...
!
     ...
     +                                          THIS_IS_REPORTING_CLASS)
            DO VARIABLE = 1, INCOME_VARS
               IF(VARIABLE == Monthly_Issue_Expense_Amorts) THEN 
                 CYCLE
               ENDIF
               IF(VARIABLE == Monthly_PS_Amort_Dividends) THEN 
                 CYCLE
               ENDIF
               IF(CLASS_TYPE == SBU .OR.
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE == Monthly_Investment_Earnings) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mthly_invstmt_earngs_rcvbl) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_LTD_Total_Interest) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == MonthlySTDInterest) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mty_intst_on_nts_pyble) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_Total_PS_Dividends) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_CommonDividends) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mty_totl_lti_income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mty_mdl_lt_nvst_income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE ==
     +                        mt_dbt_file_linvst_income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_notes_receivable_income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_Dividend_70_Earnings) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mty_rglr_dvd_income) THEN 
                    CYCLE
                  ENDIF

                  IF(VARIABLE == BTL_Monthly_STInvestmet_Income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == BTL_Monthly_Interest_Income) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == BTL_Monthly_Income_Taxes) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_Shares_Outstanding) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mthy_fed_atl_income_tax_pd) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mt_fed_btl_incm_tax_pd) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == mty_tot_fed_incm_tax_pd) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_LTD_Premium_Amort) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Monthly_PS_Premium_Amort) THEN 
                    CYCLE
                  ENDIF
                  ...
                  IF(.NOT. EXCLUDE_SBU_PROPERTY_TAX) THEN
                     ...
                     ENDIF
                     IF(VARIABLE == 
     +                    mth_modl_calcd_prop_taxes) THEN 
                       CYCLE
                     ENDIF
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_REVENUE_TAX .AND. 
     +                  VARIABLE == Monthly_Operating_Revenue_Tax) THEN 
                    CYCLE
                  ENDIF
                  ...
                  IF(.NOT. EXCLUDE_SBU_STATE_INCOME_TAX) THEN
                     IF(VARIABLE == mthy_state_atl_incm_tax_pd) THEN
                       CYCLE
                     ENDIF
                     IF(VARIABLE == mt_st_btl_incm_tax_pd) THEN
                       CYCLE
                     ENDIF
                     IF(VARIABLE == mty_tot_st_incm_tax_pd) THEN
                       CYCLE
                     ENDIF
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_CAPITAL_TAX .AND. 
     +               (VARIABLE == Monthly_State_Tax_on_Capital .OR.
     +                VARIABLE == Monthly_Federal_Tax_on_Capital)) THEN 
                    CYCLE
                  ENDIF
               ELSEIF(CLASS_TYPE == SUBSIDIARY) THEN
     ...
     +                            SUM(SUB_TAXABLE_INCOME_B4_DEDUCTS(1:))
         ...
         IF(CLASS_LINK_2 >= -1 .AND. .NOT. DONT_TALLY_THIS_CLASS) THEN
            DO VARIABLE = 1, CASH_VARS
               IF(CLASS_TYPE == SBU .OR.
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE==cash_fm_repaid_issued_notes) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == csh_4_redeeming_notes_owed) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_4_new_notes_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_fm_notes_issd_by_othrs) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == Cash_Interest_on_Notes_Payable) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_investmt_divnd_earngs) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_invest_interest_ernings) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ltd_interest) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_std_interest) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ps_dividends) THEN 
                    CYCLE
                  ENDIF
                  ...
                  IF(VARIABLE == cash_fed_income_taxs_pd) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_state_on_capital) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_fed_tax_on_capital) THEN 
                    CYCLE
                  ENDIF
                  ...
                  IF(VARIABLE==cash_subsdry_st_taxes_pd) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_consolidate_taxes_pd) THEN 
                    CYCLE
                  ENDIF

                  IF(VARIABLE == cash_ps_retirements) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ltd_retirements) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_common_stock_buyback) THEN
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ps_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ltd_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_std_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_common_stock_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_closing_balance) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_opening_balance) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_period_change) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == common_shares_issued) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_common_issue_expense) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_ltd_ps_issue_expense) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_change_ltinvestments) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_change_debt_investments) THEN 
                    CYCLE
                  ENDIF
!
...
!
                  IF(VARIABLE ==
     +                       cash_mdl_calcd_prop_taxes) THEN 
                  ...
                  IF(.NOT. EXCLUDE_SBU_PROPERTY_TAX) THEN
                     IF(VARIABLE == cash_property_taxes) THEN 
                       CYCLE
                     ENDIF
                     IF(VARIABLE == cash_exp_file_prop_taxes) THEN 
                       CYCLE
                     ENDIF
                     IF(VARIABLE == 
     +                       cash_mdl_calcd_prop_taxes) THEN 
                  ...
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_REVENUE_TAX .AND. 
     +                     VARIABLE == cash_operating_revenue_tax) THEN 
                    CYCLE
                  ENDIF
     ...
     +                   VARIABLE == cash_st_income_taxes_pd) THEN 
                    CYCLE
                  ENDIF
                  IF(.NOT. EXCLUDE_SBU_CAPITAL_TAX .AND. 
     +                  (VARIABLE == cash_state_on_capital .OR.
     +                   VARIABLE == cash_fed_tax_on_capital)) THEN 
                    CYCLE
                  ENDIF
               ELSEIF(CLASS_TYPE == SUBSIDIARY) THEN
                  ...
                  IF(VARIABLE == cash_subsidiary_dividends) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == cash_equity_to_subsidiaries) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == common_shares_issued) THEN 
                    CYCLE
     ...
     +            + CASH_VARIABLES(:,cash_exp_file_prop_taxes)  
            DO VARIABLE = 1, CASH_VARS 
               ...
               IF(VARIABLE == cash_subsdry_st_taxes_pd ) THEN 
                 CYCLE
               ENDIF
               IF(VARIABLE==cash_subsdry_st_taxes_pd_by_pt) THEN
                 CYCLE
               ENDIF
               IF(VARIABLE == cash_consolidate_taxes_pd) THEN 
                 CYCLE
...
!
     ...
     +                                          OTHER_LT_LIABILITIES_BOY
            DO VARIABLE = 1, BAL_SHEET_VARS
               ...
               IF(VARIABLE == monthly_gain_on_reaqd_debt_bal) THEN 
                 CYCLE
               ENDIF
               IF(VARIABLE == mty_regty_liablties_bal) THEN 
                 CYCLE
               ENDIF
...
!            STI_CASH_LAG = 0
         ...
         IF(END_POINT == START_STUDY_ENDPOINT()) THEN
            THIS_YEARS_RECORD = 7 + 
     +                        INT(13*(NUMBER_OF_REPORTING_CLASSES - 1)) ! PARENT IS INC
            IF(LAHEY_LF95()) THIS_YEARS_RECORD = THIS_YEARS_RECORD + 1
            NEXT_YEARS_START_RECORD = THIS_YEARS_RECORD
            INCREMENT_RECORDS= INT(13*NUMBER_OF_REPORTING_CLASSES) ! PARENT IS INC
         ELSE
...
!
     ...
     +                           SUM(TAXES_LINKED_LEVEL(1:,VARIABLE,-1))
               ...
               IF(CLASS_TYPE == SBU .OR.
     +                               CLASS_TYPE == REGULATED_GROUP) THEN
                  IF(VARIABLE == monthly_atl_state_income_taxes) THEN 
                    CYCLE
                  ENDIF
                  IF(VARIABLE == monthly_btl_state_income_taxes) THEN 
                    CYCLE
                  ENDIF
...
!***********************************************************************
!
! ALLOCATED TO THE REVENUE TAX TO  MONTHS UNTIL SPECIFIC MONTHLY INFO IS AVAILABLE
!
      use spindriftlib
      use prod_arrays_dimensions
      use mthnmcom
...
!***********************************************************************
!
! NOTE THAT THE VARIABLE POSITIONS ARE OFFSET BY 30 TO USE THE ANNUAL LOCATION
! VALUES.  
! 
!
...
!    +          + CASH_VARIABLES(R_MO,CashDOEDecommissioning) ! 9/7/00 removed to match annual
