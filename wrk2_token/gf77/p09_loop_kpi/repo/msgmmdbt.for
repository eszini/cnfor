...
!     LOGICAL*1 MONTHLY_MIDAS_ACTIVE/.TRUE./
...
!
     ...
     +             EQUITY_MARKET
      CHARACTER*20 LHS_DISTRIBUTION
!      CHARACTER*40 DRILLING_ACCOUNT_NAME
...
!    +       R_ISSUE_EXPENSE_AMORT(0:12,0:*),
!    +       R_ISSUE_EXPENSE_BAL(0:12,0:*),
...
!                
     ...
     +                               SUM(ISSUE_EXPENSE_AMORT(1:12,I))
                  ...
                  IF(ISSUE_EXPENSE_BAL(12,I) == 0.) THEN 
                    EXIT
                  ENDIF
               ENDDO
            ELSE 
...
!                
     ...
     +                               SUM(ISSUE_EXPENSE_AMORT(1:12,I))
         ...
         ELSEIF(ISSUE_EXPENSE_BALANCE /= 0.) THEN ! VECTOR NOT USED
            IF(ISSYR > BASE_YR) THEN
               I = ISSYR - BASE_YR
!              ISSUE_EXPENSE_BAL(ISSMO,I) = ISSUE_EXPENSE_BALANCE
               ISSUE_TAX_EXPENSE(ISSMO,I) = ISSUE_EXPENSE_BALANCE
               ISSUE_TAX_EXPENSE(0,I) = ISSUE_EXPENSE_BALANCE
...
!
     ...
     +                            ANNUAL_VECTOR_VALUES(AVAIL_DATA_YEARS)
                  ENDIF
                  DO MO = 1, 12
                     IF(YEAR == YRMAT .AND. MO == MATMO) THEN 
                       EXIT
                     ENDIF
                     ...
                     ELSE
!                        IF(SINKING_FUND(0,NORMALIZE_YR) /= 0. .AND.
!     +                              NORMALIZE_YR + BASE_YR > ISSYR) THEN
...
!
     ...
     +                                      PAYMENT_LAGS_ISSUE_MONTH)
            ELSE
               IF(YEAR == YRMAT .AND. ISSMO >= MATMO) THEN 
                 EXIT
               ENDIF
...
!     +                                                 ISSDA-INT2(1),YR)
                        IF(WVPA()) THEN ! CURRENT MONTH INTEREST INC
     ...
     +                              + PRINCIPAL_PAYMENT
                        SINKING_FUND(MO,I) = 0.
                        IF(WVPA()) THEN
!                           INTEREST(MO,I) = 30.*RATE(MO,I)*BAL(MO,I)
!     +                                     + INTEREST_EXPENSE_ADJ(MO,I)
                            ACCRUED_INTEREST_PAYABLE = 0.
                        ELSE
                           MGT_ACCURRED_INTEREST = RATE(MO,I)*BAL(MO,I)*
     ...
     +                                      + MGT_ACCURRED_INTEREST
     +                                      + INTEREST_EXPENSE_ADJ(MO,I)
!    +                                RATE(MO,I)*(1.-FRACTION)*BAL(MO,I)
                           ACCRUED_INTEREST_PAYABLE =
     +                                      MGT_ACCURRED_INTEREST
     +                                      + INTEREST_EXPENSE_ADJ(MO,I)
                        ENDIF
!                        INTEREST(0,I) = INTEREST(0,I) + INTEREST(MO,I)
                     ELSE
                        BAL(MO,I) = BAL(MO-1,I)
                        IF(WVPA()) THEN
...
!                    ANNUAL_SUM = 0.
!                     DO MONTH = MO+1, 12
!                        IF(MONTH > MO) SINKING_FUND(MONTH,I) = 0.
!                       ANNUAL_SUM = ANNUAL_SUM + SINKING_FUND(MONTH,I)                         
!                     ENDDO
!                    SINKING_FUND(0,I) = ANNUAL_SUM
                     EXIT
                  ENDIF
               ENDDO ! MO
...
! ELSE (YR < ISSYR) => NO COMPUTATIONS YET
                  ELSEIF(YR == YRMAT .AND. MO == MATMO) THEN
                     ...
                     SINKING_FUND(MO,I) = 0.
                     INTEREST(MO,I) = RATE(MO,I)*BAL(MO-1,I)*(MATDA-1)
!    +                               TIME_FROM_FIRST_DAY(MATMO,MATDA,YR)
     +                                            
                  ELSE
                     IF(SINKING_FUND(MO,I) /= 0.) THEN
                        FRACTION = TIME_FROM_FIRST_DAY(MO,
...
!
! ALINE THE BOOKED VALUES WITH THE CASH FLOWS
!
      IF(TYPE == COMMON) THEN
...
!         DO YR = 1, FINANCIAL_SIMULATION_YEARS
!            DO MO = 0, 12
!               CASH_PAYMENTS(MO,YR) = INTEREST(MO,YR)
!            ENDDO
!         ENDDO
      ELSE
...
! WRITE ASSET RESULTS
!
!        REPORT_ACCOUNT_RESULTS = (DEBT_ASSET_REPORT .AND. DELETE > 1)
!    +                                          .OR. REPORT_ALL_ACCOUNTS
...
!
!                  IF(CURRENT_YR == ISSYR .AND.
!     +                                  (MO == ISSMO .OR. MO == 0)) THEN
!                     PREMIUM_DISCOUNT_EXP = PREM_DISC_BALANCE
!                  ELSE
!                     PREMIUM_DISCOUNT_EXP = 0.
!                  ENDIF                  
!
...
!     +                                      - SINKING_FUND(MO,I)
     ...
     +                           + ASSET_ALLOCATOR * CASH_PAYMENTS(MO,I)
                  ...
                  IF(CLASS_POINTER > AVAIL_DATA_YEARS) THEN 
                    EXIT
                  ENDIF
                  IF(ASSET_CLASS_LIST(CLASS_POINTER) == -99) THEN 
                    EXIT
                  ENDIF
               ENDDO ! CLASS ID
            ELSE
...
!***********************************************************************
!
!        CALL RETURN_NUM_OF_DEBT_CLASSES(NUM_OF_ASSET_CLASSES,
!    +                                      MAX_ASSET_CLASS_NUM)
...
!           CALL RETURN_DEBT_CLASS_POINTER(ASSET_CLASS_POINTER)
         ENDIF
!
...
!               R_CURRENT_LTD_RETIRE = R_CURRENT_LTD_RETIRE
!     +                               + SFLTD(R_PERIOD,YR+1,ASSET_CLASS)
!     +                               + RETLTD(R_PERIOD,YR+1,ASSET_CLASS)
...
!               R_LTD_PREM_BAL = LTD_PREM_BAL(12,YR,ASSET_CLASS)
               R_LTD_PREM_BAL = 
     +                  LTD_NEW_ISSUE_PREM_DISC_AMOUNT(0,YR,ASSET_CLASS)
...
!               R_COMMON_SHARES_ISSUED_2_EQUITY =
!     +              COMMON_STOCK_ISSUED_SHARES(MO,YR,ASSET_CLASS,Equity)
               R_COMMON_STOCK_BUY_FROM_EQUITY =
     +             COMMON_STOCK_BUYBACK_AMOUNT(MO,YR,ASSET_CLASS,Equity)
!               R_COMMON_SHARES_BUY_FROM_EQUITY =
!     +             COMMON_STOCK_BUYBACK_SHARES(MO,YR,ASSET_CLASS,Equity)
               R_RETAINED_EARNINGS_ADJ =
...
!                  INCOME_VARIABLES(:,BTL Monthly LTInvestmet Income) =
!     +                  INVESTMENT_INCOME(:,YR,ASSET_CLASS,
!     +                                                Interest_Earnings)
!     +                  + NOTES_RECEIVABLE(:,YR,ASSET_CLASS,
!     +                                                Interest_Earnings)
...
!$ifdefined(monthly_code) these items need to be made monthly
!              R_LTD_ISSUE_EXP_BAL =
!    +                           LTD_ISSUE_EXP_BAL(MO,YR,ASSET_CLASS)
!              R_LTD_ISSUE_EXP_AMORT =
!    +                         LTD_ISSUE_EXP_AMORT(MO,YR,ASSET_CLASS)
!              R_PS_ISSUE_EXP_BAL =
!    +                            PS_ISSUE_EXP_BAL(MO,YR,ASSET_CLASS)
!              R_PS_ISSUE_EXP_AMORT =
!    +                          PS_ISSUE_EXP_AMORT(MO,YR,ASSET_CLASS)
!              IF(MO == 0) THEN
!                 R_ANN_PS_DIV_LTD_INT = R_ANN_PS_DIV_LTD_INT +
!    +                                   AINTPS(YR,ASSET_CLASS) +
!    +                                   AINTDB(YR,ASSET_CLASS)
...
!                 R_ANN_LTD_INT = R_ANN_LTD_INT +
!    +                                      AINTDB(YR,ASSET_CLASS)
!                 R_PS_PREM_BAL = PS_PREM_BAL(YR,ASSET_CLASS)
!                 R_PS_PREM_AMORT = PS_PREM_AMORT(YR,ASSET_CLASS)
!                 R_LTD_PREM_BAL = LTD_PREM_BAL(YR,ASSET_CLASS)
!                 R_LTD_PREM_AMORT = LTD_PREM_AMORT(YR,ASSET_CLASS)
!                 R_MIPS_INCOME_TAX_DEDUCTION =
!    +                   MIPS_INCOME_TAX_DEDUCTION(YR,ASSET_CLASS)
!              ELSE
!                 R_ANN_PS_DIV_LTD_INT = R_ANN_PS_DIV_LTD_INT +
...
!                 R_ANN_LTD_INT = R_ANN_LTD_INT +
!    +                                      AINTDB(YR,ASSET_CLASS)/12.
!                 R_PS_PREM_BAL = PS_PREM_BAL(YR,ASSET_CLASS)/12.
!                 R_PS_PREM_AMORT = PS_PREM_AMORT(YR,ASSET_CLASS)/12.
!                 R_LTD_PREM_BAL = LTD_PREM_BAL(YR,ASSET_CLASS)/12.
!                 R_LTD_PREM_AMORT = LTD_PREM_AMORT(YR,ASSET_CLASS)/12.
!                 R_MIPS_INCOME_TAX_DEDUCTION =
!    +                     MIPS_INCOME_TAX_DEDUCTION(YR,ASSET_CLASS)/12.
!              ENDIF
!$endif
...
!
     ...
     +                                + RETLTD(0,1,ASSET_CLASS)
               R_CURRENT_LTD_RETIRE =
!     +                          CURRENT_PORTION_OF_LTD(12,0,ASSET_CLASS)
...
!
     ...
     +                                           Cash_Interest_Earnings)
!         R_CURRENT_LTD_RETIRE = SFLTD(R_PERIOD,YR+1,-1)
!     +                          + RETLTD(R_PERIOD,YR+1,-1)
         R_CURRENT_LTD_RETIRE = R_CURRENT_LTD_RETIRE
...
!
     ...
     +             +  NOTES_PAYABLE(:,YR,ASSET_CLASS,Interest_Payments)
!
!            INCOME_VARIABLES(:,Monthly Investment Earnings) =
!     +                   INCOME_VARIABLES(:,Monthly Investment Earnings)
!     +                   + NOTES_RECEIVABLE(:,YR,ASSET_CLASS,
!     +                                  Intra_Company_Interest_Earnings)
            INCOME_VARIABLES(:,monthly_notes_receivable_income) =
...
! IF THE MORTGAGE SECURITY EXISTED IN THE BASE_YEAR; CONSIDER BYRBAL TO
! BE EFFECTIVE ON THE ISSUE'S ANNIVERSARY DATE MATMO/MATDA/BASE_YEAR;
...
!              DEBT WAS ISSUED DURING THIS MODEL YEAR
               ISS(I) = AMT
!              INTEREST(I) = 0. [ value retained from initialization ]
               BAL(I) = AMT
!              SINKING_FUND(MO,I) = 0. [ value retained from initialization ]
            ELSE
...
!              BAL(I) = [ value retained from prior invocation ]
!              SINKING_FUND(I) = [ value retained from prior invocation ]
            ENDIF
         ENDIF
...
!        BAL(I) & SINKING_FUND(I) retain prior mid-year values; payment is due in 1 year
...
!           BAL(I) = 0. [ value retained from initialization ]
         ENDIF
!     ELSE (YR = YRMAT)
      ELSEIF (I == 2) THEN
!        THE MORTGAGE SECURITY EXISTED IN THE BASE_YEAR, AND EXPIRES THIS YEAR
!        BYRBAL EXISTS FOR ONLY A PORTION OF THE MATURITY YEAR
...
!
         ...
         DO YR = 1, FINANCIAL_SIMULATION_YEARS
            IF(YR > YRMAT) THEN 
              EXIT
            ENDIF
            IF(INTEREST(0,YR) == 0. .AND. CARRY_OVER == 0.) THEN 
              CYCLE
            ENDIF
            DO MO = 1, 12
               IF(INTEREST(MO,YR) == 0. .AND. CARRY_OVER == 0.) THEN 
                 CYCLE
               ENDIF
...
!                 FILE NOT UP DATED.  INTEREST PAID MONTHLY               
     ...
     +                         - INTEREST_IN_SPLIT_MONTH(MO,YR)
               ...
               ENDIF
!
!              IF(BALANCE(MO,YR) == 0.) THEN   ! REMOVED 8/28/99 SO THAT INTEREST 
!                                              ! CAN BE ADDED WITHOUT A BALANCE
!                CASH_PAYMENTS(MO,YR) = CASH_PAYMENTS(MO,YR)+CARRY_OVER
!                CARRY_OVER = 0.
!                EXIT
!              ENDIF
            ENDDO
...
!         CASH_PAYMENTS(1,1) = PAYABLE
!         UNPAID_DIVIDENDS = 0.
         UNPAID_DIVIDENDS = PAYABLE
         PS_LAG_VALUE = PAYMENT_LAG
         DO YR = 1, FINANCIAL_SIMULATION_YEARS
            IF(INTEREST(0,YR) == 0. .AND. UNPAID_DIVIDENDS == 0.) THEN 
              CYCLE
            ENDIF
            DO MO = 1, 12
               IF(INTEREST(MO,YR) == 0. .AND.
     +                                     UNPAID_DIVIDENDS == 0.) THEN 
                 CYCLE
               ENDIF
!              IF(YR == IYR .AND. MO < ISSMO) CYCLE ! GOTO 67
...
!                 INTEREST(MO,YR) = DIVIDENDS_DECLARED
                  UNPAID_DIVIDENDS = UNPAID_DIVIDENDS +
     +                                                DIVIDENDS_DECLARED
                  DIVIDENDS_DECLARED = 0.
               ENDIF
!              CASH_PAYMENTS(MO,YR) = 0.
...
!                       LTDINT(I) = LTDINT(I) + (LTD_BAL - REMAIN) * LTDRTE
!                       LTDRET(I) = LTDRET(I)+CURRENT_SINKING_FUND_PAYMENT
...
!                       LTDINT(I) = LTDINT(I) + LTD_BAL * LTDRTE/2.
!                       LTDRET(I) = LTDRET(I) + LTD_BAL
                        EXIT
                     ENDIF
                  ENDDO
               ENDIF
...
!                 PSDIV(I) = PSDIV(I) + (PSINC - REMAIN) * PSRATE
!                 PSRED(I) = PSRED(I) + CURRENT_SINKING_FUND_PAYMENT
...
! ADDED 4/27/88 from FIN               
     ...
     +                                                            PSINC)
!                 PSDIV(I) = PSDIV(I) + PSINC * PSRATE/2.
!                 PSRED(I) = PSRED(I) + PSINC
                  EXIT
               ENDIF
            ENDDO
         ENDIF
...
!
         ...
         DO MO = 1, 12
            IF(MO < PAY_MO .AND. CURRENT_YR == START_PAYING_YR) THEN 
              CYCLE
             ENDIF 
            IF(PAID_WHEN == 'M') THEN
...
!
     ...
     +                  CALL SRP_SINKING_FUNDS_INFO(SRP_SINKING_PATTERN)
         IF(MORTGAGE_DEBT) THEN
            ...
            DO ISSMO = 1, 12
               IF(LTD_ISSUED_DISTRIBUTION(ISSMO) == 0.) THEN 
                 CYCLE
               ENDIF
...
!
     ...
     +                                          + MONTHLY_INTEREST
                        ...
                        SinkingFundPayments(MO) = LTD_RETIRE
                        Balance(MO) = LTD_BAL
                        IF(YR == RUN_YEAR+LDLIFE .OR. LTD_BAL==0.) THEN 
                          EXIT
                        ENDIF
                     ENDIF
                     Balance(MO) = LTD_BAL
